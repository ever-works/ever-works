...
- **isomorphic-git lets the checkout's config choose where credentials go**: unless a call passes them, `push` prefers `remote.<r>.pushurl`, `pull`/`fetch` follow `branch.<ref>.remote` to another remote, and all three route through `http.corsProxy` (Authorization header included). `GitOperations` (`packages/plugin/src/git/git-operations.ts`) passes `remote`, an explicit `url` and an empty `corsProxy` on every network call; a new one must too. Relatedly, a `.git` check on path TEXT is not enough — a link in the checkout or a Windows short name (`GIT~1`) aliases it; see `assertRealWriteTarget` in `apps/api/src/agents/agents.module.ts`.
