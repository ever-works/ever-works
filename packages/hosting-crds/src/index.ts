/**
 * `@ever-works/hosting-crds` — the Ever Works Apps hosting tier's Kubernetes API contract (APW-10).
 *
 ...
 * - The **controller** side (`apps/hosting-operator`) runs inside the hosting cluster and
 *   reconciles them.
 *
 ...
 * to `apps/hosting-operator` (APW-10 T4–T9, T23, T28, T29, T33); see that app's `README.md`.
 *
 * Layout note (owner ruling, 2026-09-20): this content originally shipped as
 * `apps/hosting-operator`. It was moved here because `apps/*` in this monorepo means "a thing
 * that starts a process" — every other entry has a `start` script or a `bin` — and this has
 * neither. `apps/hosting-operator` now holds the process.
 *
