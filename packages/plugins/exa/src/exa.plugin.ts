...
export class ExaSearchPlugin implements IPlugin, ISearchPlugin, IContentExtractorPlugin {
	...
	async validateConnection(settings: Record<string, unknown>): Promise<ConnectionValidationResult> {
		...
		if (!apiKey) {
			return { success: false, message: 'Exa API key is not configured.' };
		}

		try {
			const client = this.getClient(settings);
			await client.search('test', { numResults: 1 });
			return { success: true, message: 'Exa connection verified.' };
		} catch (error) {
			return {
				success: false,
				message: `Exa connection failed: ${error instanceof Error ? error.message : String(error)}`
			};
		}
	}
