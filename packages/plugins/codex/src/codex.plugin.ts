...
const MANIFEST: PluginManifest = {
	...
	readme: [
		...
		'## Authentication',
		'',
		'- **API Key**: connect with an OpenAI API key. Billed per token against your OpenAI platform organization.',
		'- **Access Token**: a Codex access token from your ChatGPT **Business or Enterprise** workspace admin console. This is the credential OpenAI documents for trusted non-interactive Codex runs — scripts, schedulers and private CI runners — and it draws your workspace Codex entitlement instead of per-token API pricing. A token belongs to the workspace user who created it, so give each agent its own rather than sharing one.',
		...
		'',
		'Set `authMode` to pin an agent to one of these. A pinned agent never silently falls back to **per-token API billing**: if its own credential is missing it degrades only to another subscription-backed credential, or fails. The reverse fallback is allowed, because it costs plan quota rather than money you did not authorize.',
		'',
		'## Usage',
		'',
