...
vi.mock('@ever-works/plugin/ai', () => {
	const MockAiOperations = vi.fn().mockImplementation(() => ({
		createChatCompletion: vi.fn().mockResolvedValue({ id: 'test', choices: [], model: 'gpt-4', created: 0 }),
...
describe('OpenAiPlugin', () => {
	...
	describe('lifecycle hooks', () => {
		...
		it('should load successfully', async () => {
			...
			expect(mockContext.logger.log).toHaveBeenCalledWith('OpenAI Plugin loaded');
		});

		it('should enable successfully', async () => {
			const mockContext = createMockContext();
			await plugin.onEnable(mockContext);
			expect(mockContext.logger.log).toHaveBeenCalledWith('OpenAI Plugin enabled');
		});

		it('should disable successfully', async () => {
			const mockContext = createMockContext();
			await plugin.onDisable(mockContext);
			expect(mockContext.logger.log).toHaveBeenCalledWith('OpenAI Plugin disabled');
		});
	...
	describe('validateSettings', () => {
		it('should return valid when apiKey is provided', async () => {
			const result = await plugin.validateSettings({ apiKey: 'sk-test-key' });
	...
	describe('healthCheck', () => {
		it('should return healthy status', async () => {
			...
			expect(health.message).toBe('OpenAI plugin is ready');
