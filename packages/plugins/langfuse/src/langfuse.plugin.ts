...
export class LangfusePlugin implements IPlugin, IPromptProviderPlugin {
	readonly id = 'langfuse';
	readonly name = 'Langfuse';
	readonly version = '1.0.0';
	readonly category: PluginCategory = 'utility';
	readonly capabilities: readonly string[] = ['prompt-provider'];
	...
	readonly settingsSchema: JsonSchema = {
		...
		properties: {
			secretKey: {
				type: 'string',
				title: 'Secret Key',
				description: 'Langfuse secret key (sk-lf-...). Required for API access.',
				'x-secret': true,
				'x-envVar': 'PLUGIN_LANGFUSE_SECRET_KEY'
			},
			publicKey: {
				type: 'string',
				title: 'Public Key',
				description: 'Langfuse public key (pk-lf-...). Required for API access.',
				'x-envVar': 'PLUGIN_LANGFUSE_PUBLIC_KEY'
			},
			baseUrl: {
				type: 'string',
				title: 'Base URL',
				description: 'Langfuse base URL. Only needed for self-hosted instances.',
				default: 'https://cloud.langfuse.com',
				'x-envVar': 'PLUGIN_LANGFUSE_BASE_URL'
			},
			promptLabel: {
				type: 'string',
				title: 'Prompt Label',
				description: 'Label to fetch prompts for (e.g., "production", "staging").',
				default: 'production'
			},
			cacheTtlSeconds: {
				type: 'number',
				title: 'Cache TTL (seconds)',
				description: 'How long to cache fetched prompts locally.',
				default: 300,
				minimum: 0,
				maximum: 86400
			}
		},
		required: ['secretKey', 'publicKey']
	};
	...
	async onLoad(context: PluginContext): Promise<void> {
		...
		context.logger.log('Langfuse prompt provider loaded');
	}
	...
	async getPrompt(key: string, options?: GetPromptOptions): Promise<PromptProviderResult | null> {
		...
		if (!settings || !this.isAvailable(settings)) {
			return null;
		}

		const client = this.createClient(settings);
		const label = (settings.promptLabel as string) ?? options?.label ?? 'production';
		const cacheTtlSeconds = (settings.cacheTtlSeconds as number) ?? options?.cacheTtlSeconds ?? 300;

		try {
			const prompt = await client.prompt.get(key, {
				label,
				cacheTtlSeconds,
				type: 'text'
			});
			...
			return {
				template: prompt.prompt as string,
				version: prompt.version
			};
		} catch (error: unknown) {
			const message = error instanceof Error ? error.message : String(error);

			// NotFoundError is expected when a prompt key hasn't been created in Langfuse
			if (message.includes('not found') || message.includes('404')) {
				return null;
			}

			this.context?.logger.warn(`Langfuse prompt fetch failed for "${key}": ${message}`);
			return null;
		}
	}

	async healthCheck(): Promise<PluginHealthCheck> {
		return { status: 'healthy', message: 'Langfuse prompt provider ready', checkedAt: Date.now() };
	}
	...
	getManifest(): PluginManifest {
		return {
			...
			description:
				'External prompt management via Langfuse. Enables prompt versioning, A/B testing, and production labels for all pipeline prompts.',
			category: this.category,
			capabilities: [...this.capabilities],
			readme: PROMPT_KEY_DOCS,
