...
describe('buildWorkflowPayload', () => {
	...
	it('should omit existing items summary when disabled', () => {
		const payload = buildWorkflowPayload(
			createOptions({
				existing: {
	...
	it('should omit existing items summary when no items exist', () => {
		const payload = buildWorkflowPayload(createOptions());
		expect(payload.existingSummary).toBeUndefined();
	});

	it('should include GitHub repo reference when repo access is enabled', () => {
		const payload = buildWorkflowPayload(
			createOptions({
				config: {
					target_items: 50,
	...
	it('should omit dataSource when repo access is disabled', () => {
		const payload = buildWorkflowPayload(createOptions());
	...
	it('should pass through custom actionParams', () => {
		const payload = buildWorkflowPayload(
			createOptions({
				config: {
					target_items: 50,
					action_params: { channel: '#general', thread_ts: '123.456' }
				}
			})
		);

		expect(payload.actionParams).toEqual({ channel: '#general', thread_ts: '123.456' });
	});
	...
	it('should default target items to 50 when missing', () => {
