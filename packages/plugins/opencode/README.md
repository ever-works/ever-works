...
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Generation never starts / stays at `0%`                           | `opencode` not selected as the active pipeline plugin for this work                   | Open the work → **Plugins** → `pipeline` capability and set `opencode` as the active pipeline; or set it as the global pipeline default in **Settings → Plugins** |
...
| Subprocess error: `command not found`                             | `OpenCode Generator` CLI not installed on the host running the API                    | Install the OpenCode Generator CLI on the API host and ensure it is on `PATH`; verify by running `which <cli>` from the same shell that launches `pnpm dev:api`   |
| Authentication / device-auth flow stalls                          | Device-auth code never confirmed in the upstream IDE / browser                        | Re-run the device-auth flow from **Settings → Plugins → opencode → Connect**, then complete the prompt in the upstream service before the code expires            |
