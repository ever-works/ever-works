...
export class FirecrawlPlugin implements IPlugin, ISearchPlugin, IContentExtractorPlugin {
	...
	async validateConnection(settings: Record<string, unknown>): Promise<ConnectionValidationResult> {
		...
		if (!apiKey) {
			return { success: false, message: 'Firecrawl API key is not configured.' };
		}

		try {
			const client = this.getClient(settings);
			await client.search('test', { limit: 1 });
			return { success: true, message: 'Firecrawl connection verified.' };
		} catch (error) {
			return {
				success: false,
				message: `Firecrawl connection failed: ${error instanceof Error ? error.message : String(error)}`
			};
		}
	}
