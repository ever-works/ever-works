...
describe('workspace-manager', () => {
	...
	describe('createWorkspace', () => {
		it('should create a per-run workspace and create _meta directory', async () => {
			...
			expect(fs.mkdir).toHaveBeenCalledWith('/tmp/claude-code-generator/user1/dir1', { recursive: true });
			expect(fs.mkdtemp).toHaveBeenCalledWith('/tmp/claude-code-generator/user1/dir1/run-');
			expect(fs.mkdir).toHaveBeenCalledWith(expect.stringContaining('_meta'), { recursive: true });
			expect(result).toBe('/tmp/claude-code-generator/user1/dir1/run-123');
		});
	});
	...
	describe('cleanupWorkspace', () => {
		...
		it('should not throw on failure', async () => {
			...
			await expect(cleanupWorkspace('/tmp/claude-code-generator/user1/dir1/run-123')).resolves.not.toThrow();
		});
	});
});
