	...
	getManifest(): PluginManifest {
		return {
			...
			readme: [
				...
				'| Setting        | Description                                                  |',
				'| -------------- | ------------------------------------------------------------ |',
				'| `authMode`     | `subscription` or `api-key` — pins which credential is used   |',
				'| `oauthToken`   | Claude Code OAuth token (bills your Claude plan)             |',
				'| `apiKey`       | Anthropic API key (bills your Console org per token)         |',
				'',
				'### Authentication',
				'',
				'At least one of `oauthToken` or `apiKey` must be provided.',
				'',
				'**OAuth Token** (recommended — no per-token cost):',
				'',
				'```bash',
				'claude setup-token',
				'```',
				...
				'### Billing mode',
				'',
				'`authMode` pins an agent to one credential so you always know which account pays:',
				'',
				'- `subscription` uses `oauthToken` only. If it is missing the run fails rather than quietly spending Console credits.',
				'- `api-key` uses `apiKey`, and falls back to `oauthToken` if no key is configured — that direction costs plan quota rather than money you did not authorize.',
				'',
				'Leave `authMode` unset to keep the original behaviour, where the OAuth token wins when both are configured.',
				'',
				'Credentials come only from these settings. A credential sitting in the server environment is deliberately **not** inherited, because `ANTHROPIC_API_KEY` outranks the OAuth token in the CLI and would silently bill the wrong account.',
				'## Usage',
				'',
