...
const COMPARISON_JSON_SCHEMA = {
	...
	properties: {
		title: {
			type: 'string',
			description: 'SEO-optimized comparison title (e.g., "Vercel vs Netlify: Which Hosting Platform is Better?")'
		},
		summary: {
			type: 'string',
		...
		dimensions: {
			...
			items: {
				type: 'object',
				properties: {
					name: { type: 'string', description: 'Dimension name (e.g., "Performance", "Pricing")' },
					item_a_summary: { type: 'string', description: 'Summary for Item A on this dimension' },
					item_b_summary: { type: 'string', description: 'Summary for Item B on this dimension' },
				...
				required: ['name', 'item_a_summary', 'item_b_summary', 'item_a_score', 'item_b_score', 'winner']
			},
			minItems: 3,
			maxItems: 8
		}
