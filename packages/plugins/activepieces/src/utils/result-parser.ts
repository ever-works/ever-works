...
/**
 * Parses and validates the output from an Activepieces flow execution.
 * Transforms flow output items into Ever Works ItemData format.
 */
export function parseActivepiecesOutput(raw: unknown): ParsedResults {
	if (!raw || (typeof raw !== 'object' && typeof raw !== 'string')) {
		throw new Error('Activepieces flow returned empty or non-object output');
	}
	...
	if (items.length === 0) {
		throw new Error(
			'Activepieces flow returned a valid response but with no usable items. ' +
				'Ensure the flow ends with a Return Response action that emits { items: [...] }. ' +
				'Inspect the flow run logs in the Activepieces dashboard.'
		);
	}
...
/**
 * Normalizes the raw Activepieces output into the expected shape.
 * Activepieces flows can return results in many shapes depending on whether the
 * Return Response action wraps the body or returns it directly.
 */
function normalizeOutput(raw: unknown): ActivepiecesFlowOutput {
	if (typeof raw === 'string') {
		...
		throw new Error(
			'Activepieces flow returned a string that is not valid JSON. ' +
				'Ensure the Return Response action emits a JSON object with { items: [...] }. ' +
				`Received: ${preview}`
		);
	}

	if (Array.isArray(raw)) {
		return { items: raw as ActivepiecesOutputItem[] };
	}

	const obj = raw as Record<string, unknown>;

	if ('items' in obj) {
		const items: ActivepiecesOutputItem[] = Array.isArray(obj.items) ? obj.items : [];
		return {
			items,
			categories: obj.categories as ActivepiecesFlowOutput['categories'],
			tags: obj.tags as ActivepiecesFlowOutput['tags'],
			brands: obj.brands as ActivepiecesFlowOutput['brands']
		};
	}

	// Activepieces' Return Response action commonly wraps user output under "body".
	for (const key of ['body', 'output', 'result', 'data', 'response', 'value']) {
	...
	throw new Error(
		'Activepieces flow output does not contain an "items" array. ' +
...
function parseCategories(rawCategories: ActivepiecesFlowOutput['categories'], items: ItemData[]): Category[] {
...
function parseTags(rawTags: ActivepiecesFlowOutput['tags'], items: ItemData[]): Tag[] {
...
function parseBrands(rawBrands: ActivepiecesFlowOutput['brands'], items: ItemData[]): Brand[] {
