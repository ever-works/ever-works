...
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Generation never starts / stays at `0%`                           | `activepieces` not selected as the active pipeline plugin for this work               | Open the work → **Plugins** → `pipeline` capability and set `activepieces` as the active pipeline; or set it as the global pipeline default in **Settings → Plugins** |
...
| Webhook returns `404` / `Not Found`                               | Scenario / Zap not enabled or webhook URL stale                                       | In the Activepieces Automation dashboard verify the scenario is **active**, copy a fresh webhook URL into the plugin settings, and trigger a test run                 |
