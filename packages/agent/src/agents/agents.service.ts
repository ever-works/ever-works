...
export class AgentsService {
	...
	async list(userId: string, filter: ListAgentsFilter = {}): Promise<{
		rows: AgentDto[];
		total: number;
	}> {
	...
	async create(userId: string, input: CreateAgentInput): Promise<AgentDto> {
		...
		if (!slug) {
			throw new BadRequestException('Agent name must contain at least one alphanumeric character.');
		}
		...
		this.validateAvatarFields(avatarMode, input.avatarIcon ?? null, input.avatarImageUploadId ?? null);
	...
	async update(userId: string, id: string, input: UpdateAgentInput): Promise<AgentDto> {
		...
		if (input.maxSkillContextTokens !== undefined) patch.maxSkillContextTokens = input.maxSkillContextTokens;
		...
		if (input.pauseAfterFailures !== undefined) patch.pauseAfterFailures = input.pauseAfterFailures;
		...
		if (input.avatarMode !== undefined || input.avatarIcon !== undefined || input.avatarImageUploadId !== undefined) {
			const mode = input.avatarMode ?? agent.avatarMode;
			const icon = input.avatarIcon !== undefined ? input.avatarIcon : agent.avatarIcon ?? null;
			const upload = input.avatarImageUploadId !== undefined ? input.avatarImageUploadId : agent.avatarImageUploadId ?? null;
	...
	async transition(userId: string, id: string, to: AgentStatus): Promise<AgentDto> {
		...
		if (!allowed.includes(to)) {
			throw new BadRequestException(
				`Cannot transition Agent from ${agent.status} to ${to}.`,
			);
		}
	...
	private validateScopeOwnership(input: Pick<CreateAgentInput, 'scope' | 'missionId' | 'ideaId' | 'workId'>): void {
		const popCount = [input.missionId, input.ideaId, input.workId].filter(Boolean).length;
		switch (input.scope) {
			case AgentScope.TENANT:
				if (popCount > 0) {
					throw new BadRequestException('Tenant-scoped Agents must not have missionId/ideaId/workId.');
				}
				break;
			case AgentScope.MISSION:
				if (!input.missionId || popCount !== 1) {
					throw new BadRequestException('Mission-scoped Agents require missionId (and only missionId).');
				}
				break;
			case AgentScope.IDEA:
				if (!input.ideaId || popCount !== 1) {
					throw new BadRequestException('Idea-scoped Agents require ideaId (and only ideaId).');
				}
				break;
			case AgentScope.WORK:
				if (!input.workId || popCount !== 1) {
					throw new BadRequestException('Work-scoped Agents require workId (and only workId).');
				}
				break;
			default:
	...
	async assertCanAssignAcrossScope(actor: Agent, target: Agent): Promise<void> {
		...
		switch (actor.scope) {
			...
			case AgentScope.MISSION:
				if (target.scope === AgentScope.MISSION && target.missionId === actor.missionId) return;
				if (target.scope === AgentScope.IDEA && target.missionId === actor.missionId) return;
				if (target.scope === AgentScope.WORK && target.missionId === actor.missionId) return;
				throw new ForbiddenException('Mission-scoped Agents can only assign within their Mission.');
			case AgentScope.IDEA:
				...
				throw new ForbiddenException('Idea-scoped Agents can only assign within their Idea.');
			case AgentScope.WORK:
				if (target.scope === AgentScope.WORK && target.workId === actor.workId) return;
				throw new ForbiddenException('Work-scoped Agents can only assign within their Work.');
		}
	}
}
