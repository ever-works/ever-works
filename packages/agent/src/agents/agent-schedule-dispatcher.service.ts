...
export interface AgentHeartbeatTrigger {
	enqueue(payload: { agentId: string; userId: string; scheduledFor: Date }): Promise<{ runId: string }>;
}
...
export class AgentScheduleDispatcherService {
	...
	async dispatchDue(
		...
		if (!config.agents.dispatcherEnabled()) {
			this.logger.warn('Agent dispatcher disabled (AGENTS_DISPATCHER_ENABLED=false), skipping');
			return {
				limit,
				dueCount: 0,
				dispatched: 0,
		...
		for (const agent of due) {
			...
			try {
				...
				if (!originalNext) {
					...
					summary.skipped += 1;
					summary.entries.push(this.entry(agent, { outcome: 'skipped', message: 'already claimed' }));
					continue;
				}
