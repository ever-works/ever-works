import { getCurrentPeriodStart, getNextPeriodStart, isWithinCurrentPeriod } from '../budget-period';
...
describe('budget-period (Phase 7.6 — N6 override)', () => {
    ...
    describe('day', () => {
        it('current period anchor is start of UTC day', () => {
            const now = new Date(Date.UTC(2026, 4, 26, 14, 37, 22));
            expect(getCurrentPeriodStart('day', now).toISOString()).toBe(
    ...
    describe('week', () => {
        it('current period anchor is Monday 00:00 UTC (ISO-8601)', () => {
            ...
            const now = new Date(Date.UTC(2026, 4, 26, 14, 0, 0));
            expect(getCurrentPeriodStart('week', now).toISOString()).toBe(
    ...
    describe('month', () => {
        it('current period anchor is the 1st of the UTC month', () => {
            const now = new Date(Date.UTC(2026, 4, 26, 14, 37, 22));
            expect(getCurrentPeriodStart('month', now).toISOString()).toBe(
    ...
    describe('isWithinCurrentPeriod', () => {
        it('true for a Date inside the current hour, false for the previous hour', () => {
            ...
            expect(
                isWithinCurrentPeriod('hour', new Date(Date.UTC(2026, 4, 26, 14, 5, 0)), now),
            ).toBe(true);
            expect(
                isWithinCurrentPeriod('hour', new Date(Date.UTC(2026, 4, 26, 13, 59, 59)), now),
            ).toBe(false);
        });
        it('true for a Date inside the current week', () => {
            const now = new Date(Date.UTC(2026, 4, 26, 14, 0, 0));
            expect(
                isWithinCurrentPeriod('week', new Date(Date.UTC(2026, 4, 25, 0, 0, 0)), now),
            ).toBe(true);
            expect(
                isWithinCurrentPeriod('week', new Date(Date.UTC(2026, 4, 31, 23, 0, 0)), now),
            ).toBe(true);
            expect(
                isWithinCurrentPeriod('week', new Date(Date.UTC(2026, 4, 24, 23, 59, 59)), now),
            ).toBe(false);
        });
