	...
	describe('heartbeat kind', () => {
		it('emits all 11 segments in spec order', () => {
			const out = svc.assemble({
				agent: makeAgent(),
				kind: 'heartbeat',
				skills: [{ slug: 'cron-rules', body: 'When stuck on cron, default UTC.', priority: 100 }],
				scopeContext: 'Tenant CEO of Acme Inc.',
				recentActivity: [{ at: '2026-05-26T10:00:00Z', type: 'agent_started', detail: 'CEO heartbeat' }],
				recentRuns: [{ at: '2026-05-26T09:45:00Z', status: 'completed', summary: 'No actions.' }],
