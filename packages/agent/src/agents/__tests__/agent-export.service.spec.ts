...
describe('AgentExportService', () => {
	...
	describe('importOne', () => {
		const baseEnvelope = (): AgentExportEnvelope => ({
			version: 1,
			meta: { exportedAt: '2026-05-26T00:00:00Z', sourceAgentId: 'src-a', sourceUserId: 'u1' },
			identity: { name: 'CEO', slug: 'ceo', title: 'Chief', capabilities: null, scope: AgentScope.TENANT },
			...
			files: { soulMd: '# Hi', agentsMd: null, heartbeatMd: null, toolsMd: null, agentYml: null },
			skillBindings: [],
			budget: [],
		});
		...
		it('rejects an envelope with unsupported version', async () => {
			...
			await expect(svc.importOne('u1', env as any)).rejects.toThrow(/Unsupported envelope version/);
		});

		it('rejects scope+ids mismatch (TENANT must not carry workId)', async () => {
			await expect(
				svc.importOne('u1', baseEnvelope(), { workId: 'w1' as any, overrideScope: AgentScope.TENANT }),
			).rejects.toThrow(BadRequestException);
		});

		it('happy path — creates a draft Agent with unique slug', async () => {
			agents.findByUserIdAndSlug.mockResolvedValue(null);
			agents.create.mockResolvedValueOnce(makeAgent({ id: 'new-a', status: AgentStatus.DRAFT }));
