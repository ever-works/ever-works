...
export class ProviderOverrideService {
    ...
    /**
     * Get provider for a specific parent capability.
     *
     * @param parentCapability - The parent capability (e.g., 'ai-provider', 'search')
     * @param context - Context for provider resolution
     * @returns The selected sub-provider or null
     */
    async getProviderByCapability(
        parentCapability: string,
        context: ProviderOverrideContext,
    ): Promise<ProviderOverrideResult> {
        const subProviders = this.getEnabledSubProviders().filter(
        ...
        if (subProviders.length === 0) {
            return {
                provider: null,
                reason: `No sub-providers for capability "${parentCapability}"`,
            };
        }
        ...
        for (const plugin of subProviders) {
            try {
                ...
                this.logger.warn(
                    `Sub-provider "${plugin.id}" failed canHandle check: ${(error as Error).message}`,
                );
            }
        }
        ...
        if (candidates.length === 0) {
            return {
                provider: null,
                reason: `No sub-providers for capability "${parentCapability}" can handle this context`,
            };
        }
        ...
        return {
            provider: selected.plugin,
            reason: `Selected "${selected.plugin.id}" for capability "${parentCapability}" (priority: ${selected.priority})`,
            priority: selected.priority,
        };
    }

    /**
     * Get the default provider for a parent capability.
     *
     * @param parentCapability - The parent capability
     * @returns The default sub-provider or null
     */
    getDefaultProvider(parentCapability: string): ISubProviderPlugin | null {
        const subProviders = this.getEnabledSubProviders().filter(
            (p) => p.parentCapability === parentCapability,
        );

        // Find the one marked as default
        const defaultProvider = subProviders.find((p) => {
            const registration = p.getRegistration();
            return registration.isDefault;
        });
        ...
        // Otherwise return the one with lowest priority
        if (subProviders.length > 0) {
            const sorted = [...subProviders].sort((a, b) => {
                const priorityA = a.getRegistration().priority ?? 100;
                const priorityB = b.getRegistration().priority ?? 100;
    ...
    /**
     * Get all available sub-providers grouped by parent capability.
     */
    getAvailableProviders(): Map<string, ISubProviderPlugin[]> {
        const result = new Map<string, ISubProviderPlugin[]>();
        const subProviders = this.getEnabledSubProviders();

        for (const provider of subProviders) {
            const capability = provider.parentCapability;
            if (!result.has(capability)) {
                result.set(capability, []);
            }
            result.get(capability)!.push(provider);
        }
     ...
     *
     * @param subProviderId - Sub-provider ID
     */
    async isProviderAvailable(subProviderId: string): Promise<boolean> {
        const subProviders = this.getEnabledSubProviders();
        const provider = subProviders.find((p) => p.subProviderId === subProviderId);
