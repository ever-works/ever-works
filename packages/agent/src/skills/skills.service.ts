...
export class SkillsService {
	...
	async list(userId: string, filter: ListSkillsFilter = {}): Promise<{ rows: Skill[]; total: number }> {
	...
	async create(userId: string, input: CreateSkillInput): Promise<Skill> {
		...
		if (!slug) {
			throw new BadRequestException('Skill title must contain at least one alphanumeric character.');
		}
	...
	async createBinding(userId: string, input: CreateBindingInput): Promise<SkillBinding> {
		...
		if (input.targetType !== 'tenant' && !input.targetId) {
			throw new BadRequestException(`targetId is required when targetType=${input.targetType}.`);
		}
