...
export class AgentRepository {
	...
	async findByUserIdScoped(userId: string, filter: ListAgentsFilter = {}): Promise<{ rows: Agent[]; total: number }> {
		...
		qb.orderBy('agent.updatedAt', 'DESC').take(filter.limit ?? 50).skip(filter.offset ?? 0);
	 ...
	 */
	async incrementErrorCount(agentId: string, nextHeartbeatAt: Date | null): Promise<{ paused: boolean }> {
