...
/**
 * EW-584 — Phase A. Runs a tool-calling agent that researches a newly
 * signed-up user via web search + content extraction, and persists the
 * inferred profile + derived verticals on the User row.
 *
 * The agent is bounded by step count, wall-clock timeout, and per-user
 * daily caps (UserResearchLimitsService). When the LLM does not call
 * `finalize`, the run returns status='no-data' (we do not fall back to
 * scraping the transcript — that path is brittle and worth deferring).
 */
@Injectable()
export class UserResearchService {
	...
	async research(userId: string, opts: UserResearchOptions = {}): Promise<UserResearchResult> {
		...
		if (!user) {
			return { status: 'error', tokensUsed: 0, toolCallsCount: 0, durationMs: 0, error: 'user-not-found' };
		}
		...
		try {
			...
			this.logger.debug(`Could not load social accounts for user ${userId}: ${(err as Error).message}`);
		}
		...
		try {
			...
			this.logger.warn(`Could not resolve AI provider for user research: ${(err as Error).message}`);
			return {
				status: 'error',
				tokensUsed: 0,
				toolCallsCount: 0,
		...
		if (!finalProfile) {
			this.logger.log(`User research for ${userId} ended without finalize; no profile persisted`);
			return {
				status: 'no-data',
				tokensUsed,
				toolCallsCount,
		...
		// AI SDK's tool({ inputSchema }) execute() hands us a partial-typed value,
		// so we re-parse and cast at the boundary. Runtime values are valid.
