...
export class WorkProposalService {
	...
	async generate(
		...
		try {
			const providerConfig = await this.aiFacade.getProviderConfig({ userId, workId: userId });
			if (!providerConfig.baseUrl || !providerConfig.apiKey) {
				return { status: 'error', proposals: [], tokensUsed: 0, error: 'ai-provider-not-configured' };
			}
			...
			modelName =
				...
				'';
			if (!modelName) {
				return { status: 'error', proposals: [], tokensUsed: 0, error: 'no-model-configured' };
			}
			...
			// Re-parse so TS narrows the deeply-nested types to fully-required shape
		...
		// Prune hallucinated plugin IDs — keep only those the registry knows.
		// AI SDK's structured-output inferred types come back with optional fields
		// at the leaves; the runtime values are valid (we just re-parsed), so we
		// cast at the boundary instead of mapping each field.
