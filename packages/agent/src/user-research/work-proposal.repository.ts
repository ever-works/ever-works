...
export class WorkProposalRepository {
	...
	async bulkInsert(items: CreateWorkProposalInput[]): Promise<WorkProposal[]> {
		...
		const entities = items.map((i) => this.repository.create({ ...i, status: 'pending' as WorkProposalStatus }));
	...
	async markDismissed(id: string, userId: string): Promise<boolean> {
		const res = await this.repository.update({ id, userId, status: 'pending' }, { status: 'dismissed' });
