...
describe('WorksConfigRepositorySyncService', () => {
    ...
    it('writes projected .works/works.yml and skips commit when the repo is unchanged', async () => {
    ...
    it('commits and pushes when projected .works/works.yml changes the data repo', async () => {
        const { service, gitFacade } = createService([
            { path: '.works/works.yml', status: 'modified' },
        ]);
        ...
        expect(gitFacade.commit).toHaveBeenCalledWith(
            'github',
            '/tmp/data-repo',
            'sync .works/works.yml after provider_changed',
