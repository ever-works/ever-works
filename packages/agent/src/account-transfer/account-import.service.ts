...
export class AccountImportService {
	...
	async previewImport(userId: string, payload: AccountExportPayload): Promise<ImportPreview> {
		...
		if (payload.version !== 1) {
			return {
				valid: false,
				errors: [`Unsupported export version: ${payload.version}. Only version 1 is supported.`],
	...
	private async importDirectory(
		...
		if (existing) {
			...
			if (resolution.strategy === 'rename') {
				...
				const newExisting = await this.directoryRepository.existsByUserAndSlug(userId, slug);
				if (newExisting) {
					result.errors.push(
