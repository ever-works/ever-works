...
export class GitHubSyncService {
	...
	async configureSyncRepo(
		...
		if (dto.createNew) {
			...
			if (!exists) {
				...
				// Verify it's private
				const repo = await this.gitFacade.getRepository(
					repoOwner,
					repoName,
					gitOptions,
				);
	...
	private writeExportFiles(dir: string, payload: AccountExportPayload): void {
		...
		this.writeJsonFile(
			path.join(pluginsDir, 'user-plugins.json'),
			payload.data.userPlugins,
		);
	...
	private readExportFiles(dir: string): AccountExportPayload | null {
		...
		try {
			...
			const userPlugins = this.readJsonFile(
				path.join(dir, 'plugins', 'user-plugins.json'),
			) || [];
