...
import { generateComparison, type ComparisonAiDependencies } from '@ever-works/comparison-generator-plugin';
...
export class ComparisonGenerationService {
	...
	async generateNextComparison(directoryId: string, userId: string): Promise<ComparisonResult> {
		...
		return this.generateComparisonForPair(pair, directory, dataRepo, config, comparisonState, gitOptions);
	}
	...
	async generateManualComparison(
		...
		if (!pair) {
			return { status: 'error', message: `Could not find items: ${itemASlug} and/or ${itemBSlug}` };
		}
		...
		if (comparisonState.generated_pairs.includes(pairKey)) {
			...
			if (exists) {
				return { status: 'skipped', slug: pairKey, message: 'Comparison already exists for this pair' };
			}
		}
	 ...
	 */
	async deleteComparison(directoryId: string, userId: string, slug: string): Promise<ComparisonResult> {
		const directory = await this.directoryRepository.findOneOrFail(directoryId);
		...
		await this.gitFacade.commit(directory.gitProvider, dest, `chore: remove comparison - ${slug}`);
	...
	private async generateComparisonForPair(
		...
		config: IDataConfig,
		comparisonState: { generated_pairs: string[]; last_generated_at?: string; total_generated: number },
		...
		const researchDeps: ResearchDependencies = {
			...
			extractContent: async (url: string) => {
				const result = await this.contentExtractorFacade.extractContent(url, undefined, facadeOptions);
		...
		const aiDeps: ComparisonAiDependencies = {
			askJson: async <T>(prompt: string, _schema: Record<string, unknown>) => {
				const response = await this.aiFacade.askJson(prompt, comparisonStructureSchema, undefined, facadeOptions);
