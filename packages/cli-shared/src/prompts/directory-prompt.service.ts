...
export class DirectoryPromptService extends BasePromptService {
    ...
    private async promptReadmeConfig(): Promise<MarkdownReadmeConfigDto> {
        ...
        const wantsCustomHeader = await this.promptConfirm(
            'Do you want to add a custom header?',
            false,
        );
        ...
        const wantsCustomFooter = await this.promptConfirm(
            'Do you want to add a custom footer?',
            false,
        );
