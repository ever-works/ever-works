...
- **Last tick #**: 27
- **Last tick at**: 2026-05-26 (tick 27 — Phase 13.5 wired: `TasksService.{listAttachments, addAttachment, removeAttachment}` reusing the existing `TaskAttachmentRepository` (Phase 11.5) + 3 controller endpoints — `GET /api/tasks/:id/attachments`, `POST /api/tasks/:id/attachments` (60/min throttle, body `{uploadId}`), `DELETE /api/tasks/:id/attachments/:attachmentId`. Repo injection is `@Optional()` so existing service mocks remain valid. API takes a KB `uploadId` produced by the existing upload pipeline — no new upload code; the UI file-picker primitive (extracted from the KB upload surface) remains the only deferred piece.)
