...
- **Last tick #**: 32
- **Last tick at**: 2026-05-26 (tick 32 — Phase 19.5 wired: `AccountExportPayload.version` widens from `1` to `1|2`; `data` gains optional `agents` / `skills` / `tasks` v2 tail (per `AccountExportV2Tail`). `GitHubSyncService.writeExportFiles` writes one json file per row under top-level `agents/` / `skills/` / `tasks/` subdirs — one file per row keeps git diffs clean (an Agent SOUL.md edit is a single-file diff instead of a json-blob rewrite). Manifest carries the per-section counts. Path-traversal guard mirrors the existing works/ slug check (`path.basename(slug) === slug` or skip+warn). `readExportFiles` walks the new subdirs via a new `readJsonDir` helper, surfaces them on the payload, and infers `version=2` when manifest is stale but tail subdirs exist. ~5 new specs cover: per-row file writes, manifest counts, no-subdir when arrays empty (v1-compat), traversal-slug skip, full v2 round-trip read, and v2-from-stale-manifest inference.)
