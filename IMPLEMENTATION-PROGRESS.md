...
- **Last tick #**: 29
- **Last tick at**: 2026-05-26 (tick 29 — Phase 15.6 wired: `FacadeOptions` gains optional `agentId` + `taskId` attribution columns; `PluginUsageService.RecordPluginUsageInput` accepts the same; the service write path persists them onto every event row. Agent-initiated calls without a `workId` (heartbeat runs without Work scope) now still record when `agentId` is set — previously they were dropped. All 4 `AiFacadeService.record()` callsites + the 3 plugin-facade callsites (search / screenshot / content-extractor) propagate `facadeOptions.agentId` + `facadeOptions.taskId` through. This unblocks `getTotalSpendCentsForTask` (Phase 15.7) and the per-Agent budget rollup — rows actually carry the attribution now.)
