...
- **Last tick #**: 33
- **Last tick at**: 2026-05-26 (tick 33 — Phase 19.6 wired end-to-end: `ExportOptions` extended with `includeAgents` / `includeSkills` / `includeTasks` / `includeTaskChat`. `AccountExportService` constructor takes `@Optional()` `AgentsSkillsTasksExportService` (10th arg) and calls `exportTail()` when any toggle is set; result merges into `data.{agents,skills,tasks}` and bumps version to 2 when any array has rows. Best-effort: a tail failure logs WARN and returns v1 payload (does not throw). API controller `@Get('/account/export')` accepts 4 new query params (defaults false). Web server action `exportAccountData()` now accepts either a boolean (back-compat) or a full `ExportToggles` object; web builds the query string from the object. UI `DataManagement.tsx` adds a "Additional sections (v2 payload)" fieldset with 4 checkboxes — Task chat is disabled + visually dimmed unless Tasks is checked. 5 new specs cover toggle-off-default, toggle-on roundtrip, empty-tail stays v1, tail-crash recovery, no-call-when-no-toggle. Module wiring already in place via Phase 19.8.)
