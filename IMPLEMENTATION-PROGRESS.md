...
- **Last tick #**: 36
- **Last tick at**: 2026-05-26 (tick 36 — Phase 17.8 UI shipped (closing the last interactive post-Phase-20 follow-up): new `apps/web/src/components/tasks/TaskRecurringSection.tsx` renders an inline picker on the Task detail page with two modes. Inactive: "Make recurring" CTA opens a frequency picker (Daily / Weekly / Monthly / Custom RRULE) + optional end date + max occurrences; emits `FREQ=…` RRULE that `TasksService.setRecurring` validates server-side. Active: shows rule + timezone + nextOccurrenceAt + occurrence counter (n/max) + "Stop recurring" destructive action. `tasksAPI.setRecurring` + `.clearRecurring` added to the client API. `setTaskRecurringAction` + `clearTaskRecurringAction` server actions added. Mounted into `TaskDetailClient.tsx` between transitions and conversation so it's discoverable without dominating the page. No new dependency added — the "shared Tiptap chip primitive" deferral was overcautious; native HTML select + date input ship a complete UX today.)
