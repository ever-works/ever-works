...
- **Last tick #**: 31
- **Last tick at**: 2026-05-26 (tick 31 — Phase 16.10 wired: new `packages/agent/src/agents/agent-plugin-tools-facade.ts` defines `AgentPluginToolsFacade.{searchWeb, screenshot, extractContent}` with semantic payloads (each carries `userId / agentId / workId / taskId` for Phase 15.6 attribution). `AgentToolService` injects the `AGENT_PLUGIN_TOOLS_FACADE` token via `@Optional()` + registers the 3 descriptors gated on `permissions.canCallExternalTools AND token bound`. API-side `AgentsModule` imports `FacadesModule` and binds the token via `useFactory` to a thin adapter forwarding into `SearchFacadeService.search()`, `ScreenshotFacadeService.capture()`, and `ContentExtractorFacadeService.extractContent()` — `agentId` + `taskId` thread onto every `FacadeOptions` so resulting `PluginUsageEvent` rows carry attribution. extractContent clamps `maxChars` to 50 KB default / 200 KB hard cap. ~9 specs in `__tests__/agent-tool-plugins.spec.ts` cover gating, happy invoke for all 3, empty-field rejection, and adapter-exception capture.)
