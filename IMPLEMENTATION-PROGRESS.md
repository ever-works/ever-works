...
- **Last tick #**: 34
- **Last tick at**: 2026-05-26 (tick 34 — Phase 18.6 scaffold landed: new `apps/web/src/lib/api/agent-templates.ts` defines the `AstTemplateEntry` shape + `listAstTemplates(entity)` / `getAstTemplate(entity, slug)` helpers backed by hand-curated fallback constants for Agents / Skills / Tasks. New shared `apps/web/src/components/templates/AstTemplatesBrowser.tsx` renders a left-side category filter + 3-column card grid; each card links to the matching `/new?from=<slug>` route. Three new routes — `/agents/templates`, `/skills/templates`, `/tasks/templates` — with consistent header + back-link shells. Route constants added: `DASHBOARD_{AGENT,SKILL,TASK}_TEMPLATES`. Distinct from the existing `templates.ts` (Workshop Templates catalog for kind=website/work/mission) — uses its own slug to avoid collisions. When ADR-010 lands, `listAstTemplates` body swaps to a `serverFetch('/api/agent-templates?entity=…')` call with no UI changes. 6 unit specs cover fallback shape, unique slugs, getAstTemplate hit/miss, entity scoping.)
