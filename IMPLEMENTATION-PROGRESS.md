...
- **Last tick #**: 37
- **Last tick at**: 2026-05-26 (tick 37 — Phase 18.1 + 18.2 grid mount: Dashboard home `(home)/page.tsx` now server-fetches 5 cheap counts in parallel with the existing fetches — `agentsTotal`, `agentsActive`, `tasksInProgress`, `tasksBlocked`, plus 5 most-recent in-flight Tasks. Each fetch is `.catch()` defended so a flaky endpoint surfaces zeros instead of breaking the page. `DashboardClient` accepts the 5 new optional props (all default 0/[] for backwards compat) and mounts `AgentsCountTile` + `TasksInProgressTile` in a 2-column grid between `StatsOverview` and the Missions preview, with the `RecentTasks` block sliding in below the Works list. Reading order on the home page is now: stats tiles → AST tiles → Missions preview → Work proposals → Recent Works → Recent Tasks. The tiles + RecentTasks components were shipped tick 23; this tick wires them into the actual home grid.)
