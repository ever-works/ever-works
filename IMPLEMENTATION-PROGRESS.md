...
- **Last tick #**: 30
- **Last tick at**: 2026-05-26 (tick 30 — Phase 16.6 + 16.7 wired: new `packages/agent/src/agents/agent-git-facade.ts` contract defines `AgentGitFacade.commitToRepo` + `.openPullRequest` with semantic payloads (`{userId, agentId, workId, message, files?, branch?}` / `{title, body, head, base?, draft?}`). `AgentToolService` injects the token via `@Optional()` + adds the two tool descriptors gated on `(permission.canCommitToRepo / canOpenPullRequests) AND token bound`. Non-Work-scoped Agents refuse with an actionable error at invoke time. The token is deliberately left unbound in the API-side `AgentsModule` v1 — operators wire it post-merge through `GitFacadeService.commit()` / `.createPullRequest()` once their git provider setup is stable, keeping the model from seeing tools that would fail mysteriously. ~12 specs in `__tests__/agent-tool-git.spec.ts` cover gating, Work-scope check, happy invoke, missing fields, and adapter-exception capture.)
