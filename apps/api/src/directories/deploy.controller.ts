...
export class DeployController {
    ...
    async toVercel(
        ...
        const { VERCEL_TOKEN, vercelTeamScope } = deployVercel;
        ...
        const tokenUserId = isCreator ? user.id : directoryOwner.id;

        // Get git token from connected provider
        const ghToken = await this.gitFacade.getAccessToken({
            userId: tokenUserId,
