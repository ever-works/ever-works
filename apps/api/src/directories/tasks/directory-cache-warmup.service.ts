...
export class DirectoryCacheWarmupService {
    ...
    async warmDirectoryCaches() {
        try {
            const totalEligible = await this.directoryRepository.countForDetailCacheWarmup();
            ...
            if (directories.length === 0) {
                await this.cacheManager.set(CACHE_WARMUP_CURSOR_KEY, 0, CACHE_WARMUP_CURSOR_TTL_MS);
                return;
            }
            ...
            for (const directory of directories) {
                ...
                try {
                    ...
                    const [items, config, count, categoriesTags] = await Promise.all([
                        ...
                        this.directoryQueryService.directoryCategoriesTags(directory.id, owner),
                    ]);
