...
export class CreateAgentsTables1779978010000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await this.ensureIndex(
            queryRunner,
            'agents',
            'uq_agents_user_scope_slug',
            ['userId', 'scope', 'missionId', 'ideaId', 'workId', 'slug'],
            true,
        );
        await this.ensureIndex(queryRunner, 'agents', 'idx_agents_user_status', [
            'userId',
            'status',
        ]);
