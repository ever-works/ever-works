...
export class SocialAuthService {
	...
	private async getGitHubUser(accessToken: string) {
		...
		if (!email) {
			const emailsResponse = await firstValueFrom(
				this.httpService.get<GitHubEmail[]>('https://api.github.com/user/emails', { headers })
			);
	...
	private async getGoogleUser(accessToken: string) {
		const { data } = await firstValueFrom(
			this.httpService.get<GoogleUserInfo>('https://openidconnect.googleapis.com/v1/userinfo', {
				headers: {
					Authorization: `Bearer ${accessToken}`
				}
			})
