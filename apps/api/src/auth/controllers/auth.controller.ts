...
export class AuthController {
    ...
    @Get('profile')
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Get current user profile', description: 'Returns the authenticated user profile from JWT' })
    ...
    @Get('profile/fresh')
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Get fresh user profile', description: 'Returns fresh user data from the database' })
    ...
    @Public()
    @Get('github/url')
    @ApiOperation({ summary: 'Get GitHub OAuth URL', description: 'Generate a GitHub OAuth authorization URL' })
    @ApiQuery({ name: 'callbackUrl', required: false, description: 'URL to redirect after authentication' })
    ...
    @Public()
    @Get('google/url')
    @ApiOperation({ summary: 'Get Google OAuth URL', description: 'Generate a Google OAuth authorization URL' })
    @ApiQuery({ name: 'callbackUrl', required: false, description: 'URL to redirect after authentication' })
    @ApiQuery({ name: 'state', required: false, description: 'Optional state parameter' })
    ...
    @HttpCode(HttpStatus.OK)
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Send verification email', description: 'Send an email verification link to the user' })
    ...
    @Post('verify-email')
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'Verify email', description: 'Verify email address using the token from the verification email' })
    ...
    @Post('reset-password')
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'Reset password', description: 'Reset password using the token from the reset email' })
    ...
    @ApiOperation({ summary: 'Validate email verification token', description: 'Check if an email verification token is valid' })
    ...
    @ApiOperation({ summary: 'Validate password reset token', description: 'Check if a password reset token is valid' })
