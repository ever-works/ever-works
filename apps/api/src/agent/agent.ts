...
export class Agent {
     ...
     */
    private async generateItems(task: string, existingItems: Partial<ItemData>[] = [], options: GenerateOptions = {}) {
        ...
        const subarrays = await Promise.all(context.map(c => generateItemsSubarray(task, c.url, c.rawContent)));
