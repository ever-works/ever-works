 ...
 * Ported from web: apps/web/src/app/actions/dashboard/directories.ts
 */
    ...
    .action(async () => {
        try {
            ...
            const selectedGitProviderId =
                await directoryPrompt.promptGitProviderSelection(gitProviderChoices);
            ...
            if (deployProvidersResponse.providers.length > 0) {
                ...
                selectedDeployProviderId =
                    await directoryPrompt.promptDeployProviderSelection(deployProviderChoices);
            }
            ...
            // 7. Collect directory information (name, slug, description, owner/organization)
            ...
            // 9. Create directory with slug conflict handling
            let finalSlug = directoryData.slug;
            let conflictResolved = false;
            let increment = 1;

            while (!conflictResolved) {
                const spinner = ora(`Creating directory "${finalSlug}"...`).start();
