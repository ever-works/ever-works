...
import { WorkPromptService, GenerateStatusType, canEdit } from './work-prompt.service';
    ...
    .action(async () => {
        try {
            ...
            if (work.generateStatus?.status === GenerateStatusType.GENERATING) {
                ...
                console.log(
                    chalk.gray('To check the status, use ') +
                        chalk.cyan('ever-works work status') +
                        chalk.gray(' command.'),
                );
                return;
            }
            ...
            const [config, schema_] = await Promise.all([
                apiService.getWorkConfig(work.id),
                apiService.getGeneratorFormSchema(work.id),
            ]);
            let schema = schema_;
            ...
            // Generation options:
            // - New works: always CREATE_UPDATE (no gen method prompt, matching web)
            // - Existing works: prompt for gen method + PR option
            //   (for simple updates without re-configuring providers, use "work update" instead)
            let genOptions: {
