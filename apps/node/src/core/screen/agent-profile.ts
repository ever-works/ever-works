...
export class AgentProfileManager {
	...
	private async restrict(dir: string): Promise<void> {
		try {
			await this.options.restrictToOwner?.(dir);
		} catch {
			// Best-effort hardening: a machine whose ACL tool is missing still
			// keeps profiles in separate directories under the owner's home.
		}
	}
}
