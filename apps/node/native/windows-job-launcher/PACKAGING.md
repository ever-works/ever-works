...
- `build-release.ps1` uses `cargo build --locked --release --target x86_64-pc-windows-msvc`, disables incremental compilation, sets `SOURCE_DATE_EPOCH` from the source commit, and passes the MSVC `/Brepro` linker flag.
- Generated JSON uses stable ordering, UTF-8 without BOM, and no timestamp or host path.
- The generated unsigned metadata records `productionEligible: false`, the reproducible **unsigned** binary SHA-256, the lockfile SHA-256, source commit, target, toolchain, size, SBOM digest, and provenance digest.
...
4. the signed artifact SHA-256 differs from the unsigned build hash; and
5. the SBOM and provenance still match the unsigned build metadata.
