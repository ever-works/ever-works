...
test.describe('Work member removal — revokes access end-to-end', () => {
    ...
    test('ex-member is fully locked out: list + single-member reads both 403 after removal', async ({
        ...
        const getBefore = await request.get(`${API_BASE}/api/works/${work.id}/members/${row.id}`, {
            headers: authedHeaders(member.access_token),
        });
        ...
        const getAfter = await request.get(`${API_BASE}/api/works/${work.id}/members/${row.id}`, {
            headers: authedHeaders(member.access_token),
        });
    ...
    test('double-remove is idempotent-by-404: second DELETE of the same row → 404 "Member not found"', async ({
        ...
        const first = await request.delete(`${API_BASE}/api/works/${work.id}/members/${row.id}`, {
            headers: authedHeaders(owner.access_token),
        });
        ...
        const second = await request.delete(`${API_BASE}/api/works/${work.id}/members/${row.id}`, {
            headers: authedHeaders(owner.access_token),
        });
    ...
    test('removal is privileged: viewer/editor members get 403, manager+ may evict others', async ({
        ...
        expect(midIds).toEqual(expect.arrayContaining([editorRow.id, viewerRow.id, managerRow.id]));
    ...
    test('self-departure via leave revokes access, and a wrong-work / no-auth remove is rejected', async ({
        ...
        const leaveAgain = await request.post(`${API_BASE}/api/works/${work.id}/members/leave`, {
            headers: authedHeaders(member.access_token),
        });
        ...
        const noAuth = await request.delete(`${API_BASE}/api/works/${work.id}/members/${row.id}`);
