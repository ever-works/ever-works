...
test.describe('auth-method precedence & conflict resolution', () => {
	...
	test('header CASE of the bearer scheme routes the SAME ew_live_ value to DIFFERENT paths (capital -> API-key reject, lower -> provider reject)', async ({
		...
		if (capital.body && typeof capital.body === 'object') {
			...
			expect(capital.body.error, 'capital -> api-key path carries error').toBe('Unauthorized');
		}
		...
		if (lower.body && typeof lower.body === 'object') {
			...
			expect(lower.body.error, 'lowercase -> generic reject OMITS error label').toBeUndefined();
		}
		...
		expect(lowerValid.status, 'lowercase bearer + VALID session -> 200 (scheme is case-insensitive on provider path)').toBe(
			200,
		);
	...
	test('cross-user conflict resolves DETERMINISTICALLY to exactly one presented identity, never a blended third one', async ({
		...
		if (bKey) {
			...
			expect(principalId(single.body), 'single bearer resolves to its own owner only').toBe(aId);
		}
	});
	...
	test('anonymous -> authenticated UPGRADE: same /profile flips 401 -> anonymous principal -> registered principal, each with distinct provider/isAnonymous', async ({
		...
		if ([200, 201].includes(anonRes.status())) {
			...
			if (anonToken) {
				...
				if (anonProfile.body?.isAnonymous !== undefined) {
					expect(anonProfile.body.isAnonymous, 'anonymous principal isAnonymous').toBe(true);
				}
				expect(principalId(anonProfile.body), 'anonymous principal has a stable id').toBeTruthy();
			}
		} else {
			expect(
				[201, 200, 400, 429],
		...
		const regProfile = await getProfile(request, { Authorization: `Bearer ${reg.access_token}` });
	...
	test('stateless API key SURVIVES session logout while the session bearer is revoked — two credentials, independent lifecycles', async ({
		...
		const bearerBefore = await getProfile(request, { Authorization: `Bearer ${u.access_token}` });
	...
	test('401 envelopes are NON-uniform but share statusCode 401; the seeded UI session and a fresh API principal never cross identities', async ({
		...
		if (badApiKey.body?.error !== undefined && noCreds.body && typeof noCreds.body === 'object') {
		...
		expect(principalEmail(freshProfile.body)?.toLowerCase(), 'fresh distinct from seeded').not.toBe(
			s.email.toLowerCase(),
		);
