...
async function fishChangeToken(
    ...
    while (Date.now() < deadline) {
        ...
        for (const m of messages) {
            const toMatch = m.To?.some((t) => `${t.Mailbox}@${t.Domain}`.toLowerCase() === toLower);
            if (!toMatch) continue;
...
test.describe('flow-email-change-flow — request/confirm/cancel email-change lifecycle', () => {
     ...
     */
    test('duplicate target email is rejected (uniqueness enforced)', async ({
        request,
    }, testInfo) => {
    ...
    test('confirm endpoint rejects bogus / empty / missing tokens without moving identity', async ({
        ...
        expect(bogus.status, `bogus body=${JSON.stringify(bogus.body)}`).toBeGreaterThanOrEqual(
            400,
        );
     ...
     */
    test('login + live session remain usable during a pending change', async ({
        request,
    }, testInfo) => {
        ...
        if (!probe.path) {
            ...
            testInfo.annotations.push({
                type: 'degraded',
                description:
                    'No change-email endpoint — asserting steady-state session/login invariants',
            });
        } else {
            expect(probe.status, `request body=${JSON.stringify(probe.body)}`).toBeLessThan(500);
        }
