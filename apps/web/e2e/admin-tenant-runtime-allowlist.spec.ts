...
const TENANT_ID_B = process.env.TEST_TENANT_ID_B || seed?.tenantIdNoSecret || '';
...
test.describe('Operator tenant runtime allow-list — admin UI (#1516)', () => {
    ...
    test('unauthenticated request → 404 (defense-in-depth, not 403)', async ({ browser }) => {
        requireSeededTenant();
    ...
    test('provider checkbox grid renders all 5 known providers', async ({ page }) => {
        requireSeededTenant();
    ...
    test('status banner reflects gating OFF → "read-only" copy is shown', async ({ page }) => {
        requireSeededTenant();
        test.skip(
            GATING_ENV !== 'false',
    ...
    test('uncheck a provider → save → reload → state persists', async ({ page }) => {
        requireSeededTenant();
        await gotoAllowlist(page);
        const target = page.locator('input#runtime-allowlist-temporal');
        await expect(target).toBeVisible({ timeout: 10_000 });
    ...
    test('click delete chip removes the entry from the saved list', async ({ page }) => {
        requireSeededTenant();
        await gotoAllowlist(page);
    ...
    test('empty allow-list renders the "inherits global default" copy', async ({ page }) => {
        requireSeededTenant();
        test.skip(
            GATING_ENV !== 'true',
        ...
        await expect(
            page.getByText(/Tenant inherits the global allow-list \(no per-tenant restriction\)/i),
    ...
    test('i18n: en locale renders the english title string verbatim', async ({ page }) => {
        requireSeededTenant();
        await gotoAllowlist(page);
    ...
    test('cross-tenant isolation: navigate A → B → A produces tenant-specific state', async ({
        ...
        const aTriggerChecked = await page.locator('input#runtime-allowlist-trigger').isChecked();
    ...
    test('a11y: tab focus reaches each provider checkbox in declared order', async ({ page }) => {
        requireSeededTenant();
        await gotoAllowlist(page);
