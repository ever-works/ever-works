...
test.describe('Task hierarchy (deep) — trees, completion rules, delete/cancel, reparent, depth, isolation', () => {
	...
	test('multi-level tree: ?parentTaskId is single-level; full tree recovered by walking', async ({
		...
		expect(roots.map((t) => t.id), 'the only root is the grandparent').toEqual([gp.id]);
	});
	...
	test('parent completion is gated by approvers/blockers — NOT by open subtasks', async ({
		...
		expect(wipAfter.status, 'WIP child stays in_progress after parent done').toBe('in_progress');
		...
		if (approverRes.status() === 201) {
			...
			expect(
				gated.status(),
				'→done with a pending approver is rejected (409)',
			).toBe(409);
	...
	test('delete-parent orphans the subtree (no cascade, no null-out) — then reparent to recover', async ({
		...
		const c1 = await createTask(request, token, { title: `C1 ${sfx}`, parentTaskId: parent.id });
		const c2 = await createTask(request, token, { title: `C2 ${sfx}`, parentTaskId: parent.id });
		...
		expect(c1After.parentTaskId, 'orphan c1 still points at the vanished parent').toBe(parent.id);
		expect(c2After.parentTaskId, 'orphan c2 still points at the vanished parent').toBe(parent.id);
		...
		expect(gpRecovered.meta.total, 'GP regains the two recovered orphans + sibling = 3').toBe(3);
	...
	test('cancel-parent does NOT cascade to children; the real cascade is block-resolution', async ({
		...
		expect(String((await reanimate.json()).message), 'lattice error names the dead edge').toMatch(
			/cannot transition/i,
		);
		...
		await expect
			.poll(
				async () => (await getTask(request, token, dependent.id)).status,
				{
	...
	test('reparent: legal move + detach-to-root + cycle guard (409) + create-vs-PATCH asymmetry', async ({
		...
		expect(legal.status(), `legal reparent body=${await legal.text().catch(() => '')}`).toBe(200);
		...
		expect((await getTask(request, token, l5.id)).parentTaskId, 'L5 still under L4').toBe(l4.id);
		...
		expect(String((await createOrphan.json()).message), 'create names the missing parent').toMatch(
		...
		expect(((await patchOrphan.json()) as Task).parentTaskId, 'PATCH set the dangling edge').toBe(
			bogus,
		);
	});
	...
	test('depth-64 parent-chain cap + cross-user parent isolation', async ({ request }) => {
		...
		for (let i = 1; i <= MAX_TRY; i++) {
			const res = await request.post(`${API_BASE}/api/tasks`, {
				...
				data: { title: `Deep ${i} ${sfx}`, ...(parentId ? { parentTaskId: parentId } : {}) },
			});
			if (res.status() === 201) {
		...
		expect(created, 'a deep chain (>=60 levels) formed before the cap').toBeGreaterThanOrEqual(60);
		...
		expect(String((await bUnderA.json()).message), 'cross-user parent reads as not-found').toMatch(
			/parent task .* not found/i,
		);
