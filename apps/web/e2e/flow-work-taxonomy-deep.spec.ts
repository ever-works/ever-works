...
test.describe('Work taxonomy (deep) — per-work categories / tags / collections', () => {
	test('per-work taxonomy READS (categories-tags + count) are auth-gated and return the canonical owner envelopes', async ({
		...
		expect(ownerTax.status, `owner categories-tags (body=${JSON.stringify(ownerTax.body)})`).toBe(200);
		assertEmptyTaxonomy(ownerTax.body, 'owner fresh work');

		const ownerCount = await getCount(request, u.access_token, workId);
		expect(ownerCount.status, `owner count (body=${JSON.stringify(ownerCount.body)})`).toBe(200);
	...
	test('both taxonomy reads are OWNER-scoped (stranger 403) and ghost-safe (404) with their exact envelopes', async ({
		...
		assertEmptyTaxonomy((await getTaxonomy(request, a.access_token, workId)).body, 'owner read');
		...
		expect(strangerTax.status, `stranger categories-tags; body=${JSON.stringify(strangerTax.body)}`).toBe(403);
		...
		expect(strangerCount.status, `stranger count; body=${JSON.stringify(strangerCount.body)}`).toBe(403);
		...
		expect(ghostTax.status, `ghost categories-tags; body=${JSON.stringify(ghostTax.body)}`).toBe(404);
	...
	test('dedicated taxonomy WRITES (categories/tags/collections) are git-gated: a DTO-valid owner write 500s on a non-connected work and persists NOTHING', async ({
		...
		for (const kind of TAXONOMY_KINDS) {
			...
			expect(write.body.status, `${kind} write 500 is NOT a success envelope`).not.toBe('success');
		}
	...
	test('taxonomy-write gate ORDER is validation → ownership → git: each layer fires for the right caller/body', async ({
		...
		expect(gated.status, `owner DTO-valid write hits git-gate -> 500; body=${gated.text}`).toBe(500);
	...
	test('PUT/DELETE taxonomy writes are equally git-gated, and submit-item exposes the DISTINCT reconnect-git 400 contract', async ({
		...
		expect(put.status(), `PUT category git-gate -> 500; body=${await put.text().catch(() => '')}`).toBe(500);
		...
		expect(del.status(), `DELETE tag git-gate -> 500; body=${await del.text().catch(() => '')}`).toBe(500);
		...
		expect(typeof submitBody.message, 'submit-item git-gate message is a single string').toBe('string');
		expect(submitBody.message, 'exact reconnect-git remediation').toBe(GIT_GATE_MESSAGE);
		expect(submitBody.item_name, 'submit-item git-gate echoes the item name').toBe(validItem.name);

		// After all four blocked writes the taxonomy + count are STILL empty (nothing persisted).
		assertEmptyTaxonomy((await getTaxonomy(request, u.access_token, workId)).body, 'after blocked PUT/DELETE/submit');
		const count = await getCount(request, u.access_token, workId);
	...
	test('taxonomy + count are isolated PER-WORK and the count↔read invariant holds; seeded user browses read-only end-to-end', async ({
		...
		for (const [label, workId] of [
			...
			expect(cnt.body.tags, `${label}: count.tags mirrors the read`).toBe((tax.body.tags ?? []).length);
		}
		...
		expect(loginRes.ok(), `seed login body=${await loginRes.text().catch(() => '')}`).toBeTruthy();
