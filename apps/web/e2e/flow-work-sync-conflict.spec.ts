...
interface ActivityRow {
	...
	status?: string;
	summary?: string;
	details?: { kind?: string; reason?: string; source?: string; errorClass?: string; [k: string]: unknown };
	createdAt?: string;
	[key: string]: unknown;
}
...
test.describe('flow: work data-sync state-field + conflict contract (EW-628, deep)', () => {
	...
	test('a failed sync leaves lastSyncedDataRepoSha unchanged (the conflict baseline never advances on failure)', async ({
		...
		const owner = await registerUserViaAPI(request, { name: `Sync Baseline ${uniqueSuffix()}` });
		...
		if (isGitConnected(first.body)) {
			...
			await expect
				.poll(
					async () =>
						(await getWorkState(request, token, work.id))?.lastSyncedDataRepoSha ? 'stamped' : 'pending',
					{
						timeout: 20_000,
			...
			expect(post!.pendingSyncRequestedAt ?? null, 'success clears the pending flush flag').toBeNull();
			return;
		}
		...
		expect(ERROR_CLASSES, 'errorClass is from the closed union').toContain(first.body.errorClass);
		...
		expect(after!.lastSyncedDataRepoSha ?? null, 'failed sync did NOT advance the baseline SHA').toBeNull();
		expect(after!.pendingSyncRequestedAt ?? null, 'failed sync did NOT clear/set the pending flag').toBeNull();
	...
	test('five concurrent force-syncs dedup to at most one real attempt (single-flight lock + backoff)', async ({
		...
		const token = owner.access_token;
		const work = await createWorkViaAPI(request, token, { name: `sync-dedup-${uniqueSuffix()}` });
		...
		for (const r of results) {
			expect(r.http, 'each concurrent sync is 202 ACCEPTED (never a 5xx)').toBe(SYNC_HTTP_ACCEPTED);
			expect(ENVELOPE_STATUSES, 'each envelope status is well-formed').toContain(r.body.status);
		}
		...
		if (results.some((r) => isGitConnected(r.body))) {
			...
			test.info().annotations.push({
				type: 'git-connected',
				description: 'git connected (non-CI); asserting the no-5xx + valid-reason invariant only.',
			});
		} else {
			...
			expect(skipped.length, 'the rest of the burst is suppressed').toBeGreaterThanOrEqual(BURST - 1);
		}

		// Every skipped outcome carries a reason from the closed dedup vocabulary.
		for (const r of skipped) {
			expect(SKIP_REASONS, `skip reason "${r.body.reason}" is from the closed union`).toContain(
				r.body.reason,
			);
			expect(
		...
		if (!results.some((r) => isGitConnected(r.body))) {
			expect(state!.lastSyncedDataRepoSha ?? null, 'no SHA advance from a deduped failing burst').toBeNull();
		}
	});
	...
	test('syncIntervalMinutes cannot be changed through the public Work update DTO (cadence is tamper-proof)', async ({
		...
		const token = owner.access_token;
		const work = await createWorkViaAPI(request, token, { name: `sync-cadence-${uniqueSuffix()}` });

		const before = await getWorkState(request, token, work.id);
		expect(before!.syncIntervalMinutes, 'default cadence is 5 minutes').toBe(5);
		expect(before!.githubAppInstalled, 'fresh work has the App not installed (poller path)').toBe(false);
	...
	test('a sync issued alongside a generation attempt is not falsely gated as generation-in-progress', async ({
		...
		const token = owner.access_token;
		const work = await createWorkViaAPI(request, token, { name: `sync-vs-gen-${uniqueSuffix()}` });
		...
		const genRes = await request.post(`${API_BASE}/api/works/${work.id}/generate`, {
			headers: { ...authedHeaders(token), 'content-type': 'application/json' },
			data: { name: 'Sync Gate Probe', prompt: 'A directory exercising the sync-vs-generation mutex gate.' },
		});
		...
		const genStatus = (stateMid?.generateStatus as { status?: string } | null | undefined)?.status;
		...
		if (isGenerating) {
			...
			expect(stateMid!.lastSyncedDataRepoSha ?? null, 'no generation == no SHA from gen').toBeNull();
	...
	test('deleting a work purges its sync state and removes it from the dispatch due-set (clean 404, no 5xx)', async ({
		...
		const token = owner.access_token;
		const work = await createWorkViaAPI(request, token, { name: `sync-purge-${uniqueSuffix()}` });
		...
		expect([200, 202, 204], `delete responds cleanly (got ${del.status()})`).toContain(del.status());
		...
		expect(String(body.message), 'deleted-work 404 message names "not found"').toMatch(/not found/i);
	});
	...
	test("a stranger cannot read or perturb another user's sync state (403, state stays owner-scoped)", async ({
		...
		expect(String(bBody.message), 'stranger 403 message names permission').toMatch(/permission/i);
		...
		expect([401, 403, 404], `stranger read of A's work is access-gated (got ${bRead.status()})`).toContain(
			bRead.status(),
		);
		...
		for (const row of aRows) {
			expect(row.details?.source, "A's sync rows came from the manual transport").toBe('manual');
		}
		...
		expect(aStateAfter!.lastSyncedDataRepoSha ?? null, "A's baseline unchanged by stranger").toBe(
			aStateBefore!.lastSyncedDataRepoSha ?? null,
		);
		expect(aStateAfter!.pendingSyncRequestedAt ?? null, "A's pending flag unchanged by stranger").toBe(
			aStateBefore!.pendingSyncRequestedAt ?? null,
		);
