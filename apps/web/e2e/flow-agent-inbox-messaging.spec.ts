...
test.describe('Agent inbox + messaging', () => {
	...
	test('new agent inbox is empty; compose validates from-address then body; failed send adds no row', async ({
		...
		const agent = await createAgentViaAPI(request, token, { name: uniq('Inbox empty'), scope: 'tenant' });
		...
		const noAddr = await request.post(`${API_BASE}/api/email/messages`, {
			headers: authedHeaders(token),
			data: { agentId: agent.id, to: ['dest@example.com'], subject: uniq('s'), bodyText: 'hi' },
		});
		...
		const noBody = await request.post(`${API_BASE}/api/email/messages`, {
			headers: authedHeaders(token),
			data: { agentId: agent.id, fromAddressId: addr.id, to: ['dest@example.com'], subject: uniq('s') },
		});
	...
	test('inbox is partitioned per agent — agent A and agent B threads never bleed into each other', async ({
		...
		const agentA = await createAgentViaAPI(request, token, { name: uniq('Inbox A'), scope: 'tenant' });
		const agentB = await createAgentViaAPI(request, token, { name: uniq('Inbox B'), scope: 'tenant' });
	...
	test('cross-user isolation: B cannot read A’s inbox/messages nor borrow A’s reply-from address', async ({
		...
		const agentA = await createAgentViaAPI(request, tokenA, { name: uniq('A owns'), scope: 'tenant' });
	...
	test('reply-from address lifecycle: register, set default, verify click-through, retire', async ({
		...
		expect(((await patchDefault.json()) as { address: EmailAddress }).address.defaultForReplies).toBe(true);
		...
		const afterVerify = ((await (
			await request.get(`${API_BASE}/api/email/addresses?direction=outbound`, {
				headers: authedHeaders(token),
			})
		).json()) as { addresses: EmailAddress[] }).addresses.find((x) => x.id === addr.id);
		...
		expect(((await disable.json()) as { address: EmailAddress }).address.disabledAt).not.toBeNull();

		pool = ((await (
			await request.get(`${API_BASE}/api/email/addresses?direction=outbound`, {
				headers: authedHeaders(token),
			})
		).json()) as { addresses: EmailAddress[] }).addresses;
		expect(pool.some((x) => x.id === addr.id), 'a retired reply address leaves the active pool').toBe(false);
	});
	...
	test('inbox SSE stream is an event-stream; list honours limit/offset; routes require auth', async ({
		...
		const user = await registerUserViaAPI(request);
		const token = user.access_token;
		const agent = await createAgentViaAPI(request, token, { name: uniq('Inbox stream'), scope: 'tenant' });
	...
	test('UI: per-agent Inbox renders empty thread + Compose, and the composer round-trips a send attempt', async ({
		...
		const agent = await createAgentViaAPI(request, token, { name: uniq('Inbox UI'), scope: 'tenant' });
		...
		// ---- Composer page ----
		await page.goto(`${origin}/agents/${agent.id}/inbox/compose`, { waitUntil: 'domcontentloaded' });
		await expect(page.getByRole('heading', { name: 'Compose' })).toBeVisible({ timeout: 30_000 });
