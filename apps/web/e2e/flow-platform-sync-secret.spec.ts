...
test.describe('Platform secret — webhook signing-secret rotation lifecycle', () => {
	test('Flow 1 — create + rotate mint distinct 43-char base64url secrets; rotation is never idempotent and never 409s', async ({
		...
		const { subId, secret: createSecret } = await createSubscription(request, owner.access_token, 'rot');
		...
		for (let i = 0; i < 5; i++) {
			...
			expect(res.status(), `rotate#${i + 1} body=${await res.text().catch(() => '')}`).toBe(200);
			const body = await res.json();
			...
			expect(seen.has(body.signingSecret), 'every rotate yields a globally-unique secret').toBe(
				false,
			);
	...
	test('Flow 2 — the raw signing secret is returned ONCE and is never retrievable again (GET omits it; no enc::v1:: envelope leaks)', async ({
		...
		const owner = await registerUserViaAPI(request);
		const { subId, secret: firstSecret } = await createSubscription(request, owner.access_token, 'omit');
	...
	test('Flow 3 — rotate→old secret invalid: after rotation every prior raw secret is gone from all read surfaces and the new secret is the active signing key', async ({
		...
		const owner = await registerUserViaAPI(request);
		const { subId, secret: oldSecret } = await createSubscription(request, owner.access_token, 'old');
		...
		for (const s of issued) {
			expect(listText, `issued secret must never be retrievable: ${s.slice(0, 6)}…`).not.toContain(
				s,
			);
		}
	...
	test('Flow 5 — encrypted-at-rest black box: two subscriptions with independently rotated secrets never cross-contaminate and never expose stored envelopes', async ({
		...
		for (const sub of [a, b]) {
			for (let i = 0; i < 2; i++) {
				...
				expect(allSecrets.has(s), 'each rotate across subscriptions is globally unique').toBe(
					false,
				);
				allSecrets.add(s);
			}
		...
		for (const s of allSecrets) {
			expect(listText, `at-rest secret never round-trips: ${s.slice(0, 6)}…`).not.toContain(s);
		}
