...
test.describe('flow: work collaboration — concurrent edit + activity attribution', () => {
    ...
    test('two collaborators edit one work: shared feed records both actors, per-user log splits by actor', async ({
        ...
        const owner: RegisteredUser = await registerUserViaAPI(request, {
            name: `Collab Own ${suffix}`,
        });
        const member: RegisteredUser = await registerUserViaAPI(request, {
            name: `Collab Mem ${suffix}`,
        });
        ...
        expect(
            types.filter((t) => t === WORK_CREATED),
            'one create row',
        ).toHaveLength(1);
        expect(
            types.filter((t) => t === MEMBER_INVITED),
            'one invite row',
        ).toHaveLength(1);
        expect(
    ...
    test('a viewer sees other collaborators changes on refresh but is write-gated (403)', async ({
        ...
        await expect
            .poll(async () => (await getWork(request, viewer.access_token, work.id)).name, {
                timeout: 20_000,
                message: 'viewer should see the collaborator change on refresh',
            })
            .toBe(newName);
    ...
    test('category=settings filter spans both actors edits and excludes membership events; feed is access-gated', async ({
        ...
        await patchWork(request, member.access_token, work.id, {
            description: `mem desc ${suffix}`,
        });
        ...
        expect(
            settingsRows.some((e) => e.type === MEMBER_INVITED || e.type === MEMBER_ROLE_CHANGED),
    ...
    test('update DTO rejects non-allowlisted fields (no merge corruption) and the activity page renders a recorded edit', async ({
        ...
        await page.goto(`${origin}/en/works/${uiWork.id}/activity`, {
