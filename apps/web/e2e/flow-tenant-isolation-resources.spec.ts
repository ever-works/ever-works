...
test.describe('Tenant isolation across every resource type', () => {
	test('flow 1 — Skills: two tenants see only their own; cross GET/PATCH/DELETE 404; bad/unknown-id boundary', async ({
		...
		expect(
			(await request.patch(crossUrl, { headers: b.headers, data: { title: 'hijack' } })).status(),
		).toBe(404);
		...
		expect(
			(await request.get(`${API_BASE}/api/skills/not-a-uuid`, { headers: b.headers })).status(),
		).toBe(400);
		expect(
			(await request.get(`${API_BASE}/api/skills/${UNKNOWN_UUID}`, { headers: b.headers })).status(),
		).toBe(404);
	...
	test('flow 2 — Skill bindings sub-tree: owner reads + creates; the other tenant is 404 on both verbs', async ({
		...
		expect(created.status(), `binding create body=${await created.text().catch(() => '')}`).toBe(201);
		...
		expect(
			(await request.post(bindingsUrl, { headers: b.headers, data: { targetType: 'tenant' } })).status(),
		).toBe(404);
	});
	...
	test('flow 3 — Conversations: own-only list; cross GET/PATCH/DELETE + message-injection all 404', async ({
		...
		expect(listA.status()).toBe(200);
		expect(listB.status()).toBe(200);
		const aThreads = (await listA.json()).conversations as Array<{ id: string; userId: string }>;
		const bThreads = (await listB.json()).conversations as Array<{ id: string; userId: string }>;
		...
		expect(
			(await request.get(`${API_BASE}/api/conversations/not-a-uuid`, { headers: b.headers })).status(),
		).toBe(404);
	...
	test('flow 4 — Knowledge base: KB docs are work-scoped; the other tenant gets 403/404 on every verb', async ({
		...
		const aList = await request.get(`${API_BASE}/api/works/${aWorkId}/kb/documents`, { headers: a.headers });
		...
		const crossList = await request.get(`${API_BASE}/api/works/${aWorkId}/kb/documents`, { headers: b.headers });
		...
		expect([403, 404]).toContain(
			(await request.patch(docUrl, { headers: b.headers, data: { title: 'hijack' } })).status(),
		);
		expect([403, 404]).toContain((await request.delete(docUrl, { headers: b.headers })).status());
		...
		const stillThere = await request.get(`${API_BASE}/api/works/${aWorkId}/kb/documents`, { headers: a.headers });
	...
	test('flow 5 — Agent + Task secondary writes: runs/assign-task/assignees/transition all 404 cross-tenant', async ({
		...
		const agent = await createAgentViaAPI(request, a.token, { scope: 'tenant', name: `Alpha Agent ${stamp()}` });
	...
	test('flow 6 — cross-resource sweep: the GLOBAL org-slug resolver is reachable cross-tenant yet leaks NO scoped rows', async ({
		...
		const agent = await createAgentViaAPI(request, owner.token, { scope: 'tenant', name: `Sweep Agent ${s}` });
		...
		const vWorks = await request.get(`${API_BASE}/api/works?limit=100`, { headers: visitor.headers });
		expect(vWorks.status()).toBe(200);
		expect(((await vWorks.json()).works ?? []).map((w: { id: string }) => w.id)).not.toContain(workId);

		const vAgents = await request.get(`${API_BASE}/api/agents?limit=100`, { headers: visitor.headers });
		expect(((await vAgents.json()).data ?? []).map((x: { id: string }) => x.id)).not.toContain(agent.id);

		const vTasks = await request.get(`${API_BASE}/api/tasks?limit=100`, { headers: visitor.headers });
		expect(((await vTasks.json()).data ?? []).map((x: { id: string }) => x.id)).not.toContain(task.id);

		const vMissions = await request.get(`${API_BASE}/api/me/missions`, { headers: visitor.headers });
		...
		const vSkills = await request.get(`${API_BASE}/api/skills?limit=100`, { headers: visitor.headers });
		expect(((await vSkills.json()).data ?? []).map((x: { id: string }) => x.id)).not.toContain(skill.id);

		const vConvos = await request.get(`${API_BASE}/api/conversations?limit=100`, { headers: visitor.headers });
		expect(((await vConvos.json()).conversations ?? []).map((x: { id: string }) => x.id)).not.toContain(convo.id);

		// ── Direct cross-tenant GET on every id is forbidden (404 for the
		//    own-row-or-nothing resources; 403/404 for the work guard). ──
		expect((await request.get(`${API_BASE}/api/agents/${agent.id}`, { headers: visitor.headers })).status()).toBe(
			404,
		);
		expect((await request.get(`${API_BASE}/api/tasks/${task.id}`, { headers: visitor.headers })).status()).toBe(404);
		expect(
			(await request.get(`${API_BASE}/api/me/missions/${missionId}`, { headers: visitor.headers })).status(),
		).toBe(404);
		expect((await request.get(`${API_BASE}/api/skills/${skill.id}`, { headers: visitor.headers })).status()).toBe(
			404,
		);
		expect(
			(await request.get(`${API_BASE}/api/conversations/${convo.id}`, { headers: visitor.headers })).status(),
		).toBe(404);
		expect([403, 404]).toContain(
			(await request.get(`${API_BASE}/api/works/${workId}`, { headers: visitor.headers })).status(),
		);
	});
});
