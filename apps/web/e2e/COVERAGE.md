...
name different blockers (T14's D1 guard, T15/T16's missing service-layer wiring, T18's DNS fake) —
none of them the connection.
...
| `flow-template-fork-success.spec.ts`         | [~]    | ACC-REG-02             | **the create with the fork fields is ACCEPTED (200, a Work with `kind: "app"`) and the fake still records no fork call** — because `work-lifecycle.service.ts` branches only on the repo kind, so nothing forks yet                                                                                             | `test.fixme('APW-13 T15: nothing acts on repositoryMode yet …')` — fork into the user + into an organization, with the user's token on the recorded call                            |
| `flow-activity-deploy-and-pr-events.spec.ts` | [~]    | ACC-REG-07             | the Activity read surface every step lands on: a Work step appears as a named row (`actionType` / `action` / `status` / `summary`, no payload), 401 unauthenticated, owner-scoped                                                                                                                               | `test.fixme('APW-13 T16: the fork event needs a service that acts on repositoryMode …')` — template-fork, deploy and PR events                                                      |
