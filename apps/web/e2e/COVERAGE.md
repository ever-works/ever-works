...
| `flow-managed-subdomain-allocation.spec.ts`  | [~]    | ACC-REG-05             | in `e2e.yml`: the unallocated read and the six `PUT` refusals. The per-user cap of 3 on the `ever-works` deploy provider and the DNS-provider boundary a successful allocation stops at need `DEPLOY_EVER_WORKS_ENABLED=true` in the API's environment, which `e2e.yml` does not set — so both are **red** there (run 35455975352, job 105940305867: `deployProvider` persisted as `"vercel"`), by design rather than passing vacuously | `test.fixme('APW-13 T18: needs a DNS provider fake')` — a successful allocation and the CNAME it writes                                                                               |
...
variable** — which is why the managed-subdomain row above has two red cases in that
workflow. Whether `e2e.yml` gains them (or a second job that sets them) is an open
decision, not something this table assumes.
