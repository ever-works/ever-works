 ...
 * and pull-request paths, so the event rows themselves can only be produced once
 * the GitHub connection surface exists (T63, plan §8.8) — the fixme'd test below
 * is that half.
 *
...
test.describe('Activity — template fork and PR events (T63)', () => {
    // The events ACC-REG-07 names — a template fork, a deploy, a domain, a PR, a
    // merge — are emitted by paths that need the GitHub connection surface
    // (plan §8.8), and the pull-request half additionally needs an upstream
    // estate. They are asserted here, in the surface proven above, as soon as
    // T63 un-fixmes the marker. The marker is T14's, shared with
    // T15/T16/T30/T31.
    test.fixme('APW-13 T63: no supported GitHub connection surface', async ({
        ...
        expect(created.status(), `fork create body=${(await created.text()).slice(0, 300)}`).toBe(
            200,
        );
