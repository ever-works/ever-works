...
function generateBody(prompt = 'build a directory of e2e widgets'): { name: string; prompt: string } {
...
test.describe('Work hard-delete cascade — deep integration', () => {
	...
	test('delete envelope reports deleted_repositories as an array (empty for a never-generated work)', async ({
		...
		const del = await deleteWork(request, workId, owner.access_token, { deleteRepositories: true });
		...
		expect((repos as unknown[]).length, 'no git repo to clean for a never-generated work').toBe(0);
		...
		const bad = await call(request, 'post', `/api/works/${created2.id}/delete`, owner.access_token, {
		...
		const stillThere = await call(request, 'get', `/api/works/${created2.id}`, owner.access_token);
	...
	test('delete is owner-gated: a non-owner gets 403 and cannot read the sub-resources', async ({
		...
		for (const sub of ['items', ...LIVE_SUBRESOURCES]) {
			const r = await call(request, 'get', `/api/works/${workId}/${sub}`, stranger.access_token);
		...
		expect(stillThere.code, 'owner still sees the work after a rejected non-owner delete').toBe(200);
	...
	test('cross-user isolation + item bulk-delete: each owner deletes only their own cascade', async ({
		...
		const bobCreated = await createWorkViaAPI(request, bob.access_token, { name: `Bob ${Date.now()}` });
		...
		expect(
			typeof aliceBulk.body?.succeeded === 'number' && typeof aliceBulk.body?.failed === 'number',
		...
		const aliceGone = await call(request, 'get', `/api/works/${aliceWork.workId}`, alice.access_token);
		...
		const bobStillThere = await call(request, 'get', `/api/works/${bobWorkId}`, bob.access_token);
