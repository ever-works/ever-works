...
async function inspectDeliveredLinkHost(
	...
	if (!(await isMailhogAvailable(request))) return { delivered: false, host: null, hijacked: false };
...
test.describe('Flow: mail deeplink resolution / callback allow-list / bounce', () => {
	...
	test('verify-email / reset-password / magic-link deeplinks each resolve to their correct landing', async ({
		...
		const rBad = await request.get(`${origin}/api/auth/reset-password?token=${KNOWN_BAD_TOKEN}`, {
			maxRedirects: 0,
		});
	...
	test('an evil callback host is accepted but never used in the emailed link (silent allow-list fallback)', async ({
		...
		if (verifyMail.delivered) {
			expect(verifyMail.hijacked, `verification link host hijacked to ${verifyMail.host}`).toBe(
				false,
			);
		}
		...
		if (evilForgot.status() === 200) {
			...
			if (resetMail.delivered) {
				expect(resetMail.hijacked, `reset link host hijacked to ${resetMail.host}`).toBe(false);
			}
		}
		...
		if (evilMagic.status() === 200) {
			...
			if (magicMail.delivered) {
				expect(magicMail.hijacked, `magic-link host hijacked to ${magicMail.host}`).toBe(false);
			}
		}
	...
	test('magic-link deeplink page errors on a bad token; callback DTOs are strictly whitelisted', async ({
		...
		const nonStringCases: Array<{ path: string; data: Record<string, unknown>; field: string }> = [
			{
		...
		const magicEnabled =
			providers.ok() && ((await providers.json()) as { magicLink?: boolean }).magicLink === true;
		...
		try {
			...
			await expect(async () => {
				const settled =
					(await errorPanel.first().isVisible().catch(() => false)) ||
