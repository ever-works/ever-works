...
function pickWork(raw: unknown): { id?: string; name?: string; slug?: string; description?: string } {
	const r = raw as {
...
test.describe('Work rename + slug immutability / collision / edge cases', () => {
	...
	test('rename name propagates to GET + list + detail UI; slug stays stable', async ({
		...
		const getWork = pickWork(
			await (
				await request.get(`${API_BASE}/api/works/${workId}`, { headers: authedHeaders(token) })
			).json(),
		);
		...
		const newNames: string[] = (
			(await (
				await request.get(`${API_BASE}/api/works?search=${encodeURIComponent(renamedName)}`, {
					headers: authedHeaders(token),
				})
			).json()).works ?? []
		...
		const oldNames: string[] = (
			(await (
				await request.get(`${API_BASE}/api/works?search=${encodeURIComponent(originalName)}`, {
					headers: authedHeaders(token),
				})
			).json()).works ?? []
		).map((w: { name: string }) => w.name);
	...
	test('three creates racing one slug → all distinct or rejected, never duplicate / 5xx', async ({
		...
		for (const id of collidingIds) {
			const persisted = pickWork(
				await (
					await request.get(`${API_BASE}/api/works/${id}`, { headers: authedHeaders(token) })
				).json(),
			);
			expect(
				collidingSlugs,
	...
	test('name/description sanitise-then-truncate on edit; empty name / bad slug rejected', async ({
		...
		expect(isUrlSafeSlug(uniWork.slug!), `unicode work slug "${uniWork.slug}" is URL-safe`).toBe(
			true,
		);
		...
		expect(noSlug.status(), 'create without a slug is a 4xx (slug required)').toBeGreaterThanOrEqual(
			400,
		);
		...
		expect(badSlug.status(), 'non-URL-safe slug on create is a 4xx').toBeGreaterThanOrEqual(400);
