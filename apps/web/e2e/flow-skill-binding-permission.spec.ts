...
test.describe('Skill binding — permission gate, scope rules, cross-tenant isolation', () => {
    ...
    test('cross-tenant: binding ONTO another tenant’s skill 404s, and a foreign-target binding is rejected at write time', async ({
        ...
        expect(
            (await listAgentSkills(request, aliceToken, aliceAgent.id)).map((r) => r.skill.id),
        ).toEqual([aliceSkill.id]);
        ...
        const bobBindOntoAlice = await request.post(
            `${API_BASE}/api/skills/${aliceSkill.id}/bindings`,
            {
                headers: authedHeaders(bobToken),
        ...
        const bobReadAgentSkills = await request.get(
            `${API_BASE}/api/agents/${aliceAgent.id}/skills`,
            {
                headers: authedHeaders(bobToken),
            },
        );
        ...
        const bobDelete = await request.delete(
            `${API_BASE}/api/skill-bindings/${aliceBinding.id}`,
            {
                headers: authedHeaders(bobToken),
            },
        );
    ...
    test('binding write guards (missing targetId / bad targetType / duplicate) and tenant-wide + work bindings co-resolve', async ({
        ...
        expect((await missingTarget.json()).message).toMatch(
