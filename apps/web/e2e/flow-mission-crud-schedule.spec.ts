...
test.describe('Mission CRUD + schedule/cadence + status', () => {
    ...
    test('create validation: bounds, enum, whitelist, and schedule↔type consistency all 400 truthfully', async ({
        ...
        const badBodies: Array<{ name: string; body: Record<string, unknown>; match: RegExp }> = [
            {
                name: 'title>200',
                body: { title: longTitle, description: 'x', type: 'one-shot' },
                match: /title must be shorter than or equal to 200 characters/i,
            },
            {
                name: 'description missing',
                body: { type: 'one-shot' },
                match: /description must be a string|description must be longer than or equal to 1 characters/i,
            },
            {
                name: 'description empty',
                body: { description: '', type: 'one-shot' },
                match: /description must be longer than or equal to 1 characters/i,
            },
            {
                name: 'description>10000',
                body: { description: longDesc, type: 'one-shot' },
                match: /description must be shorter than or equal to 10000 characters/i,
            },
            {
                name: 'type bad enum',
                body: { description: 'x', type: 'recurring' },
                match: /type must be one of the following values: one-shot, scheduled/i,
            },
            {
                name: 'schedule>64',
                body: { description: 'x', type: 'scheduled', schedule: longCron },
                match: /schedule must be shorter than or equal to 64 characters/i,
            },
            {
                name: 'cap < -1',
                body: { description: 'x', type: 'one-shot', outstandingIdeasCap: -2 },
                match: /outstandingIdeasCap must not be less than -1/i,
            },
            {
                name: 'unknown property',
                body: { description: 'x', type: 'one-shot', bogus: true },
                match: /property bogus should not exist/i,
            },
            {
                name: 'scheduled without cron',
                body: { description: 'x', type: 'scheduled' },
                match: /scheduled requires a non-empty `schedule`/i,
            },
            {
                name: 'one-shot WITH cron',
                body: { description: 'x', type: 'one-shot', schedule: '0 9 * * 1' },
                match: /one-shot must NOT have a `schedule`/i,
            },
        ];
    ...
    test('autoBuildWorks + cap sentinels: every toggle persists across a fresh GET', async ({
        ...
        async function patch(data: Record<string, unknown>): Promise<MissionDto> {
            ...
            expect(res.status(), `patch ${JSON.stringify(data)} body=${await res.text()}`).toBe(
                200,
            );
            return res.json();
        }
    ...
    test('status state-machine: illegal transitions 400 with exact messages from every status', async ({
        ...
        const delCompleted = await request.delete(`${API_BASE}/api/me/missions/${c.id}`, {
            headers,
        });
        ...
        // Both gone now.
        expect(
            (await request.get(`${API_BASE}/api/me/missions/${c.id}`, { headers })).status(),
        ).toBe(404);
        expect(
            (await request.get(`${API_BASE}/api/me/missions/${a.id}`, { headers })).status(),
        ).toBe(404);
    });
     ...
     */
    test('run-now bypasses cron for one-shot AND scheduled; gated by status', async ({
        request,
    }) => {
        const owner = await registerUserViaAPI(request);
        const token = owner.access_token;
        ...
        await request.post(`${API_BASE}/api/me/missions/${scheduled.id}/resume`, {
            headers,
            data: {},
        });
        await request.post(`${API_BASE}/api/me/missions/${scheduled.id}/complete`, {
            headers,
            data: {},
        });
