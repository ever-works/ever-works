...
test.describe('Work import (deep) — source/payload pipeline matrix', () => {
	...
	test('sourceType matrix: data_repo records an error-history (no items) while link_existing records a generated/linked history', async ({
		...
		const dataWork = asWork(
			await (await request.get(`${API_BASE}/api/works/${dataWorkId}`, { headers: authedHeaders(token) })).json(),
		);
		...
		const dataHistory = await (
			await request.get(`${API_BASE}/api/works/${dataWorkId}/history`, { headers: authedHeaders(token) })
		).json();
		expect(dataHistory.status).toBe('success');
		const dataRow = (dataHistory.history ?? []).find((h: any) => h.generationMethod === 'import');
		...
		const linkWork = asWork(
			await (await request.get(`${API_BASE}/api/works/${linkWorkId}`, { headers: authedHeaders(token) })).json(),
		);
		...
		const linkHistory = await (
			await request.get(`${API_BASE}/api/works/${linkWorkId}/history`, { headers: authedHeaders(token) })
		).json();
		const linkRow = (linkHistory.history ?? []).find((h: any) => h.parameters?.sourceType === 'link_existing');
		...
		const list = await (await request.get(`${API_BASE}/api/works`, { headers: authedHeaders(token) })).json();
	...
	test('re-import idempotency: same name dedups by derived slug regardless of source URL; new name imports anew', async ({
		...
		const firstWork = asWork(
			await (await request.get(`${API_BASE}/api/works/${firstId}`, { headers: authedHeaders(token) })).json(),
		);
	...
	test('import analyze: parses owner/repo, gates on auth + URL validity, and creates no work', async ({
		...
		const badMsg = Array.isArray(badBody.message) ? badBody.message.join(' ') : String(badBody.message ?? '');
	...
	test('provenance round-trip: analyze, work.sourceRepository, and history.parameters agree on owner/repo', async ({
		...
		const work = asWork(
			await (await request.get(`${API_BASE}/api/works/${workId}`, { headers: authedHeaders(token) })).json(),
		);
		...
		const history = await (
			await request.get(`${API_BASE}/api/works/${workId}/history`, { headers: authedHeaders(token) })
		).json();
	...
	test('imported work is owner-scoped: stranger gets 403 on GET + history, ghost id is 404', async ({
		...
		expect(
			(await request.get(`${API_BASE}/api/works/${workId}`, { headers: authedHeaders(owner.access_token) })).status(),
		).toBe(200);
		expect(
			(
	...
	test('malformed + config-gated imports reject and persist nothing; works/new import UI renders', async ({
		...
		const before = await (await request.get(`${API_BASE}/api/works`, { headers: authedHeaders(token) })).json();
		...
		const badType = await request.post(`${API_BASE}/api/works/import`, {
			headers: authedHeaders(token),
			data: { sourceUrl: `https://github.com/a-${s}/b-${s}`, sourceType: 'bogus', name: `Bad ${s}`, gitProvider: GITHUB },
		});
		...
		const noProvider = await request.post(`${API_BASE}/api/works/import`, {
			headers: authedHeaders(token),
			data: { sourceUrl: `https://github.com/c-${s}/d-${s}`, sourceType: 'data_repo', name: `NoProv ${s}` },
		});
		expect(noProvider.status()).toBe(400);
		const noProvBody = await noProvider.json().catch(() => ({}));
		const noProvMsg = Array.isArray(noProvBody.message) ? noProvBody.message.join(' ') : String(noProvBody.message ?? '');
		...
		if (awesome.body.providerErrors || /provider/i.test(String(awesome.body.message ?? ''))) {
			...
			expect(
				awesome.body.providerErrors !== undefined || /provider|configured/i.test(String(awesome.body.message)),
			).toBeTruthy();
		}
		...
		const after = await (await request.get(`${API_BASE}/api/works`, { headers: authedHeaders(token) })).json();
		...
		await request
			.post(`${API_BASE}/api/auth/login`, { data: { email: sUser.email, password: sUser.password } })
			.catch(() => undefined);
