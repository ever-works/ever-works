...
async function freshToken(request: APIRequestContext): Promise<{ user: RegisteredUser; token: string }> {
...
test.describe('Clone an Agent — export → import copies files/permissions/avatar', () => {
	...
	test('full clone faithfully copies files, permissions, avatar and runtime knobs into a DRAFT', async ({
		...
		await writeAgentFile(request, token, source.id, 'SOUL.md', '# Soul\nBe concise and rigorous.');
		await writeAgentFile(request, token, source.id, 'AGENTS.md', '# Playbook\nAlways cite sources.');
