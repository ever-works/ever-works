...
const uniqSlug = (base: string) =>
	`${base.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${Date.now()
		.toString(36)}${Math.random().toString(36).slice(2, 5)}`;
...
async function rawCreate(
	...
	const res = await request.post(`${API_BASE}/api/works`, { headers: authedHeaders(token), data });
...
test.describe('work kind variants — create surface, domain inference + override, org, repo visibility', () => {
	...
	test('flow 1: fresh works are kind=default/status=active regardless of intent; kind is server-owned', async ({
		...
		for (const intent of ['company', 'store', 'website']) {
			...
			if ('tenantId' in w) expect(w.tenantId ?? null, `${intent} no tenant at create`).toBeNull();
		}
	...
	test('flow 4: organization flag shapes repo ownership; org link is deferred; required boolean at create', async ({
		...
		if (orgTrue.kind !== undefined) expect(orgTrue.kind, 'org:true is still kind=default').toBe('default');
	...
	test('flow 5: repository visibility — owner enumerates repo types, PUT shape, anon 401, stranger 403', async ({
		...
		const getVis = await request.get(`${API_BASE}/api/works/${work.id}/repositories/visibility`, {
			headers: authedHeaders(token),
		});
		...
		const putVis = await request.put(`${API_BASE}/api/works/${work.id}/repositories/visibility`, {
		...
		const stranger = await registerUserViaAPI(request);
		const cross = await request.get(`${API_BASE}/api/works/${work.id}/repositories/visibility`, {
			headers: authedHeaders(stranger.access_token),
		});
	...
	test('flow 6: dup-slug rejected, rename round-trip, tenant-isolated list, 403/404 on non-owned ids', async ({
		...
		const first = await createWork(request, token, { name: `dup-a ${stamp}`, slug: sharedSlug });
		...
		const dup = await request.post(`${API_BASE}/api/works`, {
			headers: authedHeaders(token),
			data: { name: `dup-b ${stamp}`, slug: sharedSlug, description: 'd', organization: false },
		});
	...
	test('flow 7: UI /works/new exposes the live work-kind chip catalog + manual affordance', async ({
		...
		expect(liveKindHits.filter(Boolean).length, 'multiple live kind chips present').toBeGreaterThan(0);
		...
		if (storeOrCompany.filter(Boolean).length === 0) {
			test.info().annotations.push({
				type: 'note',
				description: 'Store/Company coming-soon chips not visible on this build (flag-gated catalog).',
			});
		}
		...
		const manualBtn = page
			...
			.first();
		await expect(manualBtn, 'manual create affordance present').toBeVisible({ timeout: 20_000 });
