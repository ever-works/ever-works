...
test.describe('Skill context assembly into an agent', () => {
    ...
    test('multiple distinct skills combine into one priority-ordered context bundle', async ({
        ...
        await bindSkill(request, token, sA.id, {
            targetType: 'agent',
            targetId: agent.id,
            priority: 50,
        });
        await bindSkill(request, token, sB.id, {
            targetType: 'agent',
            targetId: agent.id,
            priority: 5,
        });
        await bindSkill(request, token, sC.id, { targetType: 'tenant', priority: 100 });
        await bindSkill(request, token, sD.id, {
            targetType: 'agent',
            targetId: agent.id,
            priority: 25,
        });
     ...
     */
    test('assembled context re-resolves live when a shared skill is edited', async ({
        request,
    }) => {
        const user = await registerUserViaAPI(request);
        const token = user.access_token;
