...
function looksChallenged(status: number, body: Record<string, unknown>): boolean {
	return (
		...
		String(body.status ?? '').toLowerCase().includes('two') ||
...
test.describe('2FA full state machine — enroll → verify → challenge → recover → disable', () => {
	test('enroll surface enforces auth, never silently succeeds, and a fresh user is NOT enabled', async ({
		...
		if (!enablePath && !statusPath) {
			...
			expect(lb.access_token, 'no half-open 2FA state: login yields a full session').toBeTruthy();
			test.skip(true, '2FA enroll/status surface not exposed in this environment (PROBED 404)');
		}
	...
	test('enroll → TOTP verify → challenge-on-login (real RFC-6238 happy path; degrades when 2FA absent)', async ({
		...
		if (!enablePath) {
			...
			expect(body.access_token, 'login yields an access_token (no challenge gate)').toBeTruthy();
			expect(
		...
		if (enableRes.status() >= 400) {
			test.skip(true, `enable rejected with ${enableRes.status()} — DTO not satisfiable blindly`);
		}
		...
		if (verifyPath && secret) {
			...
			if (verifyRes.status() < 300) {
				...
				test
					.info()
					.annotations.push({ type: 'note', description: 'TOTP verify present but code rejected (skew/params)' });
			}
		} else if (verifyPath) {
			...
			if (enabledNow !== null) {
				expect(enabledNow, '2FA must not be enabled before TOTP verify confirms possession').toBe(false);
			}
		} else {
			test
				.info()
				.annotations.push({ type: 'note', description: 'enable present but no TOTP verify route to complete the happy path' });
		}
	});
		...
		if (!verifyPath) {
			...
			for (let i = 0; i < 5; i++) {
				const res = await call(request, 'POST', UPDATE_PASSWORD_PATH, {
					token: u.access_token,
					data: { currentPassword: `Wrong${i}!pw9secure`, newPassword: 'BrandNew9!secure' },
				});
	...
	test('backup/recovery codes: not issuable/usable without 2FA, and a consumed code is single-use', async ({
		...
		const verifyBackupPath = await resolvePath(request, VERIFY_BACKUP_PATHS, 'POST', u.access_token);
		...
		if (!genPath && !verifyBackupPath) {
			...
			const loa1 = await call(request, 'POST', LOGOUT_ALL_PATH, { token: u.access_token, data: {} });
			expect(loa1.status(), 'logout-all authed → 200').toBe(200);
			const loa2 = await call(request, 'POST', LOGOUT_ALL_PATH, { token: u.access_token, data: {} });
			expect(loa2.status(), 'repeated logout-all must not 5xx (idempotent-safe)').toBeLessThan(500);
		...
		if (verifyBackupPath) {
			...
			expect(preVerify.status(), `verify-backup pre-enroll → ${preVerify.status()}`).toBeGreaterThanOrEqual(400);
		...
		const enablePath = await resolvePath(request, ENABLE_PATHS, 'POST', u.access_token);
		const verifyTotpPath = await resolvePath(request, VERIFY_TOTP_PATHS, 'POST', u.access_token);
		if (!enablePath || !verifyTotpPath || !genPath || !verifyBackupPath) {
			test
				.info()
				.annotations.push({ type: 'note', description: 'single-use leg degraded: full enroll/verify/issue/consume loop not all present' });
			return;
		}
		...
		const genRes = await call(request, 'POST', genPath, { token: u.access_token, data: { password: u.password } });
	...
	test('step-up: mutating the auth factor requires the current factor (no proof → 4xx, never 5xx)', async ({
		...
		expect(missingProof.status(), 'update-password with no current factor → 400 DTO gate').toBe(400);
		...
		expect(wrongProof.status(), 'update-password with wrong current factor → 401 step-up gate').toBe(401);
		...
		if (disablePath) {
			const noProof = await call(request, 'POST', disablePath, { token: u.access_token, data: {} });
			expect(
				noProof.status(),
			...
			expect([400, 401, 403], `anon 2FA disable → ${anonDisable.status()}`).toContain(anonDisable.status());
		}
		if (genPath) {
			const noProof = await call(request, 'POST', genPath, { token: u.access_token, data: {} });
			expect(
				noProof.status(),
		...
		if (!disablePath && !genPath) {
			test
				.info()
				.annotations.push({ type: 'note', description: '2FA disable/regenerate surface absent (PROBED 404); step-up asserted via update-password' });
		}
	});
	...
	test('disable / recovery event revokes sessions and returns the account to a usable login (lifecycle close-out)', async ({
		...
		if (!enablePath || !disablePath) {
			...
			const restored = await loginViaAPI(request, { email: u.email, password: 'Rotated9!secure' });
			...
			const loa = await call(request, 'POST', LOGOUT_ALL_PATH, { token: restored.access_token, data: {} });
		...
		const disableRes = await call(request, 'POST', disablePath!, {
			token: u.access_token,
			data: { password: u.password, currentPassword: u.password, code: disableCode, token: disableCode },
		});
		expect(
			disableRes.status(),
		...
		if (disableRes.status() < 300) {
			...
			expect(reLogin.access_token, 'after a confirmed disable, plain login is restored').toBeTruthy();
		}
	});
	...
	test('seeded user: the Security settings page renders without hard-error (2FA control surface UI smoke)', async ({
		...
		const twoFa = page
			...
			.first()
			.or(page.getByRole('button', { name: /two[\s-]?factor|authenticator|2fa|enable/i }).first());
		const visible = await twoFa.isVisible().catch(() => false);
		if (!visible) {
			test
				.info()
				.annotations.push({ type: 'note', description: '2FA UI control not present on /settings/security in this env (flagged off / API surface absent / route divergence)' });
		}
