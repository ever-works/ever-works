...
test.describe('Work deploy state machine (deep integration)', () => {
    test('fresh-work deploy columns are all idle/null and a refused (unconfigured) deploy NEVER writes deploymentState/StartedAt — the gate runs before any state mutation', async ({
        ...
        const deployBody = (await deploy.json().catch(() => null)) as Record<
            string,
            unknown
        > | null;
        if (deploy.status() === 400) {
            ...
            expect(String(deployBody?.message ?? '')).toMatch(
        ...
        if (!wasAccepted) {
            ...
            expect(
                after.deploymentStartedAt,
                'refused deploy must not stamp deploymentStartedAt',
            ).toBeNull();
    ...
    test('capability gating is CONSISTENT across check/lookup/deploy on the same undeployable work, and is idempotent across repeated reads', async ({
        ...
        expect(((await check2.json()) as Record<string, unknown>).canDeploy).toEqual(
            checkBody.canDeploy,
        );
        ...
        const lookupBody = (await lookup.json().catch(() => null)) as Record<
            string,
            unknown
        > | null;
        if (!canDeploy) {
            ...
            expect(String(lookupBody?.message ?? '')).toMatch(
                /token is required to lookup|not configured/i,
            );
        } else {
    ...
    test('redeploy idempotency: repeatedly deploying an undeployable work yields a STABLE truthful refusal each time and never corrupts the work or its history', async ({
        ...
        if (firstStatus === 400) {
            ...
            expect(
                messages.every((m) => /token is required|not configured|Plugin Settings/i.test(m)),
            ).toBe(true);
        }
    ...
    test('rollback gating state machine: bad-UUID and empty-body are DTO-rejected (400), a well-formed but non-existent deployment id is 400 "Deployment not found", and the work survives every rejected rollback', async ({
        ...
        expect(JSON.stringify(badUuidBody.message ?? badUuidBody)).toMatch(
            /deploymentId must be a UUID|uuid/i,
        );
        ...
        expect(absent.status(), `absent rollback body=${await absent.text().catch(() => '')}`).toBe(
            400,
        );
        ...
        expect(String(absentBody.message ?? '')).toMatch(
    ...
    test('deploy ownership matrix: every deploy-capability verb honours ownership — cross-user 403, missing-work 404, unauthenticated 401 — across deploy/check/lookup/deployments/rollback', async ({
        ...
        const owner = await registerUserViaAPI(request);
        const other = await registerUserViaAPI(request);
        const workId = await freshWorkId(
            request,
            owner.access_token,
            `Ownership Work ${Date.now()}`,
        );
        ...
        const verbs: Verb[] = [
            {
                name: 'deploy',
                method: 'POST',
                path: (id) => `${DEPLOY_BASE}/works/${id}`,
                data: {},
            },
            {
                name: 'check',
                method: 'POST',
                path: (id) => `${DEPLOY_BASE}/works/${id}/check`,
                data: {},
            },
            {
                name: 'lookup',
                method: 'POST',
                path: (id) => `${DEPLOY_BASE}/works/${id}/lookup`,
                data: {},
            },
            {
                name: 'deployments',
                method: 'GET',
                path: (id) => `${DEPLOY_BASE}/works/${id}/deployments`,
            },
            {
                name: 'rollback',
                method: 'POST',
        ...
        for (const v of verbs) {
            ...
            expect(
                [403, 404],
                `cross-user ${v.name} status (body=${await res.text().catch(() => '')})`,
