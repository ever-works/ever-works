 ...
 */
function expectCancelSuccess(probe: { status: number; body: CancelBody }, ctx: string): void {
    ...
    expect(typeof probe.body.message, `${ctx}: cancel carries a human-readable message`).toBe(
        'string',
    );
    expect(
        probe.body.message && probe.body.message.length,
...
test.describe('flow: work generation cancellation (in-flight, idle, regenerate, race, authz)', () => {
    ...
    test('cancel when NOT generating — idempotent 202 already_finished no-op, stable across repeats + stray body', async ({
        ...
        expect(first.body.mode, 'never-generated work -> already_finished').toBe(
            'already_finished',
        );
        ...
        expect(strayBody.mode, 'stray-body cancel is still already_finished').toBe(
            'already_finished',
        );
    });
    ...
    test('cancel authz matrix — ghost 404, cross-owner 403, unauthenticated 401 (no leak, no 5xx)', async ({
        ...
        expect(String(ghostBody.message), 'ghost message names the missing id').toMatch(
            /not found/i,
        );
