...
import { WorkScheduleBillingMode, WorkScheduleCadence } from '@/lib/api/enums';
...
const updateScheduleSchema = z.object({
    ...
    cadence: z.nativeEnum(WorkScheduleCadence),
    billingMode: z.nativeEnum(WorkScheduleBillingMode),
...
export async function runWorkSchedule(workId: string) {
    const user = await getAuthFromCookie();
    if (!user) {
        return { success: false, error: 'Not authenticated' };
    }
...
export async function cancelWorkSchedule(workId: string) {
    const user = await getAuthFromCookie();
    if (!user) {
        return { success: false, error: 'Not authenticated' };
    }
    ...
    try {
        await workAPI.cancelSchedule(workId);
        revalidatePath(ROUTES.DASHBOARD_WORK(workId));
