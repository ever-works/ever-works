...
import { Directory, GenerateStatusType } from '@/lib/api';
    ...
    const getStatusConfig = () => {
        if (!directory.generateStatus) {
            return {
                ...
                action: (
                    <Button
                        onClick={() => router.push(`${ROUTES.DASHBOARD_DIRECTORY(directory.id)}/generator`)}
                        variant="primary"
                    >
                        {t('notStarted.action')}
                    </Button>
            ...
            [GenerateStatusType.GENERATING]: {
                ...
                action: (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-text-muted dark:text-text-muted-dark">{t('generating.processing')}</span>
                    </div>
                ),
            },
            [GenerateStatusType.GENERATED]: {
                ...
                action: (
                    <div className="flex gap-2">
                        <Button
                            onClick={() => router.push(`${ROUTES.DASHBOARD_DIRECTORY(directory.id)}/items`)}
                            variant="secondary"
                            size="sm"
                        >
