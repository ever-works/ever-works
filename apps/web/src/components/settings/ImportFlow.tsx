...
export function ImportFlow({ onClose, initialPayload, initialPreview, isPullMode }: ImportFlowProps) {
			...
			<div className="space-y-4">
				{!preview.valid ? (
					<div className="p-4 rounded-lg bg-destructive/10 border border-destructive/20">
						<h3 className="font-medium text-destructive mb-2">{t('validationErrors')}</h3>
					...
					<>
						...
						<div className="p-4 rounded-lg bg-surface-secondary dark:bg-surface-secondary-dark">
							...
							<div className="grid grid-cols-2 gap-2 text-sm">
								...
								<span className="text-text dark:text-text-dark">{preview.version}</span>
							</div>
						</div>
						...
						{preview.missingPlugins.length > 0 && (
							<div className="p-3 rounded-lg bg-warning/10 border border-warning/20">
								<div className="flex items-center gap-2 mb-1">
									...
									<span className="text-sm font-medium">{t('missingPlugins')}</span>
								</div>
								...
								{preview.conflicts.map((conflict) => (
									...
									>
										<div className="text-sm mb-2">
											...
											<span className="text-text-muted dark:text-text-muted-dark">
												{' '}
												— {t('existingDirectory')}: &quot;{conflict.existingName}
												&quot;, {t('incoming')}: &quot;{conflict.incomingName}&quot;
											</span>
										</div>
										<select
											value={resolutions.get(conflict.slug)?.strategy || 'skip'}
											onChange={(e) =>
												updateResolution(
													conflict.slug,
