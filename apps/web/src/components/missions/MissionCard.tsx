...
import { ChatAboutButton } from '@/components/ai/conversations/ConversationEntryButtons';
...
export function MissionCard({ mission }: { mission: Mission }) {
    ...
    return (
        // The card stays one link; "Chat about it" sits beside it, not inside
        // it, so a button is never nested in an anchor.
        <div className="relative flex flex-col">
            <Link
                href={ROUTES.DASHBOARD_MISSION(mission.id)}
                className={cn(
                    'group relative flex min-h-[12rem] flex-1 flex-col overflow-hidden rounded-lg p-4 shadow-xs',
