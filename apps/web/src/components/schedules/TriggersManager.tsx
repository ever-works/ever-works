		...
		<div className="mt-8">
			<div className="mb-4 flex items-center justify-between">
				<div>
					...
					<p className="mt-1 text-xs text-text-muted dark:text-text-muted-dark">{t('subtitle')}</p>
				</div>
								...
								<div className="min-w-0">
									...
									<p className="mt-0.5 text-xs text-text-muted dark:text-text-muted-dark">
										...
										{row.lastFiredAt ? (
											<>
												{' · '}
												{t('lastFired')} <ActivityTimestamp value={row.lastFiredAt} />
											</>
										) : null}
									</p>
								</div>
				...
				<DialogContent>
					...
					<div className="space-y-4">
						...
						<div>
							...
							/>
							<p className="mt-1 text-xs text-text-muted dark:text-text-muted-dark">{t('form.taskTitleHint')}</p>
						</div>
					</div>
					<DialogFooter>
						...
						<Button onClick={handleCreate} disabled={isSubmitting || name.trim().length === 0}>
				...
				<div className="space-y-4">
					<div>
						...
						<div className="flex items-center gap-2">
							...
							<Button size="icon" variant="ghost" aria-label={t('reveal.copy')} onClick={() => copy(url, 'url')}>
					...
					<div>
						...
						<div className="relative">
							...
							>
								{copied === 'curl' ? <Check className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4" />}
							</Button>
						</div>
					</div>
				</div>
