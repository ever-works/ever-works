...
export function getSettingsLink(
    ...
    if (
        ...
        m.includes('provider github') ||
        m.includes('github app') ||
        m.includes('installation')
    ) {
        return { label: 'Connect GitHub', href: ROUTES.DASHBOARD_SETTINGS_GITHUB_APP };
    }
    if (
        m.includes('git provider') ||
        m.includes('git credentials') ||
    ...
    if (m.includes('quota') || m.includes('budget') || m.includes('limit exceeded')) {
        return {
            label: 'View usage & budgets',
            href: ROUTES.DASHBOARD_WORK_SETTINGS_BUDGETS(workId),
        };
    }
    if (
        m.includes('configure') ||
        m.includes('settings') ||
        m.includes('plugin') ||
        m.includes('missing')
    ) {
 ...
 */
export function WorkErrorPopup({
    workId,
    message,
    warnings,
    isWarn = false,
                ...
                {isWarn && warnings && (
                    <ul className="flex flex-col gap-0.5 pr-6">
                        {warnings.map((w, i) => (
                            <li
                                key={i}
                                className="flex gap-2 text-[11px] leading-relaxed text-text-secondary dark:text-zinc-400"
                            >
