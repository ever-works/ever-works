...
export function TagsTab({ workId, initialTags, items, canEdit }: TagsTabProps) {
    ...
    const handleDelete = async (tag: Tag) => {
        ...
        try {
            const result = await deleteTag(workId, tag.id);
            if (result.success) {
    ...
    const handleSave = async (data: Partial<Tag>) => {
        try {
            if (editingTag) {
                // Update existing tag
                const result = await updateTag(workId, editingTag.id, data);
                ...
                // Create new tag
                const result = await createTag(workId, data);
                if (result.success && result.tag) {
