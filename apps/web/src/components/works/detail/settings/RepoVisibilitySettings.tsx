...
import { RepositoryStatus, RepositoryType } from '@/lib/api/work';
import { toggleRepositoryVisibility } from '@/app/actions/dashboard/works';
...
export function RepoVisibilitySettings({ initialRepositories }: RepoVisibilitySettingsProps) {
    ...
    const executeToggle = async (repo: RepositoryStatus) => {
        try {
            ...
            const result = await toggleRepositoryVisibility(work.id, repo.type, newIsPrivate);
