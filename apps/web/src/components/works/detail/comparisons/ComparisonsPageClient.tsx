...
import { ProviderSelector } from '@/components/works/detail/generator/ProviderSelector';
import {
...
export function ComparisonsPageClient({
    ...
    const refreshComparisons = useCallback(async () => {
        try {
            const updated = await listComparisons(workId);
    ...
    const handleGenerateNext = () => {
        startTransition(async () => {
            const result = await generateNextComparison(workId);
    ...
    const handleDeleteConfirm = () => {
        ...
        startTransition(async () => {
            const result = await deleteComparison(workId, slug);
    ...
    const handleGenerateAllClick = async () => {
        const { count } = await getRemainingComparisonCount(workId);
        if (count === 0) {
    ...
    const handleGenerateAllConfirm = useCallback(async () => {
        ...
        await refreshComparisons();
    }, [workId, refreshComparisons, remainingCount, t]);
    ...
    const handleSaveAiConfig = async () => {
        ...
        try {
            const result = await saveComparisonAiConfig(workId, {
    ...
    return (
        <div className="space-y-6">
            ...
            <ComparisonGenerationProgress workId={workId} isGenerating={isPending} />
