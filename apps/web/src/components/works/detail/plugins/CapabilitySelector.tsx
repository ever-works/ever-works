...
export function CapabilitySelector({
    ...
    const canConfigureWorkModels = scope === 'work' && Boolean(workId);
    ...
    const handleSelect = (pluginId: string) => {
        if (!workId || pluginId === selectedPluginId || isPending) return;

        const previousPluginId = selectedPluginId;
        setSelectedPluginId(pluginId);
        startTransition(async () => {
            try {
                const result = await setActiveCapability(workId, pluginId, capability);
                if (!result.success) {
        ...
        <div className="grid gap-2 px-5 py-3 @sm/main:grid-cols-[11rem_1fr] @sm/main:items-center">
            ...
            <div className="min-w-0">
                ...
                <ActiveProviderModels
                    ...
                    onConfigure={
                        activePluginModels && activePlugin && canConfigureWorkModels
                            ? () => setSettingsPluginId(activePlugin.pluginId)
                            : undefined
                    }
                />
            </div>
            {settingsPlugin && canConfigureWorkModels && workId && (
