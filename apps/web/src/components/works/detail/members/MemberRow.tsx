...
export function MemberRow({ workId, member, canManage, onRemoved, onUpdated }: MemberRowProps) {
    ...
    const handleRoleChange = async (newRole: AssignableMemberRole) => {
        ...
        setIsUpdating(true);
        try {
            const result = await updateMemberRole(workId, member.id, newRole);
    ...
    const handleRemove = async () => {
        setIsRemoving(true);
        try {
            const result = await removeMember(workId, member.id);
    ...
    const roleOptions = [
        { value: WorkMemberRole.VIEWER, label: t('roles.viewer') },
        { value: WorkMemberRole.EDITOR, label: t('roles.editor') },
        { value: WorkMemberRole.MANAGER, label: t('roles.manager') },
    ];
