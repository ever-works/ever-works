...
export function WorkScheduleCard({
    ...
    const hasPersistedSchedule =
        schedule.cadence !== null || schedule.status !== WorkScheduleStatus.DISABLED;
    ...
    if (!hasPersistedSchedule && !schedule.canEnable) {
        return (
            <ScheduleStateCard
                title={
                    schedule.blockingCode === 'INITIAL_WORK_SETUP_REQUIRED'
                        ? t('empty.title')
                        : t('title')
                }
                description={schedule.blockingReason ?? t('empty.description')}
...
function ScheduleForm({
    ...
    const getCadenceLabel = (cadence: WorkScheduleCadence) => {
        switch (cadence) {
    ...
    const initialForm = {
        enable: schedule.status === WorkScheduleStatus.ACTIVE,
        cadence:
            schedule.cadence ??
...
function ScheduleFormContent({
    ...
    workId: string;
    work: ReturnType<typeof useWorkDetail>['work'];
    schedule: WorkScheduleDto;
    readOnly: boolean;
    allowances: { cadence: WorkScheduleCadence; allowed: boolean }[];
    pipelineProviders: ProviderOption[];
    activeProviders: ResolvedProvider[];
    showPipelineSelector: boolean;
    getCadenceLabel: (cadence: WorkScheduleCadence) => string;
    initialForm: {
        enable: boolean;
    ...
    const isActive = schedule.status === WorkScheduleStatus.ACTIVE;
    const isGenerationRunning = work.generateStatus?.status === 'generating';
    ...
    const saveSchedule = () => {
        startSaving(async () => {
            if (
                form.enable &&
                requiresUsage &&
                form.billingMode !== WorkScheduleBillingMode.USAGE
            ) {
            ...
            <div className="grid gap-4 @lg/main:grid-cols-2">
                ...
                >
                    <Select
                        ...
                        onValueChange={(val) =>
                            updateForm({
                                cadence: val as WorkScheduleCadence,
                            })
                        }
                    >
                ...
                {subscriptionsEnabled && (
                    <FieldCard label={t('fields.billing')} helper={t('fields.billingHelp')}>
                        <Select
                            ...
                            onValueChange={(val) =>
                                updateForm({
                                    billingMode: val as WorkScheduleBillingMode,
                                })
                            }
                        >
                            <option value={WorkScheduleBillingMode.SUBSCRIPTION}>
                                {t('billing.subscription')}
                            </option>
                            <option value={WorkScheduleBillingMode.USAGE}>
                                {t('billing.usage')}
                            </option>
                        </Select>
                    </FieldCard>
