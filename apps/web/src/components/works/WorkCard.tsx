...
function getSettingsLink(
    ...
    if (m.includes('ai provider') || m.includes('baseurl') || m.includes('apikey') || m.includes('api key') || m.includes('openai') || m.includes('anthropic')) {
        return { label: 'Configure AI provider', href: ROUTES.DASHBOARD_WORK_PLUGINS(workId) };
    }
    if (m.includes('no connected account') || m.includes('provider github') || m.includes('github app') || m.includes('installation')) {
        return { label: 'Connect GitHub', href: ROUTES.DASHBOARD_SETTINGS_GITHUB_APP };
    }
    if (m.includes('git provider') || m.includes('git credentials') || m.includes('authentication') || m.includes('unauthorized')) {
...
function StatusBadge({ work, statusConfig, isOpening, isGenerating, baseStatusLabel }: StatusBadgeProps) {
    ...
    const ErrorPopup = ({ message, isWarn = false }: { message?: string | null; isWarn?: boolean }) => {
        const settingsLink = message ? getSettingsLink(message, work.id) : null;
        return (
