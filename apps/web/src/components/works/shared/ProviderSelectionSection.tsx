...
import { WorkPluginSettingsModal } from '../detail/plugins/WorkPluginSettingsModal';
...
export function ProviderSelectionSection({
    ...
    const settingsPlugin =
        workPlugins.find((plugin) => plugin.pluginId === settingsPluginId) ?? null;
    ...
    const workPluginModelsById = useMemo(() => {
        return new Map(
            workPlugins
    ...
    return (
        <>
            {(formSchema.providers.pipeline?.length ?? 0) > 0 && (
                                    ...
                                    const providerOptions =
                                        uiKey === 'ai'
                                            ? options.map((option) => ({
                                                  ...option,
                                                  models:
                                                      workPluginModelsById.get(option.id) ??
                                                      option.models,
                                              }))
                                            : options;
