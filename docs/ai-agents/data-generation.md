...
The `DataRepository` manages a `config.yml` file with deep-merge semantics. The configuration schema (`IDataConfig`) includes:
...
- Comparisons track generation state in `config.yml` metadata to avoid regenerating pairs.
