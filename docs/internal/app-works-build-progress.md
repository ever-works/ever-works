...
**Refreshed 2026-09-18 (late) after T43, T63, T5 and the API boot fix all landed: present 1 531 of 2 667 paths · landed 212 task-path rows · landed tasks 137 of 699 (19.6 %).** Three things the refresh needed, all learned the hard way: the meter resolves `SPEC_ROOT` from the **current working directory**, so it must be run from the repository root or it exits 1 with `ENOENT … scandir`; it decides existence from **`git ls-files`**, so a slice is invisible to it until it is **committed** (T43 read as unlanded for exactly that reason and APW-09 moved 2 → 3 the moment it was committed); and the landed-task counts drift fast, so the table's numbers here are a dated measurement and not a live figure.
...
| APW-02 fork lifecycle         | ready | 18 (of 47)           | 14           | 29           | the entity/service/dispatcher/UI are in, and `markReady` now stamps `nextSyncAt` (`62f247cfa`); the setup-PR check (**T43's neighbour**, `checkSetupPullRequest`) is still absent |
...
| APW-05 builds                 | ready | 8 (of 100)           | 8            | 92           | the data layer (`6ae8c678f`) and **T7–T10, the build plugin** (`2606d90d7`), both landed; T13+ (the watch job) is next                                                   |
...
| APW-08 evolve loop            | ready | 2 (of 123)           | 1            | 121          | **P0 T1–T5 have all landed** (the last two at `4793cce11`); T6 is the ship gate — the rest of the epic is the evolve loop itself                                         |
