...
**Refreshed 2026-09-18 (late, after APW-05 T17 `f758b9f53` and APW-01 T12+T13 `5b838cb97`): present 1 568 of 2 667 paths · landed 218 task-path rows · landed tasks 142 of 699 (20.3 %; 22.7 % of the 959 paths tasks promise to create).** The two slices moved APW-01 from 3 landed paths to **7** and APW-05 from 9 to **10**, and `present` jumped 34 in one round. **Every number in the table below is now reproducible with one command**: `node docs/specs/features/app-works/tools/verify-task-paths.mjs` writes a `PER EPIC` section carrying _landed paths_, _landed tasks_ and _tasks_ per epic, which is exactly this table's columns — before that the landed-task count needed a throwaway script, and a table quoted in handovers must not depend on one. A task counts once when **any** path it marks new exists, so the count is a **surfaces** count and never an effort count. Three other things the refresh needed, all learned the hard way: the meter resolves `SPEC_ROOT` from the **current working directory**, so it must be run from the repository root or it exits 1 with `ENOENT … scandir`; it decides existence from **`git ls-files`**, so a slice is invisible to it until it is **committed** (T43 read as unlanded for exactly that reason, and an uncommitted slice by another agent is invisible too — which is correct, not a gap); and the counts drift fast, so the numbers here are a dated measurement and not a live figure. ⚠️ **One artefact to know about**: the meter derives an epic's `named` count as _absent + landed_, so a path that **exists but is not marked new** is counted in neither column and `named` can _fall_ as work lands — APW-01 went 70 → 59 and APW-05 99 → 82 in this very refresh while both epics gained landed surface. Read `landed` and `present` as the trend, not `named`.

| Epic                          | Spec  | Landed paths (named) | Landed tasks | Absent paths | Next / note                                                                                                                                                                                                                                                                                                                   |
| ----------------------------- | ----- | -------------------- | ------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| APW-01 app Work kind          | ready | 7 (of 59)            | 5            | 44           | T5's create contract, **T12's inspector and T13's create service landed** (`5b838cb97`) — an `app` create now forks/links/copies and dispatches readiness; what is missing is the **HTTP route** (`POST /api/works/app-source/inspect`, T17/T18, dispatched) — without it the end-to-end create is unproven at the HTTP layer |
...
| APW-05 builds                 | ready | 10 (of 82)           | 10           | 48           | the data layer (`6ae8c678f`), **T7–T10 the build plugin** (`2606d90d7`), **T41 the `checks` matrix job** (`ed2476826`) and **T17 `AppBuildsService` + the deployable verdict** (`f758b9f53`) landed; what is left on this path is T19's prepare job and T42, which needs it                                                   |
...
| APW-09 upstream pull requests | ready | 3 (of 91)            | 3            | 88           | T1/T2/T4 are in and **T43 (the credential of record) landed** (`7a8fd3c5a`), but its three Modify targets do not exist yet (see the log); T44 (budgets) and T45 (the fake's upstream routes) remain                                                                                                                           |
...
| APW-11 app launcher           | ready | 47 (of 72)           | 21           | 25           | the epic's UI and API are essentially done, T26 (the delegated-read CORS surface) included; what is left is mostly its own hardening tasks                                                                                                                                                                                    |
...
| APW-13 golden paths           | ready | 28 (of 100)          | 20           | 72           | **P0 is complete and its lanes pass locally**; **T63 landed the connection surface** (`978f67ca4`) and un-skipped T14's connected half — the five PR-lane specs are 21 passed / 4 skipped on a real stack; T30/T31's specs do not exist and the CI matrix is the outstanding proof                                            |
