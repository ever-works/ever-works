...
**Refreshed 2026-09-19 (after APW-01 T17/T18 `2763ee5f5`, APW-05 T19 `75a82b5fd`, C7 `3b3f2ba76`, T18 `65607cf22` and C8 `f6fadb7b2`): present 1 577 of 2 667 paths · landed 221 task-path rows · landed tasks 144 of 699 (20.6 %; 23.1 % of the 957 paths tasks promise to create).** APW-01 moved 7 → **9** landed paths and APW-05 10 → **11**. The `Absent paths` column is `named − landed` **by construction** — the meter derives an epic's `named` as _absent + landed_ — and the thirteen values sum to the report's own `absent: 1090`, which is the check that this column is arithmetic rather than a second measurement. **Every number in the table below is reproducible with one command**: `node docs/specs/features/app-works/tools/verify-task-paths.mjs` writes a `PER EPIC` section carrying _landed paths_, _landed tasks_ and _tasks_ per epic, which is exactly this table's columns — before that the landed-task count needed a throwaway script, and a table quoted in handovers must not depend on one. A task counts once when **any** path it marks new exists, so the count is a **surfaces** count and never an effort count. Three other things the refresh needed, all learned the hard way: the meter resolves `SPEC_ROOT` from the **current working directory**, so it must be run from the repository root or it exits 1 with `ENOENT … scandir`; it decides existence from **`git ls-files`**, so a slice is invisible to it until it is **committed** (T43 read as unlanded for exactly that reason, and an uncommitted slice by another agent is invisible too — which is correct, not a gap); and the counts drift fast, so the numbers here are a dated measurement and not a live figure. ⚠️ **One artefact to know about**: because `named` is _absent + landed_, a path that **exists but is not marked new** is counted in neither column and `named` can _fall_ as work lands — APW-01 went 70 → 59 → 58 and APW-05 99 → 82 → 77 across refreshes while both epics gained landed surface. Read `landed` and `present` as the trend, not `named`.
...
| APW-02 fork lifecycle         | ready | 18 (of 47)           | 14           | 29           | the entity/service/dispatcher/UI are in, `markReady` now stamps `nextSyncAt` (`62f247cfa`) and **T43 landed the setup pull request follow-through** (`8e1b29df9`) on the member's own credential; what is left is mostly APW-02's own remaining P1 tasks                                                                                          |
| APW-03 App spec + catalog     | ready | 13 (of 139)          | 11           | 126          | contracts, rules and the service are in; routes and the catalog service are the bulk of what is left                                                                                                                                                                                                                                              |
...
| APW-09 upstream pull requests | ready | 3 (of 91)            | 3            | 88           | T1/T2/T4 are in and **T43's service landed** (`7a8fd3c5a`) — but it fails closed with `handover_unavailable` because the record has no home: the credential column, its migration, the repository read/write and the `UPSTREAM_CREDENTIAL_STORE` binding are **dispatched this round**. T44 (budgets) and T45 (the fake's upstream routes) remain |
