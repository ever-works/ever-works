...
- **2026-09-18 · The branch's first CI verdict arrives: 15 of 16 jobs pass, and the one red is a formatting gate that predates the session.** `0cfe81a41`, 7 files (formatting only).
  ...
  **The other four are deliberately NOT formatted**: the generated artifacts under `_build-artifacts/` — a 3 897-line `app-spec.schema.json`, two `expected-outputs/golden/*.json` files and `templates-manifest.schema.json`. Prettier rewrites them wholesale and the goldens are plausibly byte-compared by the golden-outputs work, so **trading a lint red for a test red by guesswork is not a fix** — left to their owner with the test in front of them.
