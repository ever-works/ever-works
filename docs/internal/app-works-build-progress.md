...
- **CI run `35372715843` got worse as the fleet drained, and the boot defect is NOT the reason** — checked rather than assumed. By 21:39Z it read **7 failed shards** (`15, 14, 24, 30, 27, 26, 9`) against 2 successes and 24 still queued, all of them failing at wildly different times across a four-hour window. The tempting explanation was that the run's tree carries the `LauncherDelegatedCorsMiddleware` boot defect: `git merge-base --is-ancestor 83d36d60d ea44b4593` exits **1** and the middleware file **does not exist** at that SHA, so that tree predates it and the API booted fine. **Their logs are unavailable while the run is in progress** (`gh run view --job … --log-failed` answers "still in progress"), so the seven are *unexplained* rather than cleared — the one that could be reproduced locally (shard 15's `flow-onboarding-wizard-catalog-work-chain`, above) was green in 33 s. A **fresh run `35385453583` pinned to the current tip** was dispatched so the matrix is eventually exercised on the code this ledger describes; until one of them completes, "the CI matrix passes" remains unproven, which is the honest state.
...
- **`apps/web/e2e/COVERAGE.md:702-711` is stale** — it still describes the T63 fixmes that are now removed. T19's file, left untouched.
