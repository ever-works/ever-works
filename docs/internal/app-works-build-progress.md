...
| C7  | `apps/api/src/trigger/trigger-internal.controller.ts:498-660` (the `remoteMap`) | **APW-05 T19's `app-build-prepare` job cannot execute as landed.** The task resolves `AppBuildPrepareRunner` over the internal RPC channel — correctly, because a Trigger worker owns no `DataSource` — but the API half of the pair does not exist: the runner is absent from the `remoteMap`, and `@ever-works/agent` publishes no `./app-builds` exports entry at all, so `trigger-internal.module.ts` cannot even name the provider. The failure is **named and visible rather than silent** (the proxy's call rejects and the run reports `status: 'failed'`, `reason: 'runnerUnavailable'`), and the fallback that keeps the _behaviour_ honest is real: with `APP_BUILD_PREPARE_DISPATCHER` unbound, `AppBuildsService.dispatchPrepare` takes §7.1's in-process fallback and runs the runner where the `DataSource` is. So nothing is broken today — what is missing is the **queued** path. | Not fixed inside T19: wiring it means the `./app-builds` package export, `AppBuildsModule` in the API's trigger-internal module, and a `remoteMap` entry — three files outside that task's list, and a module change that owes an API boot. Routed with the shape of the fix written down; **whoever lands T18 (the dispatcher) should land this with it**, because binding the dispatcher without the proxy turns a documented fallback into a job that fails at the first dispatch.                                                |
...
- **The CI shard failures are an API OOM at startup, NOT fleet load — and the mechanism was finally read out of the logs rather than inferred.** The nine failures of run `35372715843` were recorded here as "unexplained" while the run was in progress, because `gh run view --job … --log-failed` refuses to answer until it leaves `in_progress`. **Cancelling the run made its logs readable** (and freed the `concurrency` group that was blocking the current-tip run, which moved `pending → queued` the moment it settled), and the first failed shard answers it in one block:
  ...
  **So C3's earlier "read it as fleet load" verdict was wrong and is corrected here**, and the boot defect was already ruled out by ancestry. But **the fix I first wrote here was wrong too, and checking is what caught it**: I claimed the remedy was to "raise the shard API's heap or stop compiling the route table under a watcher in the lane". Both are **already done**, and were done _before_ that run — `git show ea44b4593:.github/workflows/e2e.yml` has **4** `start:prod` occurrences (identical to HEAD) and **2** `max-old-space-size` settings, and the step's own comment says the lane boots a **prebuilt dist** "NOT `nest start -b swc --watch`" precisely to avoid that typechecker-plus-watcher pressure. So the shard OOM'd **with the mitigation in place**, which means the mitigation is not the whole story and the mechanism is still open. Two candidates, and neither is verified: the heap **cap** may be set above what the shard runner actually has (`--max-old-space-size=16384` on an 8-vCPU runner makes V8 stop collecting before the OS kills it — the classic over-commit OOM, which would mean the fix is to **lower** it, not raise it), or some _other_ process in the shard still pays that cost. **Do not act on a heap number here without measuring the runner's real memory first** — this entry records a measured symptom and two untested hypotheses, not a diagnosis.
