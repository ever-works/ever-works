...
- A **[Work](/features/creating-a-work)** is the buildable, self-maintaining unit. The kinds you pick from the chip catalog at creation time are **website**, **landing page**, **blog**, **directory**, **awesome repo** and **repository** — the last registers an _existing_ code repository as a Work so Tasks, Goals and fleet runs can attach to it, instead of generating a site (it is not the [Repositories](/features/repositories) connection registry in settings). Two further kinds exist but are minted by dedicated flows instead of the create screen: **company** (the Register-Company flow) and **campaign** (go-to-market template activation). A **store** kind has not shipped yet — see [Store Builder](/features/store-builder). Full matrix: [Work Kinds & Capabilities](./features/work-kinds.md).
