...
Directory Import lets you bootstrap a new directory from an existing source rather than starting from scratch. Instead of manually creating items one by one or running a generation from a blank prompt, you point the platform at a repository and it does the rest — cloning data, analyzing structure, enriching content with AI, and setting up the full directory ecosystem.
...
**Type detection.** The analyzer inspects the root directory contents of the repository:

- If the repo contains a `config.yml` (or `config.yaml`) **and** a `data/` directory, it's classified as a **data repository** — an existing Ever Works-format repo with structured items.
- If the repo contains a `README.md` that has section headers and multiple categorized links (at least 5 list-style links, or 3+ internal directory links for multi-file awesome lists), it's classified as an **Awesome README**.
- If neither pattern matches, the type is `null` and the import cannot proceed.

**Structure inspection.** For data repos, the analyzer counts items (subdirectories inside `data/`) and categories (entries in `categories.yml`). For Awesome READMEs, it estimates item count from list links and category count from section headers. This gives the user a preview of what the import will contain.
...
Once analysis is complete, the platform creates a new `Directory` entity in the database. This records the directory name, slug, owner, git provider, and deployment provider. The directory's generation status is set to "generating" and a generation history entry is created to track progress.

For Awesome README imports, the platform also stores the source repository metadata (URL, owner, repo name, import type) on the directory entity. This metadata is used later for sync operations.
...
2. **Read the data.** A `DataRepository` abstraction reads the cloned filesystem. It extracts:
    - **Items** — each subdirectory inside `data/` represents one item, with a YAML metadata file and an optional markdown description.
    - **Categories** — from `categories.yml` at the root.
    - **Tags** — from `tags.yml` at the root.
    - **Config** — from `config.yml`, including directory-level settings and metadata.

3. **Initialize the target.** All extracted data is passed to the data generator, which writes it into the new directory's own data repository. Import metadata (source owner/repo, timestamp, import type) is embedded in the config.
...
The result is an exact copy of the source directory with its own independent repositories.
...
:::info
The Awesome README import delegates entirely to the pipeline plugin — it does not parse the README directly. The pipeline treats the source URL as research input and builds a significantly larger directory from it.
:::
...
3. **Run the pipeline.** The configured generation DTO is passed to the data generator, which delegates to the active pipeline plugin. The pipeline runs autonomously — fetching the source, researching items, discovering new ones, and building the full directory.

4. **Generate companion repos.** On success, the markdown and website generators are initialized, just like the data repo import.

The result is a directory that goes far beyond the original awesome list — with original descriptions, broader coverage, and an independent taxonomy.

**Expansion factor.** The expansion factor controls how aggressively the AI should discover items beyond the source. At the default of 2.5x, the source represents at most 40% of the final directory. This means a source with 100 items should produce a directory with roughly 250 items.

#### Link Existing Import

The link existing import works fundamentally differently from the other two types. Rather than cloning data or generating new content, it **references an existing data repository** that the user already owns and manages. The platform creates a directory entry that points to it, with no data copied or moved.
...
3. **Status tracking.** The directory's generation status progresses through states: `generating` while the import runs, then `generated` on success or `error` on failure. The generation history entry records start time, finish time, duration, and item counts.

4. **Completion event.** When the import finishes (success or failure), the platform emits a `DirectoryGenerationCompletedEvent` that triggers downstream actions like notifications.

### Phase 5 — Post-Import

After a successful import, the directory is fully operational:
...
```
config.yml              # Directory configuration (name, description, settings)
...
The minimum requirement is `config.yml` (or `config.yaml`) and a `data/` directory with at least one item.
...
Alternatively, a **multi-file awesome list** is detected when the README contains 3 or more links to internal directories (`[Category](./subfolder/)`).
...
| Method | Endpoint                          |
| ------ | --------------------------------- |
| `POST` | `/api/directories/import/analyze` |
...
| Method | Endpoint                                      |
| ------ | --------------------------------------------- |
| `POST` | `/api/directories/import/analyze-for-linking` |
...
| `name`               | string  | Yes      | Directory name (max 100 characters)                                              |
...
Returns `202 Accepted` for `data_repo`/`awesome_readme` (async), `200 OK` for `link_existing` (sync). The response includes `directoryId` and `historyId` for tracking progress.
...
| Method | Endpoint                               |
| ------ | -------------------------------------- |
| `GET`  | `/api/directories/import/repositories` |
