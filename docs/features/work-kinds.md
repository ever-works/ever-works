...
- **App Works are early.** The kind, its capability entry, its snapshot of the `repo` URL rules and its fail-closed chip ship first; the create form, the create API and the runtime surfaces (Builds, App environment, Deployments) land with the App Work epics. Until they do, the kind is in the vocabulary and in this registry but has no chip to reach it — which is exactly what the fail-closed flag is for.
