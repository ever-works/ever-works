...
| `app`          | App             | **App** chip on `/new` or `/works/new` → `/works/new?mode=manual&kind=app`, which opens the App form (the chip ships with that form; the kind, its capabilities and its API land first). The chip is **fail-closed** — see the feature-flag note below | An existing GitHub repository the platform runs: it is linked, forked or copied into your account, and that copy **is** the Work Repository which Builds and Deployments target. Unlike `repo`, it deploys — and unlike every generated kind, its code is not generated for you. |
...
| Capability                                              | `directory` / `default` | `awesome-repo`  | `blog`          | `website`       | `landing-page` | `repo` | `app` | `company` | `campaign` |
| ------------------------------------------------------- | ----------------------- | --------------- | --------------- | --------------- | -------------- | ------ | ----- | --------- | ---------- |
| Items surface (tab, `/items` routes, submissions)       | Yes — **Items**         | Yes — **Items** | Yes — **Posts** | Yes — **Pages** | No             | No     | No    | No        | No         |
| Taxonomy (categories, tags, collections + tiles)        | Yes                     | Yes             | Yes             | No              | No             | No     | No    | No        | No         |
| Comparisons (generator sub-tab + endpoints)             | Yes                     | No              | No              | No              | No             | No     | No    | No        | No         |
| Community pull-request intake                           | Yes                     | Yes             | No              | No              | No             | No     | No    | No        | No         |
| CSV / Excel item import & export                        | Yes                     | Yes             | No              | No              | No             | No     | No    | No        | No         |
| Source-URL validation for items                         | Yes                     | Yes             | No              | No              | No             | No     | No    | No        | No         |
| Deploy (tab + deployment endpoints)                     | Yes                     | Yes             | Yes             | Yes             | Yes            | No     | Yes   | No        | No         |
| Builds (the build capability, APW-05)                   | No                      | No              | No              | No              | No             | No     | Yes   | No        | No         |
| App environment + dependencies (APW-07)                 | No                      | No              | No              | No              | No             | No     | Yes   | No        | No         |
| Knowledge base (the **Memory** tab)                     | Yes                     | Yes             | Yes             | Yes             | Yes            | Yes    | Yes   | Yes       | Yes        |
| Data repository (`{slug}-data`)                         | Yes                     | Yes             | Yes             | Yes             | Yes            | Yes    | No    | Yes       | Yes        |
| Provider repository (`{slug}`, the browsable markdown)  | Yes                     | Yes             | Yes             | Yes             | Yes            | No     | No    | Yes       | Yes        |
| Work repository (`{slug}-website`, the template output) | Yes                     | Yes             | Yes             | Yes             | Yes            | No     | Yes   | No        | No         |
...
`app` is the mirror image. It provisions the **Work Repository** role and never the `data` role: the repository you link, fork or copy holds the app code, which is what Tasks, Builds and Deployments target. It is the one kind whose Work Repository is not template output — nothing is generated into it, the code was already there — so no capability that depends on generated content (items, taxonomy, comparisons, community PRs, item import/export, source validation) applies, the knowledge base stays on, and the Deploy, Builds and App environment surfaces are the point of the kind.
...
| `app`                   | Agents · Open Tasks · Deploy Status · Days Active                                |
...
- **App Works** show the strip **without Items** — their Work Repository is the app code you linked, forked or copied, not generated content — and keep **Deploy**. The **Builds** and **App environment** surfaces are the kind's own additions and arrive with the App runtime (APW-05 and APW-07); the Memory tab stays, because the app's runbook and decisions live with the Work.
- The **{provider} Repository** opt-out card in **Settings** only renders when the kind provisions that repository (`repos.work`), which today is every kind except `repo` and `app`.
...
- **App Works are early.** The kind, its capability entry, its snapshot of the `repo` URL rules and the fail-closed `works-app` flag logic exist, but the dashboard's **App** chip and create form have not shipped: turning App Works on does not add a chip to `/works/new` yet. Behind `EVER_WORKS_APP_WORKS_ENABLED` (off by default), the API's repository inspection (`POST /api/works/app-source/inspect`) and App create (`POST /api/works` with `kind: "app"`) have landed, and in the dashboard so have an App Work's **Upstream** tab and its **Settings → App spec** page. Builds, the App environment screens and completed Deployments have not. [App Works](./app-works.md) describes the preview part by part — what works today and what does not.
