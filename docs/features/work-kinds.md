...
- **App Works are early.** The kind, its capability entry, its snapshot of the `repo` URL rules and its fail-closed chip ship first; on the API, the repository inspection (`POST /api/works/app-source/inspect`) and the App create (`POST /api/works` with `kind: "app"`) have landed behind `EVER_WORKS_APP_WORKS_ENABLED` (off by default). The dashboard's create form and the runtime surfaces (Builds, App environment, Deployments) still land with the App Work epics, which is exactly what the fail-closed chip is for: it stays hidden until an operator turns App Works on.
