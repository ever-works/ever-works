---
id: creating-a-directory
title: Creating a Directory
sidebar_label: Creating a Directory
sidebar_position: 1
---

# Creating a Directory

When you create a new directory, the platform presents three creation methods. Each method leads to a fully functional directory backed by git repositories, but they differ in how the initial content is produced and how much control you have over the process.

## Creation Methods

```mermaid
flowchart TD
    A[New Directory page] --> B{Choose method}
    ...
    F --> G[AI generates directory]
    D --> H[Empty directory created]
...
| **Manual**      | Setting up structure first, adding content later | No                     | No — creates an empty directory scaffold         | None                                                       |
...
- **Directory name** (required) — the title of your directory (e.g., "Best React Component Libraries").
- **Prompt** (required) — instructions telling the AI what kind of directory to build, what to include, and how to organize it. The prompt is the most important input — it guides the entire generation process.

The form offers example prompts to help you get started (e.g., "Create a comprehensive directory of...").
...
| **Claude Code**       | Delegates to Claude Code CLI running in an isolated workspace. Uses Claude's code-generation capabilities to research and build the directory.                                                                                                                            | Advanced use cases where you want Claude's full reasoning capabilities applied to directory construction.           |
...
The screenshot provider captures website screenshots for items that have URLs. These screenshots appear as visual previews on item cards in the directory. Options include ScreenshotOne and Urlbox.
...
1. The platform uses the selected AI provider to generate directory details from your name and prompt: a URL-friendly slug, a description, and seed keywords.
2. A new directory is created with the generated metadata.
...
4. The generation is dispatched as a background task. The selected pipeline plugin takes over and runs autonomously — discovering items, enriching content, and building the directory.
5. You're redirected to the directory detail page where you can watch progress in real time.

## Manual Creation

Manual creation is the simplest method. It creates an empty directory scaffold — no AI, no generation, no items. You fill in the metadata yourself and add content later.

### What You Provide

- **Name** (required) — the directory title.
...
- **Description** (required) — a short description of the directory (max 500 characters).
- **Repository owner** — your personal git account or an organization you belong to. Requires a connected git provider.

### What Happens After Submission

1. The directory is created in the database with the provided metadata.
2. Git repositories are created under the selected owner.
3. You're redirected to the directory detail page.
4. **No items are generated.** The directory is empty — you can add items manually, trigger an AI generation from the detail page, or populate it via the API.

Manual creation is useful when you want full control over the directory structure, when you plan to import items through the API, or when you're setting up a directory that will be populated by community contributions.

## Import

Import bootstraps a directory from an existing git repository. It supports three source types, each with a different workflow.

For a detailed explanation of the import system — including under-the-hood mechanics, the analysis phase, ecosystem detection, and background processing — see [Directory Import](./directory-import).
...
- **Data Repository** — an existing Ever Works-format repo with `config.yml` and `data/` directory. Items, categories, and tags are copied directly.
- **Awesome README** — a curated list with categorized links (like GitHub Awesome Lists). The AI pipeline processes the source as research seeds and builds a much larger directory.
...
All three creation methods require a **connected git provider** (GitHub, GitLab, or Bitbucket). The git provider stores the directory's repositories. You select it in the sidebar before choosing a creation method, and can connect via OAuth if not already linked.
...
Optionally, you can select a **deploy provider** (e.g., Vercel) in the sidebar. This determines where the directory's website will be deployed after generation. If no deploy provider is selected, the website repository is still created but not deployed.
...
2. **Directory-level defaults** — if you're regenerating an existing directory, the providers from the last generation are pre-selected.
...
- [Directory Import](./directory-import) — Detailed import system documentation
