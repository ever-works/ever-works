...
`works.yml` is a single YAML file, committed to a directory's **data repository**, that captures the directory's generation configuration in source-controlled form. The platform reads it at import time to bootstrap a directory's settings, and writes it back after each generation to keep it in sync.

This makes a directory's configuration:
...
Use `works.yml` when you want to onboard an existing data repo into the platform without manually re-entering its settings, or when you maintain directories outside the platform UI (e.g. via PRs, CI scripts, or config templates).
:::
...
initial_prompt: |
    A curated directory of open-source AI tools across LLM,
...
| ---------------- | ---------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`           | string           | `title`                                                  | Display name for the directory.                                                                                                                 |
...
When you import an existing directory data repo, the platform:
...
When you confirm the import, the platform persists the parsed `works.yml` settings to the directory entity AND keeps the imported file in the data repo for future round-trips.
...
- Writes back the directory's current `name`, `initial_prompt`, `model`, `website_repo`, `providers`, and `schedule`.
...
1. **Re-import** the directory — the new values become the directory config.
...
- [Directory Import](./directory-import) — bootstrapping a directory from an existing repo
