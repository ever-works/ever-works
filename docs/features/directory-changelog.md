---
id: directory-changelog
title: Directory Changelog
sidebar_label: Directory Changelog
sidebar_position: 3
---

# Directory Changelog

Directory Changelog gives each directory an audit trail of what changed over time. Instead of only showing aggregate generation metrics, the History tab now records item-level and taxonomy-level activity so directory owners can review specific changes after generation runs, manual edits, comparison actions, and community PR processing.

:::tip When to use this
Use Directory Changelog when you need visibility into how a directory evolves over time, especially if multiple people or automated jobs are updating content.
:::

## What It Tracks

Each changelog entry is stored alongside directory history and includes an `activityType` plus structured change details.
...
1. **A directory mutation happens** — generation completes, an item is edited, a taxonomy entity changes, a comparison is created, or a community PR is processed.
2. **The platform records a history entry** in `DirectoryGenerationHistory`.
...
```bash
curl "http://localhost:3100/api/directories/<directory-id>/history?limit=10&offset=0&activityType=taxonomy" \
  -H "Authorization: Bearer <token>"
```

## Why It Matters

Directory Changelog improves trust and reviewability:
...
- **Community-driven directories** can track what was merged from contributor PRs
