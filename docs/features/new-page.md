...
| **Chip row**         | Rendered below the box: **Mission · Idea · Agent · Task · Website · Landing Page · Blog · Directory · Awesome Repo · Repository · Company**, then an inert **Store** chip marked **Soon**. Picking a chip writes that chip's first example prompt into the box so you have real text to edit. |
...
| **Repository**   | An existing code repository registered as a Work — Tasks, Goals and fleet runs attach to it; nothing is generated or deployed. | `/works/new?mode=manual&kind=repo` (plus `&prompt=<canonical repo URL>` when your text is one) |
...
Six user-selectable Work kinds exist (`website`, `landing-page`, `blog`, `directory`, `awesome-repo`, `repo`); the first five generate a site, `repo` registers one you already have. Two further kinds exist on persisted Works but are never offered as chips: `company` Works are minted only through the Register Company flow, and `campaign` Works only through template activation.
...
flowchart LR
    ...
    C -->|Repository| RP["No chat turn — canonical URL only"] --> RP2["/works/new?mode=manual&kind=repo"]
...
For every chip except **Company** and **Repository**, submit does two things at once:
...
The Repository chip registers a code repository you already have, so there is nothing to generate and nothing for the chat to iterate on: your prompt is **not** sent into the chat. You land on `/works/new?mode=manual&kind=repo` with the Repository form open.

Only text that reduces to `owner/repo` — a `github.com/<owner>/<repo>` URL — is forwarded, and it is forwarded in canonical form as `&prompt=https://github.com/<owner>/<repo>`. Anything else is dropped and you type it into the form field instead, so a token embedded in a pasted remote (`https://user:token@github.com/o/r`) never reaches the URL bar, browser history, the referrer of later requests, or a proxy log.
...
| `/new?type=idea` … `?type=repo`   | Any other live chip value pre-selects that chip — `?type=repo` (the **Repository** chip) included.                                           | Deep links you build yourself                                                                          |
...
Every chip — the eleven live ones plus Store — is gated by a feature flag named `works-<chip>` (`works-website`, `works-landing-page`, `works-blog`, `works-directory`, `works-awesome-repo`, `works-repo`, `works-store`, `works-company`, and so on). Flags are evaluated **server-side** through PostHog, so no analytics token ever reaches the browser; only the resulting set of disabled kinds is sent to the page.
...
| **Repository**                                               | No chat panel — nothing is generated. The Repository form opens on `/works/new`, with the repository URL pre-filled when what you typed was a canonical `github.com/<owner>/<repo>` URL.                                                   |
...
- [Creating a Work](./creating-a-work.md) — the `/works/new` form the six Work chips hand off to, including provider selection and the Manual / Import paths.
