...
- **Who did it.** A line names the actor as it was when the entry was written, so renaming or deleting an agent later does not change its past lines. Something you did to an agent — exporting or importing it, for example — is your line, not the agent's; it still shows up when you filter by that agent and still opens it.
