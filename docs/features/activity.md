...
- **Who did it.** A line names the actor as it was when the entry was written, so renaming or deleting an agent later does not change its past lines. That holds for entries that saved the actor's name when they were written, as new entries normally do. Entries from before Live Feed saved no name: they show the agent's current name, so renaming the agent changes them too, and once the agent is deleted they read "A deleted agent". Something you did to an agent — exporting or importing it, for example — is your line, not the agent's; it still shows up when you filter by that agent and still opens it.
- **Refreshing.** The view checks for new entries every 10 seconds while the tab is in the foreground. When more arrived than fits one page, it loads everything between the newest entry and what you already have, so nothing is skipped; after a very large burst it starts the list over from the newest entry instead, and **Load older** reaches the rest.
