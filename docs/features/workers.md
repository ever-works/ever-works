...
| **Long-running plugin calls**       | Execute plugin operations routed as `long-running` (an explicit profile on the call, or the manifest).  |
...
| `model-account-health`           | Every 6 hours, at :19             | Checks each enabled provider account's credential; marks accounts expiring, expired or rejected. Never changes their order.                    |
