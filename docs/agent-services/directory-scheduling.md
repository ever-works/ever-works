---
id: directory-scheduling
title: Directory Scheduling Service
sidebar_label: Directory Scheduling
sidebar_position: 3
---

# Directory Scheduling Service
...
Scheduled updates allow directories to automatically refresh their content at configurable intervals. The system integrates with the subscription/billing layer to enforce plan limits and supports both subscription-based and pay-per-use billing modes.
...
```typescript
const { schedule, directoryId } = await scheduleService.getSchedule(directoryId, user);
```

Returns a `DirectoryScheduleDto` containing current state, allowed cadences, and plan information. Any access level can view schedules.
...
2. **Configuration readiness** -- Ensures the directory has completed initial setup (has `last_request_data`). Sync directories (with `sourceRepository`) skip this check.
3. **Plan limit enforcement** -- Checks `activeScheduleCount` against `plan.maxDirectories`.
...
```typescript
await scheduleService.cancelSchedule(directoryId, user);
```
...
The `DirectoryScheduleDispatcherService.dispatchDue()` method is the entry point, typically called from a cron job:
...
4. **Execute generation** -- Calls `directoryGenerationService.runScheduledUpdate()`.
...
```typescript
calculateNextRun(cadence: DirectoryScheduleCadence, delayMinutes = 0, fromDate = new Date()): Date
```
...
1. **Plan limits** -- If `activeScheduleCount > plan.maxDirectories`, the schedule is paused.
...
Schedule state changes are synced back to the directory entity via `syncDirectory()`:
