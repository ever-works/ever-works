---
id: work-generation
title: Work Generation Service
sidebar_label: Work Generation
sidebar_position: 2
---
...
3. **History record** -- Creates a `WorkGenerationHistory` entry with status `GENERATING`.
...
1. Sets work status to `GENERATING`.
...
1. Immediately sets work status to `GENERATING` for instant UI feedback.
2. Builds a `WorkGenerationPayload` and dispatches via `generationDispatcher`.
...
2. For works with a `sourceRepository`, delegates to `runScheduledSync()` which performs an import sync.
3. For standard works, calls `updateItemsGenerator()` with schedule-specific overrides.
...
2. Sets work status to `GENERATING` with step `syncing`.
3. Calls `workImportService.syncWork()`.
4. On success/failure, updates both the schedule and work status.
