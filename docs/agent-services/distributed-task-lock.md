...
Today the service is hard-wired to the `cache_entries` PostgreSQL table. That is the **default and fully-supported backend** and will remain so. We do not plan to drop it.

For high-scale hosted deployments, we additionally want to support **Redis** as an optional backend that can be selected per deployment via environment configuration — without forking, without changing caller code, and without removing the PostgreSQL option.

Shape of the planned change (tracked in [EW-629](https://evertech.atlassian.net/browse/EW-629), design rationale in [ADR-005](../specs/decisions/005-cache-and-lock-pluggability.md)):

- A `LockProvider` interface that exposes `acquire / refresh / release / peek`.
- `PostgresLockProvider` — the current implementation, factored out behind the interface. **Default**.
- `RedisLockProvider` — uses `SET key token NX PX ttl` for acquisition and a Lua script for token-bound DELETE.
- Selection via `EVER_WORKS_LOCK_BACKEND={postgres,redis}`. Default: `postgres`.
