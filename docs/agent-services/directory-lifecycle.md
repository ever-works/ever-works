...
title: Directory Lifecycle Service
sidebar_label: Directory Lifecycle
sidebar_position: 1
---

# Directory Lifecycle Service

The `DirectoryLifecycleService` manages the complete lifecycle of a directory entity -- from creation through updates and synchronization to deletion. It is the primary entry point for all CRUD operations on directories within the agent package.

**Source:** `packages/agent/src/services/directory-lifecycle.service.ts`
...
- **DirectoryRepository** -- Database access for directory CRUD operations.
...
- **DirectoryOwnershipService** -- Enforces role-based access checks.
- **DeployFacadeService** -- Validates deploy providers (e.g., Vercel).

## Creating a Directory

The `createDirectory` method accepts a `CreateDirectoryDto` and a `User` context. It performs the following steps:

1. Extracts fields from the DTO: `slug`, `name`, `description`, `owner`, `readmeConfig`, `organization`, `gitProvider`, `deployProvider`.
2. Persists the directory record via `directoryRepository.create()`.
...
5. Returns a success response with the created directory entity.
...
When a directory is freshly created, its `generateStatus` is `null` unless pre-existing items are detected in the data repository, in which case it is set to `GENERATED`.
...
The `syncFromDataRepository` method pulls a snapshot from the data generator and updates the directory record accordingly:
...
3. Syncs pull request metadata from the snapshot if the directory lacks it.
4. Syncs `readmeConfig` (header/footer) from markdown templates when not already configured.
5. Persists all accumulated updates in a single `directoryRepository.update()` call.
...
3. Removes the directory record from the database.
...
5. Returns a `DeleteDirectoryResponseDto` with the list of deleted repositories.
