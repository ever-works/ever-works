    ...
    async execute(
        ...
        if (!execContext) {
            return this.buildErrorResult(
                new Error('Execution context is required'),
                startTime
            );
        }
...
export class OrchestratedPipelinePlugin
    implements IPipelinePlugin<MyStepId>
{
    ...
    async execute(
        ...
        throw new Error(
            'This pipeline is engine-orchestrated. '
    ...
    async healthCheck(): Promise<PluginHealthCheck> {
        return {
            status: this.stepExecutors.size === OrchestratedPipelinePlugin.STEPS.length
                ? 'healthy' : 'degraded',
...
export class SentimentAnalysisStep extends BasePipelineStep {
    ...
    readonly stepPosition: StepPosition = BasePipelineStep.after(
    ...
    async execute(
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const item = items[i];
            sentimentScores[item.name] = await this.analyzeSentiment(
                item.description ?? ''
            );
