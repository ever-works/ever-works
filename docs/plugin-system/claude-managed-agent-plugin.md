...
In the directory edit screen, set **Generation Pipeline** to `Claude Managed Agent`. The plugin will only appear in the dropdown after it has been configured (it requires an Anthropic API key).

## Configuration

The plugin uses Anthropic's standard API authentication. The exact settings schema is defined in `packages/plugins/claude-managed-agent/src/` and is rendered automatically in the directory settings UI; consult that source file for the up-to-date list of fields, defaults, and validation rules.
