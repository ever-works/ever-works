...
| **Memory**        | `/works/:id/kb`            | The Knowledge Base workbench — a document tree, upload zone, classification, a markdown editor with wikilinks and mentions, history, locks and citations, plus a **Cmd/Ctrl+K** search palette and a side AI panel. There is no create-document button: uploading and the chat rail are the two browser paths (see [Knowledge Base and Memory](./knowledge-base-and-memory.md#how-to-create-a-document-without-uploading-a-file)). |
