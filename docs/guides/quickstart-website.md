...
2. Create your first document. The workbench has no **+ New document** button yet — use the chat rail on any page of the Work (_“create a KB document at `brand/voice` in the brand class”_), `kb.create` over MCP, or `POST /api/works/:id/kb/documents`; uploading a `.md` file (step 3) creates one as well. However it is created, the document opens in the markdown editor (WYSIWYG or raw markdown), and metadata — class, tags, status, lock — lives in the front matter or the metadata side panel. See [How to: create a document without uploading a file](knowledge-base-and-memory.md#how-to-create-a-document-without-uploading-a-file).
