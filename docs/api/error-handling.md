...
{
	"statusCode": 400,
	"message": ["url must be a URL address", "viewportWidth must not be greater than 3840"],
	"error": "Bad Request"
}
```
...
// Service unavailable
throw new HttpException('Failed to communicate with Twenty CRM', HttpStatus.SERVICE_UNAVAILABLE);
```
...
return next.handle().pipe(
	catchError((err) => {
		...
		this.logger.error(`Error Response: ${method} ${originalUrl} ${statusCode || 400} - ${delay}ms`);
