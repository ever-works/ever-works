...
- **Items** — catalog entries (directory rows, blog posts, …). The thing a Work _outputs_. Stored under `<dataRepo>/items/<slug>.json` (or `.md` for content-heavy items). Generated by the standard pipeline.
...
An Idea may have its own Tasks (for validity checks, market research, prototyping). These stay on the Idea — they do not auto-forward to Works derived from the Idea (see [features/task-tracking/spec.md §5.7](../features/task-tracking/spec.md)).

Full distinction table: [features/task-tracking/spec.md §1.1](../features/task-tracking/spec.md).
