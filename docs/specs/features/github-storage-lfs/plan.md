...
| Existing `STORAGE_BACKEND=github-storage` deployments behave differently after upgrade because LFS now defaults to `true`.                                           | Migration rule §8 in spec: if no `mode` and no `lfsEnabled` keys exist in the settings record AND the GH_STORAGE_* env vars are set, `lfsEnabled` resolves to `false`. Codified in unit tests.            |
