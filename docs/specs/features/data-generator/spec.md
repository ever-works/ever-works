...
DataGeneratorService.initialize(payload)
    ...
    │
    ├── Read existing .works/works.yml, items/, categories.yml, tags.yml
    │
    ...
    │
    ├── Write to repository
    │   ├── Update .works/works.yml (increment version)
...
The `version` field in .works/works.yml auto-increments on each update:
