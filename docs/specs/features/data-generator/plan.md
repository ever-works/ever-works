...
| File format — config | YAML (`js-yaml`)                                 | Human-readable, edits-friendly, the format directories grew up with      |
...
| Commit strategy      | Direct push **or** PR per directory setting      | PR mode supports review-driven directories; direct mode for single-owner |
...
├── config.yml          # DirectoryConfig (name, slug, version, metadata)
...
The data generator is invoked from inside the directory-generation
trigger task; it has no public HTTP surface. The directory-level
endpoints `POST /api/directories/:id/generate` and
`POST /api/directories/:id/regenerate` (see `data-management` spec)
...
- Web: the directory generation trigger lives on the directory detail
  page; results (stats, PR link) surface in the generation history
  panel and the recent-logs stream.
- CLI: `everworks generate <directoryId>` triggers the same flow
  through the public CLI.

## 7. Background Jobs

The full generation runs inside the Trigger.dev `directory-generation`
...
- Authorization is enforced upstream in
  `DirectoryOwnershipService.ensureAccess` before the trigger task
...
- `GenerationLogCollector` streams structured logs back to the
  `directory_generation_history.logs` JSONB column and a recent-logs
  ring buffer.
- Activity log emits `directory_generation_started` /
  `directory_generation_completed` with the resolved stats.
...
| Risk                                                               | Mitigation                                                                            |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| Concurrent runs on the same directory clobber each other's commits | Schedule dispatcher claims runs via CAS; manual triggers respect `GENERATING` status  |
...
| GitHub rate limits during many-directories generation              | Token rotation + per-user clone cache + retry-with-backoff in `GitFacade`             |
