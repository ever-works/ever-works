...
Long-running directory generations can be cancelled mid-flight from the web
...
- **Given** a directory is currently generating and I have edit rights,
  **when** I click "Cancel" on the directory page, **then** the generation
  stops shortly afterwards and the directory's status becomes `cancelled`.
- **Given** I cancel a generation, **when** I open the activity log,
  **then** I see an entry "Generation cancelled for `<directory>`" marked
...
- **Given** the directory is not currently generating, **when** I call the
  cancel endpoint, **then** I get `409 Conflict` and the directory state
  is unchanged.
- **Given** the directory's generation completed between my cancel click
  and the API call, **when** the cancel runs, **then** the response
  reports `mode: already_finished` and no state changes.
- **Given** the directory was flagged as generating but no actual run is
...
- **Given** I do not have edit rights on the directory, **when** I attempt
...
- **FR-7** The system MUST write an activity-log entry "Generation cancelled
  for `<directory>`" with status `COMPLETED`, not `FAILED`.
...
- **FR-9** When a cancel is routed in-process, the system MUST signal an
  ...
  steps.
- **FR-10** A cancelled directory MUST be in a clean, retriable state — the
...
- **Security & privacy**: cancel requires edit rights; ownership-checked
  against `DirectoryOwnershipService`.
...
- [x] `POST /api/directories/:id/cancel-generation` returns `202` for an
      in-progress run.
- [x] All four cancellation modes are tested.
- [x] Cancelled run produces an activity-log entry with `COMPLETED` status.
- [x] Web dashboard exposes a Cancel control on directories that are
      actively generating.
- [x] `409` is returned for non-generating directories.
...
- [x] **VI — Tests**: covered in `directory-generation.service.spec.ts` and
...
- Implementation:
    - `apps/api/src/directories/directories.controller.ts:516` (endpoint)
