# Feature Specification: Directory Changelog
...
The directory changelog turns each directory's History tab from a metrics
...
community PR processing — produces a `DirectoryGenerationHistory` entry
...
- **FR-1** Every mutation that changes a directory's content MUST
  produce a `DirectoryGenerationHistory` row with an `activityType`.
...
- **Performance**: a History page (default 20 entries) returns in
  ≤ 200 ms for a directory with 100 K history rows.
...
- **Security & privacy**: history requires viewer access on the
  directory; secret fields are never written to changelog entries.
...
| ---------------------------- | ----------------------------------------------------------------------------------- |
| `DirectoryGenerationHistory` | Per-directory log row with status, duration, metrics, activityType, changelog       |
| `DirectoryChangelog`         | `{summary?, addedCount, updatedCount, removedCount, entries[]}`                     |
| `DirectoryChangelogEntry`    | `{entityType, action, name, slug?, fieldsChanged?[]}`                               |
...
- Cross-directory changelogs (per-directory only).
...
- [x] **VI**: covered by service tests in
      `packages/agent/src/activity-log/` and `directory-history/`.
...
- User-facing doc: [`../../../features/directory-changelog.md`](../../../features/directory-changelog.md)
