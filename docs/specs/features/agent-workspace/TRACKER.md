...
that date. Every claim below names the pull request it rests on. The only Agent Workspace pull
request open at this refresh was #2508 (AW-22 backend hardening: backup trim reports, cancel during
the attachment copy, the one-hour ceiling); it has no web half, so AW-22 stays `In progress`.
...
- The six `In progress` rows are that rule applied to a phase that shipped its backend and stopped.
  Those pull requests added no web component (AW-17's #2428 added only the costs API route and its
  client helpers under `apps/web`), while its epic's P1
...
- No epic reads `Verified` yet. The legend defines `Verified` as e2e green on `develop`, but
  `.github/workflows/e2e.yml` runs on pushes to `stage` and on `workflow_dispatch` only (no
  `pull_request` or `develop` push trigger, by an owner decision of 2026-07-30), so `Verified`
  needs a manual dispatch on `develop` (as on 2026-09-25) or a redefinition as green on `stage`.
  #2455 repaired the specs and merged on 2026-09-17; it did not make the gate green — every
  completed stage run since #2455 merged has failed (the latest on 2026-09-21; two runs on
  2026-09-18 were cancelled, not failed). Choosing the definition is an owner decision, so this
  refresh leaves the legend alone.
