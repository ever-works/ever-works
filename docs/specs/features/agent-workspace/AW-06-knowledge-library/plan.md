...
_Why first:_ it is the only phase with a migration, and every later phase reads its columns.
...
_Why second:_ it is the highest-value half of the epic but depends on `revision` existing and being
...
_Why last:_ it is the only part that introduces a plugin, and it is the part users can most easily
