...
and that _"a free-text question cannot be rendered as a control, cannot be
...
`tryResumeLinkedRun` is best-effort by contract: _"the escalation IS resolved; a
resume hiccup must not undo that answer."_
...
whose `id` is `${sourceType}:${ownerId}` and is documented as _"synthetic, never a
DB PK"_).
...
migration of live data. The queue needs a _view_, and it gets one.
...
serialized `HitlAnswer`, validated by `validateHitlAnswer`. The five _user-facing_
...
| `fact`             | `text`                        | never                                             |
| `access`           | `access` _(new union member)_ | never                                             |
| `action`           | `action` _(new union member)_ | never                                             |
...
- **Secrets** (Constitution VII): an `access` question carries the _name_ of what is
...
acceptable placeholders in the siblings; the _structure_ must match or next-intl
