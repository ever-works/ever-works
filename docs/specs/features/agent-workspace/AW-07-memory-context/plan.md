...
- `truncateTailFirst(text, capTokens)` — _keeps the end, drops the beginning_,
...
3. `ContextBudgetService` calls it too, with the _saved_ file bodies rather than
...
```ts
export function truncateMiddleOut(
	text: string,
	capTokens: number
): {
	text: string;
...
> `expectedHash` (read moments earlier) still matches the _old stored_ value.
...
export interface MemoryFactListDto {
	facts: MemoryFactDto[];
	total: number;
	counts: Record<MemoryFactStatus, number>;
	nextCursor?: string;
	semantic: boolean;
}
...
- `ChatAttachments.tsx` renders a `'context-file'` chip with a document icon and,
  on click, opens the existing preview overlay with the file's _budgeted_ text
...
- **No new plugin.** Memory facts and context files are first-party platform data
  describing how _our_ agents behave; nothing external is integrated.
...
| #2 No duplicate nouns                            | ✅ One new noun, **Context file**, justified in spec §5.1 and added to the program vocabulary table in the same change. "Personality" and "Identity" are deliberately _not_ new files (spec §5.2).                                                                          |
