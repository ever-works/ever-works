...
export const CHANGELOG_ENTRIES: readonly ChangelogCatalogEntry[] = [ /* … */ ] as const;
```
...
- The list is *not* re-fetched after a read batch; the response's `unreadCount` updates the
...
- Constitution I (plugin-first) is about *external integrations*. This epic has none: no
