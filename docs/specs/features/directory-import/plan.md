# Implementation Plan: Directory Import
...
flowchart LR
    ...
    Apply --> DB[(Directory entity created)]
```
...
| Slug conflict  | Case-insensitive lookup against `directories` | Matches GitHub repo naming                    |

## 3. Data Model

- Additive columns on `directories`: `worksConfigPath` (nullable string),
...
| ------ | --------------------------------- | ------------------------------------- |
| `POST` | `/api/directories/import/preview` | Dry-run; returns parsed preview       |
| `POST` | `/api/directories/import`         | Confirm import; creates the directory |
...
- CLI: `ever-works directory import <url>`.

## 6. Background Jobs

Awesome-List item normalisation (extracting structured fields per item)
runs as a Trigger.dev fan-out task once the directory is created.
...
- Activity log: `directory_import` action with status and detected shape.
