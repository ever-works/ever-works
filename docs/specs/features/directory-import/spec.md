# Feature Specification: Directory Import

**Feature ID**: `directory-import`
...
A user can bootstrap a new directory in the platform from an **existing**
...
referenced in any `works.yml`, and creates the directory with all parsed
metadata pre-filled.
...
- **Given** I have a data repo with `works.yml`, **when** I import it,
  **then** the import flow pre-fills the directory's name, prompt, model,
...
- **Given** the source repo's slug conflicts with an existing directory I
...
- **FR-3** When `works.yml` is found, the import flow MUST pre-fill the
  directory's name, prompt, model, providers, and schedule from it.
...
- **FR-8** On confirmation, the platform MUST create the directory entity,
...
| `WorksConfigImportApplier` | Applies the validated plan to the directory entity    |
...
- [x] **IV**: long-running parts (Awesome-List item normalisation) run as
      background work.
- [x] **V**: only additive directory schema fields (e.g. `worksConfigPath`
      column).
...
- User-facing doc:
  [`../../../features/directory-import.md`](../../../features/directory-import.md)
