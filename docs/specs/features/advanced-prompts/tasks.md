...
- [x] **T1**. `DirectoryAdvancedPrompts` entity at
      `packages/agent/src/entities/directory-advanced-prompts.entity.ts`
    - All 7 prompt fields as `text NULL`; one-to-one with `Directory`
      with cascade delete.
    - Export from `packages/agent/src/entities/index.ts`.
- [x] **T2**. Migration adding `directory_advanced_prompts` table
      with unique index on `directoryId`. Additive only.
- [x] **T3** (parallel with T2). DTO
      `UpdateDirectoryAdvancedPromptsDto` at
...
- [x] **T4**. Response interface `DirectoryAdvancedPromptsResponseDto`
      mirroring the entity with timestamps as ISO strings.

## Phase 2 — Repository

- [x] **T5**. `DirectoryAdvancedPromptsRepository` at
      `packages/agent/src/database/repositories/directory-advanced-prompts.repository.ts`
    - `findByDirectoryId(id) → entity | null`
...
- [x] **T6**. `DirectoryAdvancedPromptsService` at
      `packages/agent/src/services/directory-advanced-prompts.service.ts`
    - `getAdvancedPrompts(directoryId) → response | null`
    - `updateAdvancedPrompts(directoryId, userId, dto) → response`
      with editor-role check
    - `getPromptsForGeneration(directoryId) → AdvancedPromptsContext`
...
- [x] **T7**. Wire into the directory module so the service is
...
- [x] **T10**. Load advanced prompts in
      ...
      `getPromptsForGeneration(directoryId)` and attach to context
...
- [x] **T12**. `GET /api/directories/:id/advanced-prompts` controller
      method on `apps/api/src/directories/directories.controller.ts`.
- [x] **T13**. `PUT /api/directories/:id/advanced-prompts` controller
...
- [x] **T15**. e2e tests in `apps/api/test/` covering: viewer cannot
      ...
      returns 400, missing directory returns 404.

## Phase 6 — Web UI

- [x] **T16**. API client at `apps/web/src/lib/api/directory.ts`:
    - `getAdvancedPrompts(id)`
    - `updateAdvancedPrompts(id, data)`
- [x] **T17**. Server action
      `updateAdvancedPrompts(directoryId, data)` at
      `apps/web/src/app/actions/dashboard/directories.ts` with Zod
...
- [x] **T20**. Translations under
      `dashboard.directoryDetail.settings.advancedPrompts.*` in
