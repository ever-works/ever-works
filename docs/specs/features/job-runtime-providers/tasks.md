...
T1 + T2 ✅ Done in [#1354](https://github.com/ever-works/ever-works/pull/1354) (cascaded [#1360](https://github.com/ever-works/ever-works/pull/1360) -> [#1362](https://github.com/ever-works/ever-works/pull/1362)). T3 ✅ Done in [#1369](https://github.com/ever-works/ever-works/pull/1369). T5 ✅ Done in [#1368](https://github.com/ever-works/ever-works/pull/1368). T6 ✅ Done in [#1370](https://github.com/ever-works/ever-works/pull/1370). T4 ✅ Done in [#1376](https://github.com/ever-works/ever-works/pull/1376). Phase 0 complete — only EW-686 P1 (re-house Trigger.dev as the `trigger` provider, plus wiring `buildJobRuntimeProviders()` into `TasksModule`) remains.
...
- [x] **T4.** Binding factory `packages/agent/src/tasks/job-runtime.providers.ts`: `JOB_RUNTIME_PROVIDER_REGISTRY` token + `InMemoryJobRuntimeProviderRegistry` default impl + `buildJobRuntimeProviders()` that binds all 11 `*_DISPATCHER` symbols to a per-call `registry.getActive()?.dispatchers ?? null` lookup. **Declared but not wired** — the existing `useExisting: TriggerService` bindings in `packages/tasks/src/trigger/trigger.module.ts` stay untouched until a follow-up cutover PR flips the wiring. Tenant-aware overlay (EW-742 P3 / EW-747) can extend the registry to `getActive(tenantId?)` without churning call sites. Shipped in [#1376](https://github.com/ever-works/ever-works/pull/1376).
