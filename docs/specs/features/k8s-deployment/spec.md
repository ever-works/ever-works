...
2. **`deployProvider` in `.works/works.yml`** — the works-config layer learns about `deployProvider` so a work's data repo can declare its target. The same change also applies to Vercel (any `deployProvider: <id>` recognised by the deploy facade is accepted).
...
- **Given** my work is set to `deployProvider = 'k8s'` (either via the dashboard or in `.works/works.yml`), **when** I deploy it, **then** the platform builds a container image of the website, pushes it to the configured registry (GHCR by default), applies a Deployment, Service and (optional) Ingress to the configured namespace, and reports the resulting URL back through the standard deploy facade.
- **Given** my work's `.works/works.yml` declares `deployProvider: k8s`, **when** I import or sync the work, **then** the platform sets `work.deployProvider = 'k8s'` accordingly. The same behaviour applies to `deployProvider: vercel` and to any future provider IDs registered through the deploy capability.
...
- **Given** my cluster does not have an ingress controller, **when** I deploy without configuring an ingress host, **then** the deployment still reaches `ready` once the pod is healthy and the work is reachable in-cluster via the `ClusterIP` Service. v1 does **not** auto-fall-back to a `LoadBalancer` Service (cloud-provider-specific dependency); the deployment result includes a `kubectl port-forward` hint so users can verify the pod locally.
...
- **Given** my `.works/works.yml` declares `deployProvider: someUnknownId`, **when** the platform syncs the file, **then** the import surfaces a validation error pointing at the unknown id, the work's `deployProvider` is left unchanged, and a clear message tells the user which provider IDs are available.
...
| `deployProvider` (works-config) | Optional field in `.works/works.yml` that declares which deployment plugin id (`k8s`, `vercel`, …) the work should use.                                                                                                                                                                                                                           |
...
- [ ] **works-config**: a `.works/works.yml` containing `deployProvider: k8s` results in `work.deployProvider = 'k8s'` after sync; same for `vercel`; an unknown id surfaces a validation error and leaves the existing value untouched.
...
- **D4 — Provider visibility & works-config**: the plugin's `visibility` is `'user-only'` (matches Vercel). Independently of visibility, both the dashboard **and** `.works/works.yml` can set `deployProvider`; the works-config layer is extended generically so this works for k8s, Vercel, and any future deploy plugin. (FR-17, FR-18, FR-19.)
