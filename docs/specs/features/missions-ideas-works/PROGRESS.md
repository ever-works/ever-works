...
**Tick 14 (DONE):** Phase 3 PR H — Mission CRUD + lifecycle live end-to-end. **Service** (`packages/agent/src/missions/missions.service.ts`): added `create` (validates schedule-vs-type consistency + clamps title), `getForUser` (404 on missing or wrong owner), `update` (partial PATCH semantics; handles type-flips by clearing orphan schedule), `pause`/`resume`/`complete` state-machine transitions (shared `transition()` helper enforces source-status guard), `delete` (any status; DB cascade leaves child Ideas + clones intact), and `runNow` (placeholder until PR J wires Trigger.dev — returns `noop-placeholder` for now). Added `assertScheduleConsistency` + `normalizeSchedule` internals to keep the cron-vs-type invariant in one place. **Controller** (`apps/api/src/missions/missions.controller.ts`): 8 new endpoints all decorated for OpenAPI/MCP per Decision A19; write paths rate-limited 30/min, runNow at 10/min. **DTOs** (`apps/api/src/missions/dto/mission.dto.ts`): `CreateMissionDto` + `UpdateMissionDto` with class-validator rules — `outstandingIdeasCap` accepts `>=-1` (negative sentinel for "unlimited"), `schedule` allows explicit null via `ValidateIf`. **Tests** (`packages/agent/src/missions/__tests__/missions.service.spec.ts`): 19 new tests across create validation, the 4 state-machine transitions including their idempotency-error cases, ownership 404s, partial-update semantics, and the type-flip edge cases. All green.
**Next:** Phase 3 PR I — shared titler service. AI-driven `generateTitle(prompt)` used by user-manual Idea create (Phase 1 PR B placeholder), Mission create (Phase 3 PR H — title currently required from caller), and Work title fallback. One service, multiple callers (no duplication). Will swap the `deriveTitle` placeholder in `WorkProposalService.createUserManual`.
