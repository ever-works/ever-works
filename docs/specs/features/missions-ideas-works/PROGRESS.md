...
**Tick 13 (DONE):** Phase 3 PR G — `MissionsModule` skeleton landed end-to-end. **Agent side** (`packages/agent/src/missions/`): new `MissionsService` (injects `Repository<Mission>` directly — no custom repository class yet, can extract later if query complexity warrants), `MissionsModule` (TypeORM forFeature + exports), `types.ts` with `MissionDto` + `toMissionDto` mapper. The `listForUser(userId)` read path is implemented; full CRUD + lifecycle ships in PR H. **API side** (`apps/api/src/missions/`): `MissionsController` with one route `GET /me/missions` (decorated for OpenAPI/MCP auto-derivation per Decision A19), `MissionsModule` thin wrapper importing agent-side module + AuthModule. **Wiring**: registered `MissionsModule` in `api.module.ts` after WorkAgentModule; added `./missions` to the agent package.json `exports` map so `@ever-works/agent/missions` resolves at TypeScript-build time (mirrors the existing `./work-agent` entry). 4998 agent tests green; api type-check shows no new errors. The new route returns `[]` for users with no Missions, exercising the full DI graph at boot.
**Next:** Phase 3 PR H — Mission CRUD + lifecycle endpoints. Adds `POST /me/missions`, `GET /me/missions/:id`, `PATCH /:id`, `POST /:id/pause`, `POST /:id/resume`, `POST /:id/complete`, `DELETE /:id`, `POST /:id/run-now`. Service-side: extend `MissionsService` with create / get / update / state-transition methods. DTOs with class-validator rules.
