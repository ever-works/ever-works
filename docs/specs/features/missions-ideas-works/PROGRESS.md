...
**Tick 10 (DONE):** Phase 1 PR D — surfaced all 4 promoted-constant prefs + 3 auto-retry knobs + 2 account-wide budget knobs (9 total new fields) through `WorkAgentPreferencesDto` and `UpdateWorkAgentPreferencesInput`. Tri-state PATCH semantics on the 6 nullable knobs (omit = unchanged, null = reset to default, value = override). Wired ONE consumer — `WorkProposalsApiService.runPipeline` now reads `autoGenerateBatchSize` from prefs and passes `targetCount` to the generator (rendered as "Generate exactly N proposals" in the prompt, clamped 1–20). Other consumers wire in later phases (cadence → scheduled-rerun dispatcher, throttle → PR FF goal-completion, mission cap → PR J Mission tick). `findOrCreatePreferences` sets the new field defaults explicitly so test stubs match the DB-shaped entity. Updated work-agent.service.spec, work-proposals.service.spec, and the web client interface. 129 tests green (108 agent + 21 API).
**Next:** Phase 1 PR FF — `POST /me/work-proposals/:id/retry` + `POST /me/work-proposals/:id/rebuild` endpoints + the Goal-completion handler with built-in transient-error classifier (network / 429 / 5xx / plugin-internal) + auto-retry loop reading `maxAutoRetries` / `backoffSeconds` / `exponentialBackoffFactor` from prefs. **This is the last Phase 1 PR.** After FF lands, Phase 2 (Dashboard renames + Total Missions/Ideas tiles) starts — the smallest user-visible PR.
