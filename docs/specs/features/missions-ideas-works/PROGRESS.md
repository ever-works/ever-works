...
**Tick 15 (DONE):** Phase 3 PR I — shared `TitlerService` extracted into its own module. **New** `packages/agent/src/titler/` (service + module + index + 9-test spec): `generateTitle(prompt, opts?)` returns ≤80-char title (configurable maxChars, floor 8). Pure heuristic for v1 (first-sentence boundary → whitespace collapse → trailing-punct strip → clip-to-maxChars), with the AI-backed implementation slot already factored in behind the same signature so swap-in is a single-method change later. Kind hint (`idea`/`mission`/`work`) drives empty-input fallback strings and is plumbed for future model-prompt steering. **Wiring**: `UserResearchModule` imports `TitlerModule`; `WorkProposalService.createUserManual` deletes its private `deriveTitle` placeholder from Phase 1 PR B and calls `titler.generateTitle(description, { kind: 'idea', userId })` when no title was supplied. `MissionsModule` imports `TitlerModule`; `MissionsService.create` makes `title` optional and falls back to titler when caller's title is empty. `CreateMissionDto.title` flipped to `@IsOptional`. **Package**: `./titler` added to `packages/agent/package.json` exports map. **Tests**: 9 titler-spec (first-sentence, whitespace collapse, trailing-punct strip, clip-to-maxChars, maxChars floor=8, kind-specific fallbacks, defensive non-string handling, newline-as-boundary) all green. Existing 19 missions-service + 3 work-proposal-service tests still pass with real `TitlerService` injected in `beforeEach`.
**Next:** Phase 3 PR J — Mission tick worker. Trigger.dev `schedules.task({id, cron, run})` modeled on `packages/tasks/src/tasks/trigger/work-schedule-dispatcher.task.ts` (Decision A7). For each `ACTIVE` + `SCHEDULED` Mission whose schedule matches, honors the outstanding-Ideas cap (PR D's `missionDefaultOutstandingCap` + per-Mission `outstandingIdeasCap` override), calls the PR C proposal generator with `missionContext`, persists spawned Ideas with `source=MISSION` + `missionId`. If `autoBuildWorks=true`, calls PR B's `queueForBuild` on each new Idea so it lands as `QUEUED` straight away.
