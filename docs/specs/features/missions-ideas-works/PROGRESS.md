...
**Tick 12 (DONE — Phase 2 COMPLETE):** Phase 2 PRs E + F — the smallest user-visible PR. **PR E (i18n)**: `proposals.header.title` → "Ideas", `proposals.header.titleWithName` → "Ideas" (no username variant), `proposals.actions.accept` → "Build", `proposals.actions.refresh` → "Suggest more", `dashboard.works.recent` → "Works". en.json updated by hand; the 20 other locales bulk-updated via a small Node script that preserved their structure where the key existed and added `stats.totalMissions` + `stats.totalIdeas` everywhere. Audit confirms all 21 locales have the new stats keys. **PR F (Stats)**: `WorkRepository.getAccessibleStats` extended with `totalMissions` + `totalIdeas` — uses `repository.manager.query` to run 2 raw COUNT(*)s on `missions` + `work_proposals` tables in parallel with the existing Works query (defensive `.catch(() => [{ c: 0 }])` so a missing table on a freshly-cloned dev box returns 0 rather than crashing the whole stats endpoint). `WorkStatsResponse` interface + `getWorkStats()` action + Dashboard page + `DashboardClient` + `StatsOverview` all extended end-to-end. `StatsOverview` now renders 5 tiles in spec §5.1 order `[Missions][Ideas][Works][Items][Sites]` with new icons (Target + Lightbulb) and a 5-column grid breakpoint. 80 work-repository tests green (including the 3 existing `getAccessibleStats` specs updated with the new envelope + the new `repository.manager.query` mock in beforeEach).
**Next:** Phase 3 PR G — `MissionsModule` skeleton. New `Missions` controller + service + DTOs registered in the API app, importing `Mission` entity via TypeORM forFeature. Doesn't add endpoints yet (that's PR H); just the wiring scaffold + a placeholder `listMine` so DI is exercised at module-init time.
