...
**Tick 4 (DONE):** PR 0.4 — added 4 nullable columns to `WorkAgentPreference` (`autoGenerateCadence`, `autoGenerateBatchSize`, `autoBuildThrottlePerDay`, `missionDefaultOutstandingCap`) + migration. NULL on each = "use platform hardcoded default" — safe to deploy ahead of PR D since the read path isn't wired yet. 86 tests green (work-agent + drift).
**Next:** PR 0.5 — `AddAutoRetryPrefs` migration. Add `max_auto_retries` (int 0–5, default 2), `backoff_seconds` (int 10–3600, default 60), `exponential_backoff_factor` (decimal 1.0–4.0, default 2.0) to `work_agent_preferences`. These have non-NULL defaults (unlike 0.4) because they're behavior-affecting from day one per Decision A23.
