...
**Tick 5 (DONE):** PR 0.5 — added 3 NOT-NULL retry-policy columns to `WorkAgentPreference` (`maxAutoRetries` default 2, `backoffSeconds` default 60, `exponentialBackoffFactor` float default 2.0) + migration. Non-NULL with defaults so Phase 1 PR FF retry handler activates immediately for every existing user without requiring a settings visit. 86 tests green.
**Next:** PR 0.6 — `AddAccountWideBudget` migration. Add `account_wide_monthly_cap_cents` (bigint, NULL) + `account_wide_allow_overage` (bool, default true) to `work_agent_preferences`. NULL on the cap = no account-wide cap. v6 / Decision A28 — drives the Dashboard `Month Spend` tile and the `#account-budgets` settings sub-section.
