...
**Tick 9 (DONE):** Phase 1 PR C — `buildProposalsPrompt` now takes (a) `existingIdeas: ExistingIdeaContext[]` rendered as a dual exclusion+context block with per-row status hint (model leans adjacent to ACCEPTED themes, avoids replays of DISMISSED ones — spec §3.3), and (b) optional `missionContext: { description, kbExcerpts? }` rendered as a "bias all proposals toward this Goal" block (Phase 3 PR J payload). Token-budget guards: ≤50 Ideas per prompt, ≤140 chars per description. `WorkProposalService.generate` fetches every Idea once (all statuses, reuses existing `findRecentByUser`), threads it through the prompt builder, AND uses it for the existing post-coercion dedupe loop (single fetch, dual purpose). `GenerateProposalsOptions` gains `missionContext` + `missionId` so Mission tick worker can both bias generation and persist the back-FK. Added 5 focused tests on the new behavior. Existing callers unchanged — params default to back-compat empty. 51 prompt-related tests green.
**Next:** Phase 1 PR D — promote four work-agent loop constants to user prefs read path. Auto-generate cadence + batch size + auto-build throttle + Mission default outstanding cap. Read from `WorkAgentPreference` columns added in PR 0.4, fall back to existing hardcoded defaults when NULL.
