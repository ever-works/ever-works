...
**Tick 11 (DONE — Phase 1 COMPLETE):** Phase 1 PR FF — added the transient-error classifier (pure function, platform-managed per Decision A23, NOT user-configurable), the `computeBackoffSeconds` helper (spec §3.9 formula clamped to [backoff, 1day]), the goal-completion decision API (`handleGoalCompletion` returns `{ outcome: accepted | rebuild-accepted | retry | failed | noop, ... }`), and the two new API endpoints `POST /me/work-proposals/:id/retry` (FAILED → QUEUED + new Goal) and `POST /me/work-proposals/:id/rebuild` (ACCEPTED → BUILDING + new Goal; `acceptedWorkId` re-points on completion, original Work preserved per Decision A27). The classifier handles structured HTTP statuses (429/5xx → transient, 4xx-other → permanent) and message-pattern fallback (ECONNREFUSED, ETIMEDOUT, rate-limit prose, upstream 5xx prose, plugin timeouts, validation phrasing). 21 focused classifier-spec tests cover every branch. Idea status STAYS at BUILDING across auto-retries (Decision A24 — no flicker). Auto-retry scheduling primitive deferred to the goal-execution wiring (which doesn't exist yet); the decision API returns `retryDelaySeconds` for the caller to schedule. **Phase 1 complete: A + B + C + D + FF all DONE.**
**Next:** Phase 2 — the smallest user-visible PR. i18n renames (Ideas/Works/Build/Suggest more across all 21 locales) + Dashboard stats row reorder with new Total Missions + Total Ideas tiles. Designed early in the PLAN to de-risk the migration + i18n paths.
