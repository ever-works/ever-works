...
to a directory's deployed website, replacing the provider-assigned URL
...
upgrades the directory's primary URL away from the provider subdomain
...
- **Given** my directory is deployed to Vercel, **when** I add
...
- **Given** I've configured my DNS to point `tools.example.com` at
  ...
  `verified: true`, and auto-promotes my directory's URL.
- **Given** my directory's primary URL was `<slug>.vercel.app` and my
  custom domain just became verified, **when** auto-promote runs,
  **then** the directory's `siteUrl` is updated to the custom domain
...
- **Given** I try to add a domain to a directory that has no deploy
...
- **Given** I add a domain that already exists on another directory in
...
- **FR-5** When a domain becomes verified AND the directory's current
...
- **FR-7** Domain operations MUST require directory edit permission.
...
- **Security & privacy**: domain operations require directory edit
...
| ----------------- | ---------------------------------------------------------------------- |
| `CustomDomain`    | DB row: `{domain, verified, environment, provider, directoryId}`       |
| Provider sync     | One-way mirror from DB → deploy provider                               |
| Auto-promote      | Replace directory `siteUrl` once a custom domain verifies              |
| DNS record advice | Apex → A record; subdomain → CNAME (provider-specific values returned) |

## 6. Out of Scope

- Multiple deployment environments per directory beyond `production` (the
