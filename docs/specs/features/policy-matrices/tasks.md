...
- [ ] **T40** _(OQ-6)_ Decide whether `protectedBranches` should be case-normalised
      ...
      that behave as one.
- [x] **T41** _(OQ-7)_ Harden `isCredentialKey` to reject non-string input.
      Shipped 2026-08-23: the signature is now `(value: unknown): value is string`
      with an explicit `typeof` guard, and the spec that pinned the coercion hole
      was flipped into an 11-case regression matrix.
