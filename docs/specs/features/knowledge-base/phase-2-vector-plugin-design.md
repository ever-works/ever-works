...
| **D4** | **Embedding mode is per-Work configurable** via a cascading chain `env default → org override → Work setting → plugin capability fallback`. A `kbEmbeddingMode: 'platform' \| 'plugin' \| 'auto'` Work setting drives selection. `auto` (the default) picks `plugin` iff the vector store plugin's `capabilities.embedsOnWrite === true`, otherwise `platform`. Operators pin a platform-wide default via `KB_EMBEDDING_MODE`; orgs can override; Works can override again. Resolves the original §12 Q1 ("`embedsOnWrite` plugins").                                                  | Adopt — flexible per-Work, same shape as every other plugin selection in the platform.                                     |
| **D5** | **Plugin manifest declares `namespacePerWork: 'collection' \| 'namespace' \| 'rowFilter'`** as an explicit field on `VectorStoreCapabilities`. The operator UX surfaces this so admins understand the tenancy shape ("this plugin uses one collection per Work" / "this plugin uses one shared collection, namespace-isolated" / "this plugin uses a `WHERE workId = $1` filter on a shared table"). Resolves the original §12 Q2 ("multi-tenant collections").                                                                                                                        | Adopt — small contract surface, large operator-clarity win.                                                                |
| **D6** | **`BaseVectorStore` exposes `normalize(rawScore: number): number`** that every concrete plugin overrides to return a calibrated `[0, 1]` score. `QueryHit.normalizedScore` is mandatory on results; `QueryHit.rawScore` is preserved for diagnostics. RRF consumers can use ordinal `rank` (unchanged) OR `normalizedScore` (new). UI surfaces (confidence badge, sort-by-strength) get a stable cross-plugin number from day one. Resolves the original §12 Q3 ("score normalization").                                                                                               | Adopt — small per-plugin cost, future-proofs every "confidence" UI feature.                                                |
| **D7** | **`embeddingModel` change triggers an auto Trigger.dev re-embed sweep**. When a Work-scope vector-store plugin instance's `embeddingModel` setting changes (or the resolved `kbEmbeddingProviderId` changes), the platform dispatches `kb-reembed-work` Trigger.dev task with `{ workId, previousModel, newModel }`. The task walks every `(workId, documentId)` row in `work_knowledge_chunk_coordinates` and re-upserts chunks through the facade. Watermarked by `embedding_model` + `embedding_dims` so the sweep is idempotent. Resolves the original §12 Q4 ("re-embed sweeps"). | Adopt — automatic correctness over manual ceremony; cost is bounded by the Work's KB size and visible in the activity log. |
...
## 12. Resolved decisions (2026-06-06)

The four open questions from the original RFC have been resolved by the platform owner. Each maps to a new design decision in §3:

1. ~**`embedsOnWrite` plugins**~ → **D4** (adopted). Embedding mode is per-Work configurable through the cascading chain `env default → org override → Work setting → plugin capability fallback`. The new `kbEmbeddingMode: 'platform' | 'plugin' | 'auto'` Work setting plus `KB_EMBEDDING_MODE` operator env knob make this the same shape as every other "per Work configurable thing" in the platform. `auto` (the default) inspects `capabilities.embedsOnWrite` and picks `plugin` when supported, else `platform`.

2. ~**Multi-tenant collections**~ → **D5** (adopted). `VectorStoreCapabilities.namespacePerWork: 'collection' | 'namespace' | 'rowFilter'` is now a required manifest field. The operator UX surfaces it so admins understand the tenancy shape without reading the plugin source.

3. ~**Score normalization**~ → **D6** (adopted — moved up from "post-slice-1" to slice 1). `BaseVectorStore.normalize(rawScore)` returns a calibrated `[0, 1]` score; `QueryHit.normalizedScore` is mandatory; `QueryHit.rawScore` preserved for diagnostics. RRF can still use ordinal `rank`. UI confidence badges + sort-by-strength get a stable cross-plugin number from day one.

4. ~**Re-embed sweeps**~ → **D7** (adopted — auto-sweep, not lazy). When `embeddingModel` (or the resolved `kbEmbeddingProviderId`) changes for a Work, the platform dispatches `kb-reembed-work` Trigger.dev task with `{ workId, previousModel, newModel }`. Watermarked by `embedding_model` + `embedding_dims` in `work_knowledge_chunk_coordinates` so the sweep is idempotent and re-resumable. Activity-log entry emits `KB_REEMBED_STARTED` / `KB_REEMBED_COMPLETED`.

These resolutions are reflected in the slice tickets (EW-724, EW-725, EW-726). No further open questions block slice 1 implementation.
