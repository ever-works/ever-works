...
- For `deployProvider = 'ever-works'` (managed) we _do_ auto-create
...
- For `deployProvider = 'k8s'` (bring-your-own-cluster — the path the migration
  used) there is **no DNS automation at all**: the Ingress host is merely _derived_
...
- **Orphan cleanup**: on slug regenerate / Work delete, remove the _persisted_ old
