...
- [ ] **T2. Provider stood up (private operations repository).** _(operator attestation)_
      ...
      **Test**: the Ever Works administrator **Test connection** (T6) against the development provider returns every
      FR-3 check green; the result is recorded in the private operations change log as `apw12-provider-standup`.
      **Done when**: the development discovery document satisfies every row of [plan §4.3](./plan.md) by
      manual inspection, recorded in the operations change log.
...
- [ ] **T4 (parallel with T3). Identity provider capability contract.**
      ...
      **Test**: `packages/plugin/src/contracts/__tests__/identity-provider.interface.spec.ts` (**new**) — the type
      ...
      `pnpm --filter @ever-works/plugin test`.
      **Done when**: the plugin package builds and no existing category or capability changed.
...
- [ ] **T11. Migrations.**
      ...
      **Done when**: a fresh database and a copy of stage's schema both migrate up and down cleanly.
...
- [ ] **T15. Linking service and Activity members.**
      ...
      **Test**: `apps/api/src/auth/services/ever-id-linking.service.spec.ts` (**new**) — the spec §5.3 decision tree as a
      table test (ACC-12-13…ACC-12-18); S3 returns no account ID; `canDisconnect` truth table (ACC-12-20); sign-up runs
      in one transaction with terms recorded (ACC-12-14); the concurrent sign-up/connect race ends with one connection
      and the other outcome S12 (ACC-12-19); no row written by any path contains a token (ACC-12-22).
...
- [ ] **T16. Request-shape guards and `authMethod`.** _(Resolution R-19)_
      ...
      **Done when**: ACC-12-10 and ACC-12-21 hold at unit level, no existing guard assertion changed, and a grep of
      `apps/api/src` finds no `authMethod` value other than `'session'`, `'api-key'` and `'ever-id-delegated'`.
...
- [ ] **T21. Fail-closed flag.**
      ...
      **Test**: `apps/web/src/lib/feature-flags/ever-id.flag.unit.spec.ts` (**new**) — no key → configuration decides;
      error, timeout, `undefined` → off (ACC-12-02); existing work-kinds unit tests unchanged.
      **Done when**: ACC-12-02 holds.
...
- [ ] **T22. Providers, API client, server actions, cookies.**
      ...
      **Create** `apps/web/src/lib/auth/ever-id-cookies.ts` (**new**).
      **Test**: extend `apps/web/src/app/actions/auth.unit.spec.ts` — `startEverIdSignIn` rejects an absolute `returnTo`
      (ACC-12-12 web half); "Also sign out of Ever ID" redirects to the URL from `GET /logout-url` (ACC-12-26 web half);
      no action returns an upstream message verbatim; extend `apps/web/src/lib/api/auth.unit.spec.ts` — providers
      default `everId: { enabled: false }` when absent.
      **Done when**: no action returns an upstream error message verbatim (translated generic messages only).
...
- [ ] **T26. Connected identities card, connect confirmation, sign-out option.**
      **Create** `apps/web/src/components/settings/ConnectedIdentitiesCard.tsx` and
      `apps/web/src/app/[locale]/(dashboard)/settings/security/connect-ever-id/page.tsx` (**new**). **Modify**
      `apps/web/src/components/settings/SecuritySettings.tsx` and the sign-out dialog that calls `logout()`.
      **Test**: `apps/web/src/components/settings/ConnectedIdentitiesCard.unit.spec.tsx` (**new**) — connected, not
      connected, cannot disconnect, turned off, and the delegated apps list with last-used time (ACC-12-36).
      **Done when**: ACC-12-36 renders from API data and the S14 variant disables Disconnect with its reason.
...
- [ ] **T27. i18n.**
      ...
      **Test**: `apps/web/src/lib/auth/ever-id-copy.unit.spec.ts` (**new**) — every key present in every locale, no leaf
      contains a dot, no English value contains "SSO" or "single sign-on" (ACC-12-38).
      **Done when**: ACC-12-38 holds.
...
- [ ] **T30. Playwright suites.**
      ...
      **Test**: run `pnpm --filter ever-works-web test:e2e ever-id`. Assertions: S1 sign-in with Activity (ACC-12-13),
      ...
      (ACC-12-15), S11 (ACC-12-16); connect with a session aged past 12 h in the database → S15 and within 300 s
      `auth_time` succeeds (ACC-12-17), S12 (ACC-12-18), disconnect keeps the current session (ACC-12-20); sign-out notice
      ends two Ever ID sessions ≤ 5 s and a password session survives, replayed `jti` → 400 (ACC-12-23, ACC-12-24,
      ACC-12-25); disabled: no button, sign-in endpoints 404, card and notices still work (ACC-12-01, ACC-12-04); axe
...
- [ ] **T32. Admin health and telemetry.**
      ...
      **Test**: `apps/api/src/auth/services/ever-id-telemetry.spec.ts` (**new**) — every §9.1 payload built by the
      ...
      `apps/api/src/auth/controllers/ever-id.controller.spec.ts` — both admin routes refuse non-admins.
      **Done when**: ACC-12-37 holds for telemetry as well as Activity (T45).
...
- [ ] **T44. Characterise the existing terminal browser sign-in.** _(lands before T28)_
      **Create** `apps/cli/src/commands/auth/__tests__/login.command.browser-flow.spec.ts` (**new**) — no production code
      change.
      **Test**: that spec — `login` without `--ever-id` starts the loopback hand-off of
      `apps/cli/src/commands/auth/oauth.service.ts` on port 44663, accepts the `sessionToken` it returns and stores it
      exactly where it does today; `--manual` still prompts for a token; neither path imports or calls
      `ever-id-device.service.ts` (ACC-12-32); run `pnpm --filter ever-works-cli test`.
