...
- **Cross-epic prerequisites** (typed fakes until they land): APW-03 effective App spec + `app.spec.applied` and the
  `generate.keypair.format` schema field (R-11); APW-05's latest deployable Build fingerprints; APW-06's runtime target
  resolver (kubeconfig, context, namespace, app pod labels), current Deployment fingerprints, domain state, the
  `delete-app-work` and `verification-deploy` ops (R-15, R-10); APW-01's `WorkCapabilities.appEnvironment` (R-7);
  APW-10's `AppsTierPolicy` (R-5).
...
- [ ] **T2 (parallel with T1). App dependency contracts.**
      **Create** `packages/contracts/src/apps/app-dependencies.ts` with `APP_DEPENDENCY_KINDS`,
      ...
      and the `AppDependencyView` type.
      **Test**: `packages/contracts/src/apps/__tests__/app-dependencies.spec.ts` — output names and secret flags equal
      ...
      `APP_DEPENDENCY_BACKUP_OVERDUE_MS` = 93,600,000.
...
CASCADE`, indexes; no `ALTER TABLE "works"`; up/down on SQLite and Postgres.
      **Done when**: `pnpm --filter ever-works-api test -- CreateAppEnvAndDependencies` is green, both databases migrate
      cleanly and the timestamp is above the newest `develop` migration.
...
- [ ] **T8. Repositories.**
      ...
      **Modify** `packages/agent/src/database/index.ts` (exports).
      **Test**: `packages/agent/src/database/repositories/__tests__/work-app-env-value.repository.spec.ts` — 20 concurrent
      ...
      second active row for the same kind is refused while a kept one is allowed.
      **Done when**: `pnpm --filter @ever-works/agent test -- work-app-env-value.repository work-app-dependency.repository`
      is green on SQLite and the Postgres test container.
...
- [ ] **T14. `AppEnvResolver` and the `AppRuntimeEnvSource` port.**
      **Create** `packages/agent/src/app-env/app-env.resolver.ts` ([plan §2.2 and §4.6](./plan.md)) and
      ...
      `ew-dep://<kind>/<output>` placeholders for target `ever-works-apps` (`resolveEphemeral` is T43).
      **Test**: `packages/agent/src/app-env/__tests__/app-env.resolver.spec.ts` — one case per §2.2 row for both phases;
      ...
      `notAvailableAtBuild`; dependency not ready → listed in `notReadyDependencies`; template depth 11 →
      `templateUnresolvable`; fingerprints `v<n>` / `d<n>` / sha256 only for non-secret values; unresolved items never
      carry a value. `packages/agent/src/app-env/__tests__/app-env-runtime.source.spec.ts` — port shape (`values`,
      `secretNames`, `unsetRequired` naming both unset required values — ACC-07-09, `notReadyDependencies`, `egress`
      host/ports for `smtp-external` and `s3-external`); keypair `<NAME>_PUBLIC` included and the private value only in
      `values` (ACC-07-06); managed target emits placeholders and no output value.
...
- [ ] **T16. `AppDependencyFacadeService` and `AppDependenciesService`.**
      ...
      **Test**: `packages/agent/src/app-dependencies/__tests__/app-dependencies.service.spec.ts` — reconcile transitions
      (new kind → pending + dispatch; removed kind → `inSpec=false`, no dispatch; target change → old `kept`, new pending);
...
- [ ] **T17. `app-dependency-provision` dispatcher and job.**
      ...
      **Modify** `packages/agent/src/tasks/index.ts`, `packages/agent/src/tasks/_tasks-symbols.ts`
      ...
      `packages/tasks/src/tasks/trigger/index.ts`, `packages/tasks/src/trigger/trigger.module.ts`.
...
- [ ] **T19. `k8s-inline-postgres`.**
      ...
      **Test**: `packages/plugins/k8s/src/app-dependencies/__tests__/postgres-operator-path.spec.ts` — CRD served + allowed →
      ...
      `ever-works.io/retain` on the PVC; missing `ew-allow-deps` → `namespacePoliciesMissing`, so no pod outside the app's
      namespace can reach it (ACC-07-14); no default storage class → `failed noDefaultStorageClass` with the S18 reason
      (ACC-07-20); `directUrl` output only when declared.
...
- [ ] **T21 (parallel with T19). `k8s-inline-minio`.**
      ...
      **Test**: `packages/plugins/k8s/src/app-dependencies/__tests__/object-storage.spec.ts` — StatefulSet 20 GiB; init Job
      ...
      `true` deletes PVCs and re-lists to zero.
...
- [ ] **T22. `app-dependencies-external` plugin (SMTP, S3).**
      **Create** `packages/plugins/app-dependencies-external/` (`package.json` named
      ...
      `app-dependency`, capability `app-dependency`; deps `nodemailer`, `@aws-sdk/client-s3`; tsup/vitest like
...
- [ ] **T24. App env controller.**
      **Create** `apps/api/src/app-env/app-env.controller.ts`, `apps/api/src/app-env/dto/app-env.dto.ts`
      ...
      [plan §5](./plan.md). **Modify** `apps/api/src/api.module.ts` to import the module.
      **Test**: `apps/api/src/app-env/app-env.controller.spec.ts` — foreign id 404 on every route and viewer 403 on PUT and
      rotate (ACC-07-23); 503 `secureStorageUnavailable` without key (ACC-07-12); rotate without or with a wrong
      `confirmName` 422, with it 200 and the entry's version + 1 (ACC-07-04); 11th rotation 429; every response body and all
      captured logger output scanned for each submitted value → absent (ACC-07-05); import results per line
      (ACC-07-11).
...
- [ ] **T25. App dependencies controller.**
      **Create** `apps/api/src/app-dependencies/app-dependencies.controller.ts`, `apps/api/src/app-dependencies/dto/app-dependencies.dto.ts`,
      `apps/api/src/app-dependencies/app-dependencies.module.ts`; **Modify** `apps/api/src/api.module.ts`.
...
- [ ] **T36. `apps-tier-dependencies` plugin.**
      **Create** `packages/plugins/apps-tier-dependencies/` (package `@ever-works/apps-tier-dependencies-plugin`;
      ...
      `src/managed-object-storage.provider.ts` for providers `managed-postgres`, `managed-redis`, `managed-object-storage`; `provision` writes `{ kind, ref }` through
      `IAppsTierProvider.applyWork` and reads `Work.status.dependencies[]`; no endpoint or credential settings;
      `outputsEncrypted` never written).
      ...
      **Test**: `packages/plugins/apps-tier-dependencies/src/__tests__/apps-tier-dependencies.plugin.spec.ts` against an
      `IAppsTierProvider` fake — refs written per declared kind; ready mirrors zone status; no connection string or role
      name leaves the zone fake (ACC-07-26). Extend `packages/agent/src/app-dependencies/__tests__/app-dependencies.service.spec.ts`
      ...
      environment as ACC-07-26 via probe LG-14.
...
- [ ] **T37. Managed backup reporting.**
      ...
      **Test**: extend `packages/plugins/apps-tier-dependencies/src/__tests__/apps-tier-dependencies.plugin.spec.ts` — fresh,
      25-hour-old and absent timestamps map to `healthy`, `overdue`, `unknown`, and a card view built from a 3-hour-old
      timestamp reads "last completed" (ACC-07-28).
...
- [ ] **T40. Contracts and schema parity.**
      **Create** `packages/contracts/src/apps/__tests__/dependency-outputs-parity.spec.ts` asserting
...
kinds)` calling providers with `ephemeral: true` and returning outputs in memory. **Modify**
      ...
      **Test**: `packages/agent/src/app-env/__tests__/app-env-ephemeral.spec.ts` — a repository spy records zero inserts and
      ...
      the `runner` recipe contains no value from the store or generator (sentinel search) (ACC-07-31).
