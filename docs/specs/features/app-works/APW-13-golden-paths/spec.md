...
- **S2 — Cal.diy from a Blueprint, as a user sees it.**
  ...
  **then** the form shows **Cal.diy (community build)**, the MIT licence, the trademark notice, and asks only for the
  administrator's email and password; the app is live on its domain with telemetry disabled, the administrator can sign
  in, and their administrator account already existed when the app first became reachable.
...
- **FR-9.** Acceptance runs never fork the canonical repository. Each run generates its own upstream from it in the test
  organization, so each fork starts a new repository network.
...
- **FR-19.** The Blueprint is displayed as **Cal.diy (community build)** with the upstream owner's trademark notice, and
  declares the licence file and trademarked logo assets read-only to agents.
...
- **FR-26.** The app runs with a writable root filesystem (its image rewrites its public address at boot) and a startup
  budget of 10 minutes; a domain change restarts it and never rebuilds it.
...
- **FR-38.** **PR lane**: runs with the existing suite on a local stack. It uses a fake GitHub, no model and no real
  cluster, completes the App Works scenarios within **8 minutes** summed across shards, and needs no secret.
- **FR-39.** **PR — cluster lane**: a local stack plus a throwaway kind cluster and the fixture's published image; no
  real GitHub, no model; **25 minutes**.
- **FR-40.** **Nightly lane**: against the development deployment with the real test estate and a real model; fixture
  ...
  **90 minutes**, **1.2 M tokens**; Actions minutes only on public test repositories.
...
`newer upstream not yet verified` is an independent flag set and cleared by the upstream-sync canary.
...
and a digit") and validate before submit.
...
- [ ] **ACC-13-05** The digest-pinned image deploys without a build; the default administrator password is refused and
      the chosen one accepted, and the ingress was created after the job completed.
- [ ] **ACC-13-06** The running app reports telemetry and update checks disabled.

**Cal.diy**

- [ ] **ACC-13-07** The pinned build completes within 60 minutes inside its declared resources.
...
- **[NEEDS CLARIFICATION: the Cal.diy repository the golden-path lane links.]** The owner asked for no automated fork of
  ...
  lane only links it and proves the fork step on the fixture in the same run._
...
- **[NEEDS CLARIFICATION: signup on Cal.diy App Works.]** Closing public signup needs a build-time setting the upstream
  ...
  decide between an overlay Dockerfile and a runtime check after the first verification run._
...
- **[NEEDS CLARIFICATION: the injection fixture's visibility.]** _Default: public within the test organization, with a
  banner stating it is a hostile test fixture; never referenced from product documentation._
