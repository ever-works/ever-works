...
- All App cluster I/O runs in Trigger.dev tasks on queue `app-cluster-io` (`concurrencyLimit: 20`). The API module
  imports no App cluster code path. **Added (APW06-G02):** `AppRuntimeFacadeService` is *constructed* in every
...
**The `dispatchers` view and the arity pin (added, APW06-G02).** `TriggerService` *is* the `dispatchers` view, so
...
export interface AppRuntimeTargetPort {
	prepareDependencyTarget(workId: string): Promise<
...
- **Custom domains on the managed tier (GAP-27).** They ship in **Wave 2**, as APW-10 builds them (LG-17 is P2): T49's
  ...
  or App Work removal. The Wave-3 note in spec §9 is narrowed to *additional* edge features, not to custom domains.
