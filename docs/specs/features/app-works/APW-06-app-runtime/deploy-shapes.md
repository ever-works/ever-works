...
An App Work has **exactly one *chosen* deploy target** (APW-06 FR-1) — that is a product rule and it stays. But the
*choices* are not two, and the substrate underneath them is not one cluster. The platform already resolves a deploy
...
`ClusterSource` value `k8s-works-shared`, label *"Ever Works shared customer cluster"*
...
`k8s-works`, label *"Ever Works internal cluster (admin only)"*, offered only to a platform admin and only when the
...
`custom-kubeconfig`, label *"Custom — paste your own kubeconfig"* (`cluster-source-matrix.ts:45,53,60`). The CLI
...
nothing needs to reach *in* — which is exactly what a customer behind NAT or a home-lab install needs.
...
LG-01, LG-03, LG-16 and D15 asserted *separate hosts / network / egress identity* — written when the tier was imagined
...
| C/F/G (customer cluster, connected node, SSH host) | the same *properties*, attested for **that** host — the tier does not weaken them just because the hardware is the customer's |
...
- **Whether the tier is *offered* on F/G in Wave 2 or Wave 3** — APW-10 `FR-33`/`FR-35` keep the offering rules; the
