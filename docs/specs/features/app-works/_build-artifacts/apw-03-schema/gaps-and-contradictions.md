...
**Revision warning.** Other agents were editing this worktree *while this artifact was built*
...
- Related: OQ-06 in the sibling catalog artifact proposes a separate `.works/template.yml` so the App
  spec's meaning is identical in both modes. That resolves *where a template's own coordinates live*,
...
- **Recommended answer:** either (a) delete `components[].target` from §10 and the schema, or (b) state
  in §10 that it selects a *stage within the one image* and is only meaningful when the image contains
...
lines 342-345) and APW-06 `tasks.md:426-431` define when the *source offer* is due, and ACC-03-36 makes a
...
construct *and* its imperative condition. This is the summary of **why** each imperative rule is
...
| Mode | `blueprint_mode_forbidden_key` | depends on *where* the file is validated, not on its content (G1) |
...
| **Q3** | `build.strategy: none` **with** components is accepted (R2's second half only requires `strategy` to be *declared*). | Is `strategy: none` + components legal? | No — reject it. A declared component with `none` cannot be built or deployed; add `strategy: none` to the `components_require_strategy` condition. This artifact currently follows the literal text, and no fixture pins it either way. |
