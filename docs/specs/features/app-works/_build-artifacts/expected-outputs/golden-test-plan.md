...
§24 is `schema.md:507-718`), and that the two *real* App specs are the APW-13 Blueprints, which are what this
...
| ACC-E2E-05 (`:330-333`) | the default branch carries the spec and `ever-works-build.yml` committed as *Add Ever Works build workflow*; a push yields `succeeded` with a digest |
...
| ACC-E2E-14 (`:616-621`) | on **Link** the workflow arrives as the PR *Add Ever Works build workflow* and is merged by a person |
...
The managed target is **not a second renderer**. APW-06 renders once; on `ever-works-apps` the *same* rendered
...
**What the test must assert for the managed target:** that the *rendered* objects differ in exactly the rows
