...
10. One transaction: `WorkRepository.create(workData)` (with `sourceRepository.createdByThisWork` = `true` only when
    step 9 issued the fork request or created the private-copy repository, and `sourceRepository.blueprintId` /
    `blueprintMatchSource` from step 6a) + `WorkUpstreamStateRepository.create({
workId, relation, readinessState: 'preparing', … })` (APW-02 repository). Write-only prompted values sent with the
