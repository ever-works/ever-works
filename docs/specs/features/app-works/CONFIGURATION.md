...
| `PLUGIN_EAGER_BUILTINS`                   | `false` | Landed 2026-09-26 (`60916d328`). Lazy mode only (`PLUGIN_LAZY_LOAD` not `false`): `true` materialises the builtIn plugins discovered on disk at boot instead of on first use — the runtime switch back to the earlier boot                                                                          | `packages/agent/src/plugins/services/plugin-bootstrap.service.ts` (API and worker)                                         |
