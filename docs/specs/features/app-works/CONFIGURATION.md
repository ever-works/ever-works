...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API process environment                   | `packages/agent/src/config/index.ts` — one `config` object literal of plain getters reading `process.env` directly; there is **no Joi/zod env schema** in this repo | `packages/agent/src/config/index.ts:107` (`export const config = {`) · `:1044` (`everWorks: {`) · `apps/api/src/config/` holds no `index.ts`            |
...
| `EVER_WORKS_APPS_CATALOG_TOKEN`                     | env                         | unset — optional token for the authenticated catalog fallback read                                                                                                                 | **yes** | APW-03                | [CONTRACTS.md](./CONTRACTS.md):477                                                                                                |
...
| `EVER_WORKS_APPS_CATALOG_TOKEN`                    | set only if the listing repository is read through the authenticated fallback                                                                         | production deployment configuration (secret)  | [CONTRACTS.md](./CONTRACTS.md):477                                                                                                   |
...
| `ever-works/<app>-template` (e.g. `cal-diy-template`, `umami-template`, `app-fixture-hello-template`) | one **App Blueprint**: `.works/works.yml` + overlay + smoke tests                                 | `main` (required default branch)                   | PR; a **tag `v<version>` must resolve to the `sha` the manifest pins**             | `v<semver>` per Blueprint version    | the pinned `blueprint.sha`                                                     |
