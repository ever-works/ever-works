...
| `APP_WORKS_CLOUD_PUSH_ENABLED`                      | env                         | `false` — on only for exactly `'true'`. Off: a cloud run's `finalizeRun` pushes nothing and blocks the Task until APW-08 T12; agent git tools ignore it (THREAT-MODEL T-03)        | no      | APW-08                | `packages/agent/src/config/index.ts:1090` · [`APW-08/plan.md`](./APW-08-evolve-loop/plan.md) §2.5                                 |
...
process (`apps/api/src/config/constants.ts`); `PLUGIN_EAGER_BUILTINS` is read by the shared `PluginBootstrapService`,
so it applies in every process that bootstraps plugins, the Trigger.dev worker included
(`packages/tasks/src/trigger/worker/services/trigger-plugin-hydrator.service.ts` calls `bootstrap`). The user-facing
reference is [`docs/features/plugins.md`](../../../features/plugins.md).
...
| `PLUGIN_EAGER_BUILTINS`                   | `false` | **Pending — lands with the plugin-bootstrap lazy-builtIns change; not yet committed on this branch.** Lazy mode only (`PLUGIN_LAZY_LOAD` not `false`): `true` materialises the builtIn plugins discovered on disk at boot instead of on first use — the runtime switch back to the earlier boot     | `packages/agent/src/plugins/services/plugin-bootstrap.service.ts` (API and worker)                                         |
