# Feature Specification: Creating a Work
...
Users can create a work through three paths: **AI Creation** (provide a
...
ends with a fully wired work: three git repositories, a database row,
...
- **AI Creation**: **Given** I'm on the New Work page with a connected
  git provider, **when** I choose "AI Creation", enter a name + prompt,
  pick a pipeline (or accept the default), and submit, **then** the
  platform creates the work + three git repos and dispatches an AI
  generation job; the page redirects to the work detail view where
...
- **Import — data repo**: **Given** I have a previously-generated Ever
  ...
  detects the `.works/works.yml`, copies items/categories/tags verbatim, and
...
- **Given** the slug I generated (AI/Import) collides with an existing
  work I own, **when** the platform detects it, **then** I'm
...
- **FR-3** AI Creation MUST resolve plugin defaults via the cascade:
  user form selection → work-level defaults (when regenerating) →
...
- **FR-6** Import MUST detect source-repo shape (data repo / Awesome
  ...
  [`work-import`](../work-import/spec.md) and
...
- **FR-11** Each pipeline plugin MAY contribute additional form fields
  via `form-schema-provider`; the New Work form MUST render those
  fields when the pipeline is selected.
- **FR-12** The system MUST emit an activity-log entry for every
  creation with `work_created` action and the chosen method
...
- **Reliability**: if any of the three repo creates fails, the platform
  rolls back the DB row and surfaces the failure (no orphan works).
...
- Cloning a work from another user (use export/import flow).
- Switching creation method mid-flow (each is a separate form).
- Creating a work without any git provider (always required).
- Creating a work targeting two git providers simultaneously.

## 7. Acceptance Criteria

- [x] All three methods produce a fully wired work.
...
- [x] **II — Capability-driven**: form pulls plugins by capability, not by
      id.
- [x] **III — Source-of-truth repos**: three repos per work; user
      owns them.
- [x] **IV — Trigger.dev**: generation is dispatched async.
- [x] **V — Forward-only migrations**: work schema evolution covered
...
- User-facing doc: [`../../../features/creating-a-work.md`](../../../features/creating-a-work.md)
- Related:
    - [`work-import/spec.md`](../work-import/spec.md)
    - [`works-config/spec.md`](../works-config/spec.md)
    - [`plugin-system/spec.md`](../plugin-system/spec.md)
- Implementation: `apps/web/src/app/[locale]/works/new/`,
  `apps/api/src/works/`,
