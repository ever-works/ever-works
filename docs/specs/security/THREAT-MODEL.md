...
Ever Works is a **managed multi-tenant SaaS** that _also_ ships as
...
- **Plugins are 1st-party only.** The runtime _technically_ loads any
...
| API → App Work repositories (fork, private copy, link)         | outbound (untrusted input)        | **Untrusted content + a token acting for the user.** Repository text is hostile input (rule 9); the platform writes one workflow file and `EW_` Actions secrets, never overwriting a hand-edited one. See [§B-1, §B-3](../features/app-works/THREAT-MODEL.md).       |
| API → App Work clusters (a user-pasted kubeconfig)             | outbound (untrusted credential)   | **Refused before any connection.** A kubeconfig that shells out, uses a token file or skipped verification is refused; `https` and public addresses only; dialled from an isolated worker, never the API. See [§B-4](../features/app-works/THREAT-MODEL.md).         |
| Tenant workloads → Ever Works Apps zone                        | inbound (untrusted workload)      | **Admitted only while the tier gate is green.** One namespace per App Work, sandboxed runtime, non-root, no service-account token, deny-by-default networking and no platform credential in a tenant. See [§B-5](../features/app-works/THREAT-MODEL.md).             |
| Ever Works → upstream maintainers (PRs)                        | outbound in the member's name     | **The member's own token and the member's own approval.** Never a platform or installation token, never auto-approved or bulk-approved, rate-limited; the platform never merges or comments upstream. See [§B-9](../features/app-works/THREAT-MODEL.md).             |
| Launcher hosts → API (delegated read)                          | inbound                           | **Read-only, on marked routes only.** `@DelegatedRead(scope)` handlers, audience `ever-works`, lifetime ≤ 3,600 s, and ≤ 50 `https` origins from an operator allow-list; anything else is refused. See [§B-7](../features/app-works/THREAT-MODEL.md).                |
| Runtime catalogs → builds and launcher                         | inbound (supply chain)            | **Curated, but a supply chain.** Rows are validated and sanitized, a `red` licence class drops a row whatever it claims, licence class gates hosting, and only verified Blueprints reach the managed tier. See [§B-6](../features/app-works/THREAT-MODEL.md).        |
| Git provider base URL ← E2E fake switch                        | local (test-only redirect)        | **Non-production only.** `EVER_WORKS_E2E_FAKES` is honoured only when `NODE_ENV` is not production and is set in the PR lanes alone, so no production path redirects Git provider calls. See [§B-8](../features/app-works/THREAT-MODEL.md).                          |
...
| 2026-09-17 | App Works program (SK-06/XC-03)                | App Works trust boundaries added to §2 (B-1…B-11).                  |
