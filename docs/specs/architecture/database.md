...
| *(`Work` column)*       | `works.kb_config`           | `simple-json` per-Work KB configuration: storage plugin override, retrieval budget, per-class inheritance mode (`inherit` / `override` / `disabled`).                                                                                                                |
