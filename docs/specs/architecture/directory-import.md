...
Directory import is the platform's "bootstrap from an existing
...
**post-completion sync** that ensures the resulting directory looks
identical to one created via AI Creation.

The user-facing description lives in
[`features/directory-import/spec`](../features/directory-import/spec.md);
...
└── directory-import.task.ts               # Trigger.dev task wrapper
```

And the API surface:

```
apps/api/src/directories/import/           # Controllers + DTOs
```
...
3. Probe for **data-repo markers**:
    - `config.yml` at root
    - `data/` directory with at least one item file
...
Source repo                          Platform
    ...
    │                                    │
    │                                    ▼
    │                              Insert directory + relations + repo refs
    │                                    │
...
Source README                        Platform
    ...
    │                                    ├─ Insert directory shell
    │                                    ├─ Create 3 git repos
    │                                    ├─ Dispatch directory-generation
...
generation. The directory immediately appears in the dashboard with
...
Returns a `DirectoryImportResult` with the new directory id, the
...
if (sourceType === 'awesome-readme') {
	const directory = await this.createDirectoryShell(input);
	const repos = await this.createThreeGitRepos(input, directory);
	...
	return { directoryId: directory.id, repos, generationRunId: runId };
}

if (sourceType === 'data-repo') {
	const directory = await this.createDirectoryShell(input);
	if (input.importMode === 'copy') {
		await this.copyDataRepoToOwnedRepo(input.sourceUrl, directory);
	} else {
		directory.dataRepository = parseRepositoryReference(input.sourceUrl);
	}
	await this.copyEntities(directory); // members, plugins, schedule from works.yml
	return { directoryId: directory.id };
}

// link-existing
const directory = await this.createLinkedDirectory(input);
return { directoryId: directory.id };
```

Each branch ends with a `directory_imported` activity-log entry
...
All three run **before** any side effect (no orphan directories from
a half-completed validation).

## 10. Cancellation

Awesome imports inherit cancellation from the underlying
`directory-generation` task — see
...
- `directory_imported` activity-log entry with `details.sourceType`,
...
`apps/api/test/directories/import.e2e-spec.ts`.
...
| V — Forward-only migrations | No new schema; uses existing `directories` + relations.                              |
...
- Source:
    - `packages/agent/src/import/`
    - `packages/tasks/src/tasks/trigger/directory-import.task.ts`
    - `apps/api/src/directories/import/`
- Related specs:
    - [`features/directory-import/spec`](../features/directory-import/spec.md) (user-facing)
    - [`features/works-config/spec`](../features/works-config/spec.md)
    - [`features/creating-a-directory/spec`](../features/creating-a-directory/spec.md)
    - [`pipeline-executor`](./pipeline-executor.md)
    - [`trigger-worker`](./trigger-worker.md)
- User docs: [`docs/features/directory-import.md`](../../features/directory-import.md)
