...
1. A user manually generates a work and saves a prompt/config (`config.metadata.last_request_data` and `config.metadata.last_request_data`).
...
4. `WorkScheduleService` persists schedule information and mirrors denormalized data onto the `Work` entity (so dashboards can render state quickly).
5. `WorkScheduleDispatcherService` runs on a cron-like cadence (Trigger.dev task or Nest fallback). Each run fetches due schedules in batches and enqueues generation runs.
6. `WorkGenerationService.runScheduledUpdate()` reuses the same pipeline as manual runs, pulling `last_request_data` and invoking Trigger.dev or in-process fallbacks.
...
- **Work** (denormalized fields) – `scheduledUpdatesEnabled`, `scheduledCadence`, `scheduledNextRunAt`, `scheduledStatus` keep the dashboard fast.
...
- `WorkScheduleService`
    - Ensures the caller owns the work.
    ...
    - Mirrors schedule state back to the `Work` table.
...
- `WorkScheduleDispatcherService`
    ...
    - Invokes `WorkGenerationService.runScheduledUpdate()` for each reserved schedule.

- `WorkGenerationService`
    - Reuses last request data from the work config (handles stale config gracefully by pausing schedule).
    ...
    - Calls back into `WorkScheduleService.markRunCompleted()` or `.markRunFailed()` when the run finishes.
    - **Drift Prevention**: Calculates next run time based on the _scheduled_ time, not completion time.

### Trigger.dev + Fallback Cron

- Primary mode: `packages/tasks/src/tasks/trigger/work-schedule-dispatcher.task.ts` registers a Trigger.dev cron task (default every 5 minutes). The task boots a Nest context, resolves `WorkScheduleDispatcherService`, and processes the batch.
...
- `runScheduledUpdate()` loads the full `Work` and `User`, validates subscription limits (pausing if exceeded), fetches `last_request_data`, and calls `updateItemsGenerator()`.
...
- When **true**:
    ...
    - `WorkScheduleService` enforces `plan.maxWorks`.
...
- When **false**:
    ...
    - `WorkScheduleService` skips work-count limits and never forces pay-per-use.
    - `UsageLedgerService` becomes a no-op.
    - `WorkScheduleDto.planCode` is omitted, and the UI hides billing controls.
...
1. `WorkLayout` fetches `/works/:id` + `/works/:id/config`. If `config.metadata.last_request_data` is missing, the schedule tab is hidden and `/schedule` routes return 404.
2. `WorkSchedulePage` fetches `/works/:id/schedule`. If the API fails (no schedule yet), the card renders an empty state prompting a manual refresh.
3. `WorkScheduleCard` displays:
...
4. Client actions (`updateWorkSchedule`, `runWorkSchedule`, `cancelWorkSchedule`) call server routes which wrap the API. Success/failure toasts come from `sonner`.

---

## 7. Trigger.dev Task Summary

- **File**: `packages/tasks/src/tasks/trigger/work-schedule-dispatcher.task.ts`
- **Trigger**: `onSchedule({ cron: '*/5 * * * *' })` (configurable via env).
- **Workflow**:
    1. Boot Nest application context.
    2. Resolve `WorkScheduleDispatcherService`.
...
- `maxFailureBeforePause` defaults to `SCHEDULED_UPDATES_MAX_FAILURE_BEFORE_PAUSE` but can be set per work (bounded 1–10).
...
1. **UI**: `WorkLayout` only enables the schedule tab when the config contains an initial prompt. Otherwise, `/schedule` routes return 404.
2. **Backend**: `WorkScheduleService.ensureWorkConfigReady()` loads the config using `DataGeneratorService.config()` for every `getSchedule` and `updateSchedule` call. If the prompt is missing, it throws a `400` (“Complete an initial work setup before enabling scheduled updates.”).
