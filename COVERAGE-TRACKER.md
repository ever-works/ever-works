...
- [x] `integrations/twenty-crm` — 107 tests, 2026-05-07 (utils retry+mapping, config+guard+decorator, services twenty-crm+crm-tenant+client, controllers companies+people) — (this PR).
- [ ] `config`, `events`, `integrations/github-app` (large; existing partial coverage — `github-app.service.spec.ts`, `github-app-onboarding.service.spec.ts`, `github-app-sync.service.spec.ts` already shipped, controllers + module test pending)
