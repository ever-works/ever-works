...
- [ ] `works/*` (largest surface — split by sub-module)
  ...
  - [ ] `works/works.controller.ts` (1716 lines) — being split per endpoint group across multiple PRs:
    ...
    - [ ] generation + cancellation (generateItems / updateItemsGenerator / cancelGeneration / generator-form / generator-form/:id / generate-details).
    - [ ] schedule (get / update / cancel / run).
    - [ ] items mutations (submit-item / remove-item / update-item / check-item-health / extract-item-details / bulk-capture-images).
    - [ ] domain + repo (domain-type / regenerate-markdown / update-readme / update-website / switch-website-template / sync-data / repositories/visibility GET+PUT).
    - [ ] advanced-prompts (GET + PUT).
    - [ ] import (analyze / analyze-for-linking / import / repositories list).
    - [ ] taxonomy (categories CRUD / tags CRUD / collections CRUD).
    - [ ] community-pr.
    - [ ] comparisons (list / remaining-count / generation-status / get / generate / generate-manual / delete).
    - [ ] source-validation (GET + PUT).
