...
| 2026-05-07 | api auth (api-key + auth.service) | (this PR) | Adds 60 unit tests across `apps/api/src/auth/services/`: `ApiKeyService` (15) — 10-key cap, past-expiry rejection, `ew_live_` prefix + sha256 hashing, unique-key generation, validateKey expiry semantics + fire-and-forget `updateLastUsed`; `AuthService` (45) — assertCanRegister, validateSocialUser (new-user creation w/ trusted-email confirmation emit, unverified-email gating w/ provider-link bypass, suspended-account rejection, upsertProviderAccount field mapping incl. defaults), sendVerificationEmail (24h expiry + callback-URL token-append-vs-default), verifyEmail, forgotPassword (1h expiry, callback-URL handling, generic-message safety on unknown email), getUserByPasswordResetToken, consumePasswordResetToken, getUserProfile (sensitive-field stripping, github repo-scope gating, expired-token filtering), updateUserProfile (selective updates, committer-field clearing), validate{Email,Password}Token. Mocks `@ever-works/agent/{database,entities}` to avoid the agent runtime tree. |
...
- [ ] `auth/services/social-auth.service` (still pending)
